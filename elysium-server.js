...
const VERSION = '2.6.38';   // srv-2.6.38 (21 Sep 2026, AN OUST PAYS THE LIVING AND MOVES THE TURN -- restart): from the bot project's 21 Sep spec (items 3 + 6). `bounty` and `hostOust` paid the ring predecessor without asking whether it was still in the game -- a dead seat collected the VP and the 6 pool (measured live: seat 2 out, seat 3 falls, the bounty went to seat 2). prevLiveSeat() skips `out` (NOT vacant: a vacant seat is still in the game). And an oust of the seat that HOLDS the turn now hands it on (turnOnFromOusted -> nextLiveSeat, after the roster), as vacateSeat always has and as the protocol doc had promised for `bounty` all along; the ousted seat's own late `pass` is silently ignored (bot v0.6.140+ still sends one), a live seat passing out of turn still errs. No new verb, no new field. Previous note: elysium-CHANGELOG.md.
...
/* v2.5.68 (L1): named flags ride alongside the two positionals.
   ...
   --referee-pass <p> (alias --admin-pass) the keys: hello {admin} grants per-connection referee/admin (inherits host gates)
   ...
   --bind <host>       listen address (default: all interfaces). A tunnel deployment binds
                       127.0.0.1 so the raw port is unreachable even from the LAN (plan H1-a)
   --data-dir <path>   where elysium-rooms.json / elysium-saves.json live (default: beside this
                       server file -- EXACTLY the pre-2.6.20 behavior). A hardened systemd deploy
                       points this at its ReadWritePaths dir so code can stay root-owned read-only
   --origin <list>     comma-separated WebSocket Origin allowlist (default: off = accept all,
                       the pre-2.6.21 behavior). Public deploy: --origin https://elysium.cards,null
                       ('null' keeps the downloaded file:// client working). Requests WITHOUT an
                       Origin header always pass (bots/tools -- CSWSH needs a browser, and browsers
                       always send one). Closes cross-site WS abuse-by-proxy; see PROTOCOL history
   Flags live in the mutable CFG object so the dispatch test-suite can simulate them. */
const _argv = process.argv.slice(2);
const CFG = { srvPassHash: null, adminPassHash: null, createPolicy: 'anyone', trustProxy: false, bind: null, dataDir: null, origin: null };
const _pos = [];
for(let i = 0; i < _argv.length; i++){
  const a = _argv[i];
  if(a === '--server-pass'){ CFG.srvPassHash = passHashLater(_argv[++i]); }
  else if(a === '--admin-pass' || a === '--referee-pass'){ CFG.adminPassHash = passHashLater(_argv[++i]); }   // srv-2.6.34: --referee-pass is the clearer name (a referee = spectator with host powers); the wire field stays `admin`
  else if(a === '--create-policy'){ CFG.createPolicy = _argv[++i] === 'admin' ? 'admin' : 'anyone'; }
  else if(a === '--trust-proxy'){ CFG.trustProxy = true; }
  else if(a === '--bind'){ CFG.bind = String(_argv[++i] || '') || null; }
  else if(a === '--data-dir'){ CFG.dataDir = String(_argv[++i] || '') || null; }
  else if(a === '--origin'){ const v = String(_argv[++i] || '').trim(); CFG.origin = v ? new Set(v.split(',').map(x => x.trim()).filter(Boolean)) : null; }
  else _pos.push(a);
}
...
const MAX_ROOMS   = 50;
const RESERVED_POPULATED = 10;      // v2.6.22: of MAX_ROOMS, at least this many slots are HELD for rooms that have ever seated a player -- an
                                    // empty-room flood (botnet resource-squatting: many IPs, 4 rooms each, sockets held open) can fill at most
                                    // MAX_ROOMS - RESERVED_POPULATED = 40 slots, so a real game can always still be created. (Johan's catch, 24 Jul.)
const NEVER_POP_GRACE_MS = 10 * 60000;   // a room that has NEVER seated anyone is reaped after this (vs the 60-min all-offline rule for real rooms)
const BOT_ROOM_GRACE_MS  = 10 * 60000;   // srv-2.6.36: a room with bots seated and no human online -- the bots live in the host's tab, so there is nothing left to come back to; the full hour is for a real table mid-match
const MAX_PLAYERS = 6;
...
const MAX_CONN_TOTAL  = 400;         // absolute socket ceiling, all IPs INCLUDING loopback -- the memory guard (hosting plan H1-b; 400 > MAX_ROOMS*MAX_PLAYERS=300 so full legitimate load never trips it)
const MAX_WATCHERS    = 10;          // spectators per room (hosting plan H1-c)
const SAVES_PER_GROUP = 3;           // named saves per password group -- same passHash = same friend group, the saveList scoping rule (hosting plan H1-e / decision D4)
const SAVE_TTL_MS     = 90*24*3600000;  // a save untouched (no save/load) this long expires (D4); loadMatch refreshes touchedAt
...
process.on('uncaughtException',  err => { console.log('[uncaught] ' + (err && err.stack || err)); try{ errAppend(serverErrRecord('uncaught', err)); }catch(e){} });   // v2.6.25 amend: the server reports on itself into the same JSONL -- survival semantics unchanged by design (rooms outlive a stray exception; snapshots + systemd are the escalation path)
process.on('unhandledRejection', err => { console.log('[unhandled] ' + (err && err.stack || err)); try{ errAppend(serverErrRecord('unhandled', err)); }catch(e){} });
...
function wsSendFrame(sock, op, payload){
  ...
  if(sock.writableLength > 2 * 1024 * 1024){              // v2.6.21: slow-reader guard -- a peer that accepts pings but never drains its
    try{ sock.destroy(); }catch(e){}                      // receive buffer would otherwise grow our send buffer without bound. 2 MB of
    return;                                               // queued frames = not a player anymore. (Harness fake socks lack the property:
  }                                                       // undefined > cap is false, so tests exercise the normal path untouched.)
...
const ERR_MAX      = 10;                 // B-54: client-error posts per IP...
const ERR_WIN      = 10 * 60000;         // ...per 10 minutes
const ERR_BODY_MAX = 64 * 1024;          // hard request-body cap -- the server never read bodies before this endpoint; keep it tiny and paranoid (B-63: 32 -> 64 kB, the dump budget + headroom)
const ERR_RING_MAX = 200;                // B-63: report ring depth, lockstep with the client's RING_MAX (was an inline 30)
const ERR_LOG_MAX  = 1024 * 1024;        // rotate elysium-errors.log at 1 MB (rename to .1, clobbering the old .1)
const ERR_TOTAL_MAX= 4 * 1024 * 1024;    // .log + .1 disk guard of last resort
const ipErrs = new Map();
const ipLogAuth = new Map();   // v2.6.25 amend: failed admin-key attempts against the admin HTTP routes (log pull + maintenance; same window/max as the intake)
let maint = null;              // v2.6.26: {until: ms-epoch, msg, setAt} -- while armed, room creation pauses (admins exempt) and sys notices go out
function maintFile(){ return path.resolve(DATA_DIR, 'maintenance.json'); }
function maintActive(){
  if(maint && Date.now() >= maint.until){ maint = null; try{ fs.unlinkSync(maintFile()); }catch(e){} }   // auto-expiry: the server reopens itself
  return !!maint;
}
function maintNotice(){
  if(!maint) return '';
  const mins = Math.max(1, Math.round((maint.until - Date.now()) / 60000));
  const when = mins >= 90 ? (Math.round(mins / 60) + ' hour(s)') : (mins + ' min');
  return (maint.msg ? maint.msg + ' ' : 'Server maintenance is planned. ')
       + '(New games are paused; please finish ongoing games within ' + when + '.)';
}
function saveMaint(){ try{ fs.writeFileSync(maintFile(), JSON.stringify(maint)); }catch(e){} }
function loadMaint(){ try{ const m = JSON.parse(fs.readFileSync(maintFile(), 'utf8')); if(m && m.until > Date.now()) maint = m; else try{ fs.unlinkSync(maintFile()); }catch(e2){} }catch(e){} }   // an expired file is tidied at boot, not left lingering
...
function helperMode(policy){          // srv-2.6.33: the create-time helper mode, read back from the stored policy (the client's helperPolicyFor() inverse)
  const p = policy || {};
  return p.tournament ? 'tournament' : (p.allowLocal === false ? 'all' : 'local');
}
function buildRoomList(){
  const out = [];
  rooms.forEach(r => {
    out.push({ room: r.name, started: !!r.started, lobby: !r.started,
               board: r.boardMode === 'classic' ? 'classic' : 'structured',   // srv-2.6.33: the table shape, so Lite can tag Full tables
               ref: !!r.refHash || undefined,                                   // srv-2.6.35: the room has a referee password
               helpers: helperMode(r.helperPolicy),
...
const HELPER_KEYS = ['on','unlockPrompt','counts','passConfirm','warn','edge','hand','handReminder','pathBlood','resBlock','resVote','resCombat','modeChoice','chatFx'];   // srv-2.6.37: +chatFx, the calmer chat-text animation tier (v2.6.107). THIS LIST AND THE CLIENT'S HELPER_DEFS ARE ONE CONTRACT: cleanHelperPolicy builds settings ONLY from here, so a key the client knows and this list does not is silently dropped from every host-locked policy.   // srv-2.6.24 (R1 port): +modeChoice, the pre-commit Inferior/Superior chooser   // host-lockable helpers — mirror the client's HELPER_DEFS (v2.6.15: +the three Played-tab resolvers)
...
function hostOrRef(conn, room, p){   // srv-2.6.35: the host, the server operator (hello{admin}), or this room's referee (a watcher who joined with the room's referee password)
  return !!(conn.admin || (p && p.token && p.token === room.hostToken) || (conn.ref && conn.room === room));
}
const REF_VERBS = new Set(['start','hostSetPool','hostOust','hostUnoust','setHelperPolicy','hostPass','hostSetClock','kick','closeRoom','demote','promote','mute','saveMatch','listSaves','delSave','loadMatch']);   // srv-2.6.35: the host-gated verbs a referee watcher may use
function roster(room){
  return room.players.map((p, i) => ({
    ...
    bot: p.bot || undefined                          // bot-elements exception: any player may control this seat's cards
  }));
}
...
/* srv-2.6.36 (Johan): a room whose only remaining players are BOTS is over.
   `room.players` has never distinguished a human from a bot -- p.bot is a
   self-declaration on join (1336/1441) and only ever fed the roster's
   control-my-elements exception -- so this is the first place the server counts
   them. `humansLeft` deliberately counts seats in the ROSTER, not sockets: a
   human who drops their connection is still a player and keeps their room
   (that is what the reconnect window is for), while a human who really LEAVES
   is gone and a table of bots alone has nobody to play for. */
function humansLeft(room){ return (room.players || []).filter(q => !q.bot).length; }
function closeRoomNow(room, msg){
  if(!room || !rooms.has(room.name)) return false;                 // idempotent: two paths may race to close the same room
  const socks = [];
  (room.players || []).forEach(q => { if(q.sock) socks.push(q.sock); });
  (room.watchers || []).forEach(w => { if(w.sock) socks.push(w.sock); });
  bcast(room, { t:'closed', msg });                                 // say why BEFORE the door shuts -- the client tears down on this and must not try to rejoin a room that no longer exists
  rooms.delete(room.name); scheduleRoomList();
  socks.forEach(sk => { try{ wsClose(sk); }catch(err){} });         // each close fires drop*(), which operates on the now-detached room object: harmless, and rooms.delete is already done
  console.log('[room] "' + room.name + '" closed (' + msg + ')');
  return true;
}
function rosterUpd(room){ bcast(room, { t:'roster', players: roster(room), watchers: (room.watchers || []).length, watcherList: (room.watchers || []).map(w => ({ name: w.name, muted: !!w.muted, ref: !!w.ref || undefined })) }); }
...
function lobbyState(room){
  return { t:'lobby',
    ...
    players: room.players.map((q, i) => ({
      ...
      seat: q.seatPick || null, muted: !!q.muted, bot: q.bot || undefined,
      deck: !!(q.game && q.game.deckLib && q.game.deckLib.length) })),
    watchers: (room.watchers || []).map(w => ({ name: w.name, muted: !!w.muted, ref: !!w.ref || undefined })) };   // srv-2.6.35: the lobby marks referees
}
...
/* srv-2.6.38 (bot-project spec 21 Sep, items 3 + 6): an oust needs two things the
   server did by accident or not at all. (a) THE LIVE PREDATOR: `bounty` and `hostOust`
   took the ring predecessor `(i - 1 + n) % n` without asking whether that seat was
   still in the game -- at a table where it had already been ousted, the 6 pool and
   the VP went to a dead seat and the real predator got nothing. Only `out` is skipped:
   a VACANT seat is still in the game (its deck and board wait for a replacement) and
   stays the predator, as before. Returns -1 when nobody else is left. (b) THE TURN:
   when the seat that HOLDS the turn is ousted (its own Fame, a referendum it called,
   the host's oust) the turn stayed on it, and only a hostPass could move the table.
   vacateSeat has handed the turn on in exactly this situation since it was written
   (below); an oust now does the same. The ousted seat's own late `pass` is refused by
   the v2.5.64 sender guard, so the turn can never be advanced twice. */
function prevLiveSeat(room, fromIdx){
  const n = room.players.length;
  for(let k = 1; k < n; k++){ const c = ((fromIdx - k) % n + n) % n; const q = room.players[c]; if(q && !q.out) return c; }
  return -1;
}
function turnOnFromOusted(room, idx){
  if(!room.started || room.turnSeat !== idx + 1) return;
  const ni = nextLiveSeat(room, idx);
  if(ni === idx) return;                                            // nobody left to hand it to -- the game is over, leave the marker where it is
  room.turnSeat = ni + 1;
...
function joinReply(sock, room, p){
  ...
  wsSend(sock, msg);
  if(maintActive()) wsSend(sock, { t:'sys', msg: maintNotice() });   // v2.6.26: late joiners hear about the drain too
  ...
  if(room.tally) wsSend(sock, Object.assign({ t:'tally', who: '' }, room.tally));   // v2.6.11: a live resolver survives a reconnect / late join
}
...
function expandDeck(list){
  ...
  if(Array.isArray(list)) for(const e of list){
    ...
    const qty  = Math.max(1, Math.min(parseInt(e.qty, 10) || 1, 90));   // v2.6.25 amend: lockstep with client parseDeck -- VTES has no per-card copy limit, 90 = the library ceiling; MAX_PILE (250) stays the total backstop
    if(!name) continue;
...
const GAME_HANDLERS = {
  ...
  deck(conn, room, p, m){
    ...
    if(room.tournLock && (crypt.length < 12 || lib.length < 60 || lib.length > 90))   // v2.6.25 amend: tournament rooms enforce legality server-side (VEKN: 12+ crypt, 60-90 library) on the EXPANDED counts -- a modified client cannot smuggle an illegal deck past the client gate; existing err verb, zero wire change
      return wsSend(conn.sock, { t:'err', msg:'Tournament rooms require a legal deck: at least 12 crypt and 60-90 library (this one is ' + crypt.length + ' crypt / ' + lib.length + ' library).' });
  ...
  pass(conn, room, p, m){
    ...
    if(p.out && room.turnSeat !== i + 1) return;                 // srv-2.6.38: an OUSTED seat's late pass is expected, not an error -- the oust itself has already handed the turn on (turnOnFromOusted), and bot v0.6.140+ still passes by itself afterwards. Measured live: without this line every such oust put an `err` into the bot's trace. Silent, and the turn is never advanced twice
  ...
  start(conn, room, p, m){
    if(!hostOrRef(conn, room, p)) return wsSend(conn.sock, { t:'err', msg:'Only the host can start the game.' });
    if(room.started)               return wsSend(conn.sock, { t:'err', msg:'The game has already started.' });
    const N = room.players.length;
    ...
    room.started = true;
    room.turnSeat = 1;
    room.tally = null;                                                         // v2.6.11: a resolver never rides into a fresh match
  ...
  tally(conn, room, p, m){
    // v2.6.11: shared Played-tab resolver counters (Block / Vote / Combat). Every message
    // carries the WHOLE state (mode + both numbers, absolute) -- last write wins, which is
    // self-healing under concurrent clicks -- and ANY seat may adjust (no driver/reactor
    // roles; the table referees, sandbox rule). Stored on the room so a late joiner or a
    // reconnect mid-referendum sees the live counters (joinReply); cleared on start.
    if(!room.started) return;
    const mode = (m.mode === 'block' || m.mode === 'vote' || m.mode === 'combat' || m.mode === 'free') ? m.mode : null;   // v2.6.17: +'free' -- the client shipped it in v2.6.37 ("behaves exactly like Block/Vote/Combat"), but this validator was still nulling it, so free-mode counters never actually SHARED; found during the tally-race audit
    /* v2.6.17 (bot-track tally-race audit): ABSENT fields now PRESERVE the room's
       current value while the mode is UNCHANGED -- the whole-state last-write-wins
       design (v2.6.11) was self-healing under human click speeds, but two BOTS
       writing within the same ~100 ms raced: the attacker's stealth write (carrying
       its last-OBSERVED b) clobbered the defender's just-written intercept, and a
       contested verdict read "Intercept 0" against a played +1. Same class, same
       cure as v2.6.14's duel map: a push from player A must never erase player B's
       fact. Fully backward-compatible: the human client always sends every field
       (avrPush sends the whole state), so presence-checks change nothing for it;
       only field-owning writers (the bots) send partials. A MODE CHANGE still
       starts fresh -- stale ph/rd from a previous referendum must not leak into a
       new one. */
    const prev = (room.tally && room.tally.mode === mode) ? room.tally : null;
    const a  = ('a'  in m) ? Math.max(0, Math.min(99, m.a | 0)) : (prev ? prev.a  : 0);
    const b  = ('b'  in m) ? Math.max(0, Math.min(99, m.b | 0)) : (prev ? prev.b  : 0);
    const ph = ('ph' in m) ? Math.max(0, Math.min(6, m.ph | 0)) : (prev ? prev.ph : 0);   // v2.6.12: combat phase index (the rulebook's 7 steps per round)
    const rd = ('rd' in m) ? Math.max(1, Math.min(99, (m.rd | 0) || 1)) : (prev ? prev.rd : 1);   // v2.6.13: combat round counter (bumped client-side when the phase wraps)
    let duel;
    if(mode === 'combat'){
      // v2.6.14: the duel map is SERVER-MERGED per seat -- combatants are per-seat facts, so a
      // whole-state counter push from player A must never erase player B's announcement. The
      // map survives counter pushes while mode stays 'combat' and is dropped the moment the
      // tally leaves combat (or clears), which also covers `start` via room.tally = null.
      duel = Object.assign({}, (room.tally && room.tally.mode === 'combat' && room.tally.duel) || {});
      if(m.duelCid !== undefined){
        const seat = room.players.indexOf(p) + 1;
        if(m.duelCid){ duel[seat] = { cid: cleanCard(String(m.duelCid)).slice(0, 16), name: cleanCard(String(m.duelName || '')).slice(0, 60), who: p.name }; }
        else delete duel[seat];                                         // duelCid:null = leave
      }
    }
    room.tally = mode ? (mode === 'combat' ? { mode, a, b, ph, rd, duel } : { mode, a, b, ph, rd }) : null;
    const out = { t:'tally', who: p.name, mode, a, b, ph, rd };
    if(mode === 'combat') out.duel = duel;
    if(m.res) out.res = 1;                                              // v2.6.12: transient resolve flag -- relayed, never stored (each client closes its batch + applies its OWN clear setting)
    bcast(room, out);   // no `except`: the sender's echo is an idempotent no-op and keeps every client rendering from the SAME broadcast
  },
  fx(conn, room, p, m){
    if(!room.started) return;
  ...
  bounty(conn, room, p, m){
    ...
    if(room.players.length > 1 && i > -1){
      const pi = prevLiveSeat(room, i), pred = pi > -1 ? room.players[pi] : null;   // srv-2.6.38: the LIVE predator
      if(pred){
      ...
      } else sys(room, p.name + ' was ousted.');
    }
    rosterUpd(room);
    if(i > -1) turnOnFromOusted(room, i);           // srv-2.6.38: after the roster, so every client knows WHY the turn moved
  },
  hostSetPool(conn, room, p, m){
    if(!hostOrRef(conn, room, p)) return wsSend(conn.sock, { t:'err', msg:'Only the host can set a player’s pool.' });
  ...
  hostOust(conn, room, p, m){
    if(!hostOrRef(conn, room, p)) return wsSend(conn.sock, { t:'err', msg:'Only the host can oust a player.' });
    ...
    if(room.players.length > 1){
      const pi = prevLiveSeat(room, i), pred = pi > -1 ? room.players[pi] : null;   // srv-2.6.38: the LIVE predator (q.out is already set, so q is never its own)
      if(pred){
    ...
    rosterUpd(room);
    turnOnFromOusted(room, i);                                         // srv-2.6.38
  },
  hostUnoust(conn, room, p, m){
    if(!hostOrRef(conn, room, p)) return wsSend(conn.sock, { t:'err', msg:'Only the host can do that.' });
    const q = room.players[(m.seat | 0) - 1];
  ...
  hostPass(conn, room, p, m){
    ...
    if(!hostOrRef(conn, room, p)) return wsSend(conn.sock, { t:'err', msg:'Only the host can pass a turn for someone.' });
    if(!room.started) return wsSend(conn.sock, { t:'err', msg:'The game has not started yet.' });
    const seat = m.seat | 0;
  ...
  hostSetClock(conn, room, p, m){
    ...
    if(!hostOrRef(conn, room, p)) return wsSend(conn.sock, { t:'err', msg:'Only the host can adjust the match clock.' });
    if(!room.started) return wsSend(conn.sock, { t:'err', msg:'The game has not started yet.' });
  ...
  closeRoom(conn, room, p, m){
    if(!hostOrRef(conn, room, p))
      return wsSend(conn.sock, { t:'err', msg:'Only the host can close the table.' });
    closeRoomNow(room, (p && p.name ? p.name : 'The host') + ' closed the table.');
  },
  ...
  mute(conn, room, p, m){
    if(!hostOrRef(conn, room, p)) return wsSend(conn.sock, { t:'err', msg:'Only the host can mute.' });
  ...
  saveMatch(conn, room, p, m){
    if(!hostOrRef(conn, room, p)) return wsSend(conn.sock, { t:'err', msg:'Only the host can save the match.' });
    ...
    const clash = saves.get(nm);                          // v2.6.19 (uncertainty round): saveMatch never checked passHash on OVERWRITE --
    if(clash && clash.passHash !== room.passHash)         // delSave always did. Pre-existing cross-group clobber; on a public instance it is
      return wsSend(conn.sock, { t:'err', msg:'That save name is taken by another table. Pick a different name.' });   // a griefing vector (guess a name, destroy the save)
    ...
    if(!saves.has(nm)){                                   // H1-e (D4): per password-group cap -- overwriting an existing save never counts
      let g = 0; saves.forEach(sv => { if(sv.passHash === room.passHash) g++; });
      if(g >= SAVES_PER_GROUP)
        return wsSend(conn.sock, { t:'err', msg:'Saved-match limit for this password group (' + SAVES_PER_GROUP + ') is reached. Delete one first.' });
    }
  ...
  listSaves(conn, room, p, m){
    if(!hostOrRef(conn, room, p)) return wsSend(conn.sock, { t:'err', msg:'Only the host can manage saved matches.' });
  ...
  loadMatch(conn, room, p, m){
    if(!hostOrRef(conn, room, p)) return wsSend(conn.sock, { t:'err', msg:'Only the host can load a saved match.' });
    ...
    sv.touchedAt = Date.now(); persistSaves();            // H1-e: an active campaign never expires -- the 90-day TTL counts from the last save OR load
...
function handle(conn, raw){
  ...
  if(m.t === 'create'){
    ...
    if(maintActive() && !conn.admin)                                           // v2.6.26: graceful drain -- existing games finish, new ones wait; admin connections test on
      return wsSend(conn.sock, { t:'err', msg: maintNotice() });
    ...
    if(!loopback && !conn.admin){                       // v2.6.23 (Johan's tournament nuance): the ADMIN bypasses the create throttle --
      if(strikes(ipCreates, conn.ip, CREATE_WIN) >= CREATE_MAX)   // a --create-policy admin server (the tournament mode) previously throttled
        return wsSend(conn.sock, { t:'err', msg:'Room creation limit reached — try again later.' });   // its own organizer at table 5. Admin is password-authenticated; trusted by definition.
      strike(ipCreates, conn.ip);
    }
    // reservation gate: count rooms that have ever seated someone; a brand-new empty shell may not consume the last reserved slots
    { let pop = 0; rooms.forEach(r => { if(r.everPop) pop++; });
      const emptyShells = rooms.size - pop;
      if(emptyShells >= (MAX_ROOMS - RESERVED_POPULATED))
        return wsSend(conn.sock, { t:'err', msg:'The server is busy with too many idle rooms right now — try again shortly.' }); }
    const mins = parseInt(m.minutes, 10);
    const rs   = parseInt(m.react, 10);
    // v2.6.22: two-tier admission -- a NEW room may only take a reserved-populated slot if the room being created will be occupied.
    // At creation the creator always becomes a seated player (below), so a create counts as populated; the guard bites the EMPTY-shell
    // pattern where sockets linger in rooms nobody sits in. Empty rooms already self-delete on last-leave (the reap below adds the
    // never-seated short grace). The reservation is enforced here: if the only free slots are the reserved ones, a create still
    // succeeds (creator sits down = populated), but it flips everPop true so it is NOT itself squattable.
    const room = { name, passHash: passHash(m.pass || ''), players: [], watchers: [], hostToken: null, created: Date.now(), everPop: false,
                   refHash: (m.refPass !== undefined && String(m.refPass) !== '') ? passHash(String(m.refPass).slice(0, 40)) : null,   // srv-2.6.35: the room's referee password (null = no referees at this table)
    ...
    room.players.push(p); room.everPop = true;       // v2.6.22: the creator is a seated player -> a legit new room is 'real' at once; only ABANDONED empty shells become squatter targets
    rooms.set(name, room);
    conn.room = room; conn.player = p;
    if(m.bot === true) p.bot = true;                 // bot-elements exception (Johan, 13 July): a self-declared seat whose elements any player may control, since a headless bot cannot model every triggered ability
  ...
  if(m.t === 'join'){
    ...
    if(m.token){
      ...
      if(p){
        ...
        p.sock = conn.sock; p.last = Date.now(); p.off = 0; room.everPop = true;   // v2.6.22: a seated player marks the room real
        conn.room = room; conn.player = p;
    ...
    let refJoin = false;                                                       // srv-2.6.35: Join as referee -- the key is the ROOM's, the strike key is the room password's
    if(m.refPass !== undefined && String(m.refPass) !== ''){
      if(!room.refHash) return wsSend(conn.sock, { t:'err', msg:'This room has no referee password.' });
      if(!passMatch(room.refHash, String(m.refPass))){ strike(ipFails, failKey); return wsSend(conn.sock, { t:'err', msg:'Wrong referee password.' }); }
      refJoin = true;
    }
    if(m.spect || refJoin){
      room.watchers = room.watchers || [];
      if(room.watchers.length >= MAX_WATCHERS)            // H1-c: per-room spectator cap (none existed before -- hosting audit 17 July 2026)
        return wsSend(conn.sock, { t:'err', msg:'Spectator limit for this room (' + MAX_WATCHERS + ') is reached.' });
      ...
      room.watchers.push({ name: pname, sock: conn.sock, ref: refJoin || undefined });
      conn.room = room; conn.watch = true; conn.wname = pname; conn.ref = refJoin;
      const msg = { t:'joined', room: room.name, spect: true, ref: refJoin || undefined, you: pname,
      ...
      if(maintActive()) wsSend(conn.sock, { t:'sys', msg: maintNotice() });   // v2.6.26: late joiners hear about the drain too
    ...
    room.players.push(p);
    conn.room = room; conn.player = p;
    if(m.bot === true) p.bot = true;                 // bot-elements exception (Johan, 13 July) — see the create-path comment
    if(m.anon === true) p.anon = true;               // v2.5.68 L2
    joinReply(conn.sock, room, p);
    vWarn(conn, m);
  ...
  if(conn.room && conn.watch){
    if(conn.ref && REF_VERBS.has(m.t)){                                       // srv-2.6.35: a referee is a watcher with the host's verbs -- a pseudo-p carries the name for sys lines and the q===p guards
      const rp = { name: '\u2696 ' + conn.wname, token: null, ref: true, watcher: true };
      const h = GAME_HANDLERS[m.t]; if(h) h(conn, conn.room, rp, m);
      return;
    }
    if(m.t === 'chat'){
...
function dropWatcher(conn){
  ...
  rosterUpd(room);
  if(!room.started) bcastLobby(room);
  conn.room = null; conn.watch = false; conn.ref = false;
}
function dropPlayer(conn, forGood){
  ...
  else if(forGood && !humansLeft(room)){                            // srv-2.6.36 (Johan): the last human LEFT and only bots are seated -- close now instead of letting the hour-long all-offline rule hold an unplayable room in the list. Only on a real leave: a disconnect keeps its reconnect window (see the reaper's bot grace)
    closeRoomNow(room, 'The last player left — the table closed.');
  }
...
/* ---------- B-54: client error/report intake (design: elysium-error-telemetry-design.md) ---------- */
function errScrub(v, cap){ return String(v==null?'':v).replace(/[\u0000-\u001f\u007f]+/g,' ').slice(0, cap); }
function errRecord(m){                       // pure: parsed body -> whitelisted record (null = drop). Unit-tested in test-server-dispatch.js.
  if(!m || typeof m!=='object') return null;
  const rec={ ts:new Date().toISOString(),
    kind: m.kind==='report' ? 'report' : 'crash',
    ver: errScrub(m.ver || (m.summary && m.summary.ver), 16),   // v2.6.27: reports carry ver only inside summary -- mirror the ua fallback below, or the digest histogram shows '?' (found by Johan's first real report)
    ua:  errScrub(m.ua || (m.summary && m.summary.ua), 120) };
  if(m.msg!==undefined)   rec.msg   = errScrub(m.msg, 300);
  if(m.src!==undefined)   rec.src   = errScrub(m.src, 200);
  if(m.line!==undefined)  rec.line  = m.line|0;
  if(m.col!==undefined)   rec.col   = m.col|0;
  if(m.stack!==undefined) rec.stack = errScrub(m.stack, 2000);
  if(m.mode!==undefined)  rec.mode  = errScrub(m.mode, 40);
  else if(m.summary && m.summary.mode!==undefined) rec.mode = errScrub(m.summary.mode, 40);   // v2.6.27: the SAME two-shape fallback as ver/ua -- field discipline, applied everywhere it fits
  if(m.note!==undefined)  rec.note  = errScrub(m.note, 500);
  if(Array.isArray(m.ring)) rec.ring = m.ring.slice(-ERR_RING_MAX).map(e =>
    (e && typeof e==='object' && e.m!==undefined) ? { t:dInt(e.t), m:errScrub(e.m, 200) }   // B-63: v2.6.60+ clients send {t,m} -- keep the timestamp (dInt, NOT |0: ms epochs overflow int32)
                                                  : errScrub(e, 200));                      // <=2.6.58 clients send bare strings -- the two-shape discipline, both shapes on disk
  if(m.summary && typeof m.summary==='object'){
    const su=m.summary;
    rec.summary = { mode:errScrub(su.mode,20), libN:su.libN|0, cryptN:su.cryptN|0,
                    hand:su.hand|0, cards:su.cards|0, deckName:errScrub(su.deckName,60) };
  }
  if(m.dump && typeof m.dump==='object'){ const dr=dumpRecord(m.dump); if(dr) rec.dump=dr; }   // B-63: the support bundle, whitelisted level by level
  if(m.dumpTrunc) rec.dumpTrunc=1;           // B-63: the client's degrade ladder dropped the dump to fit the budget
  if(m.dumpErr)   rec.dumpErr=1;             // B-63: buildDump threw client-side; the report still arrived (T7)
  return rec;                                // note: NO ip field -- the address lives only in the in-memory limiter
}
function dInt(v){ const n=Math.floor(+v); return Number.isFinite(n)?n:0; }   // B-63: safe integer coercion for ms epochs / long uptimes -- |0 would silently int32-wrap them
function dumpRecord(d){                      // B-63, pure: parsed dump -> whitelisted record (null = drop). The errRecord pattern one level down; unknown keys never reach disk at ANY depth. Unit-tested in test-server-dispatch.js.
  if(!d || typeof d!=='object') return null;
  const out={};
  if(d.game && typeof d.game==='object'){ const g=d.game; const gg={};
    gg.turn=dInt(g.turn); gg.phase=dInt(g.phase);
    gg.activeSeat=(g.activeSeat==null)?null:dInt(g.activeSeat);
    gg.mySeat=(g.mySeat==null)?null:dInt(g.mySeat);                          // null is meaningful: a spectator / unseated reporter
    if(Array.isArray(g.seats)) gg.seats=g.seats.slice(0,8).map(s=>{ if(!s||typeof s!=='object') return {};
      const r={ s:dInt(s.s), name:errScrub(s.name,40), pool:(s.pool==null)?null:dInt(s.pool) };
      if(s.out) r.out=1; if(s.vac) r.vac=1; return r; });
    gg.deckName=errScrub(g.deckName,60);
    gg.libN=dInt(g.libN); gg.cryptN=dInt(g.cryptN); gg.handN=dInt(g.handN);
    if(Array.isArray(g.hand)) gg.hand=g.hand.slice(0,120).map(n=>errScrub(n,60));
    if(Array.isArray(g.board)) gg.board=g.board.slice(0,600).map(c=>{ if(!c||typeof c!=='object') return {};
      const r={ id:errScrub(c.id,24), z:errScrub(c.z,16), s:(c.s==null)?null:dInt(c.s), x:dInt(c.x), y:dInt(c.y) };
      if(c.l) r.l=1; if(c.fd) r.fd=1;
      if(c.n!==undefined) r.n=errScrub(c.n,60);                              // ABSENT on opponents' face-down stubs -- the client filtered capture-side; a modified client could lie, the server cannot verify (manifest S6)
      if(c.k!==undefined) r.k=errScrub(c.k,2);
      if(c.b) r.b=dInt(c.b); if(c.u) r.u=dInt(c.u); if(c.g) r.g=dInt(c.g);
      if(Array.isArray(c.c)) r.c=c.c.slice(0,12).map(s=>errScrub(s,40));
      if(c.h!==undefined) r.h=errScrub(c.h,24);
      if(Array.isArray(c.att)) r.att=c.att.slice(0,12).map(s=>errScrub(s,24));
      if(c.sup!==undefined) r.sup=errScrub(c.sup,8);
      return r; });
    out.game=gg;
  }
  if(d.env && typeof d.env==='object'){ const e=d.env; const ee={};
    ee.ua=errScrub(e.ua,120); ee.screen=errScrub(e.screen,12);
    ee.dpr=Number.isFinite(+e.dpr)?Math.round(+e.dpr*100)/100:1;
    ee.lang=errScrub(e.lang,16); ee.uptimeMs=dInt(e.uptimeMs);
    if(e.ws && typeof e.ws==='object') ee.ws={ connected:!!e.ws.connected, status:errScrub(e.ws.status,8),
      reconnects:dInt(e.ws.reconnects), lastMsgAgoMs:(e.ws.lastMsgAgoMs==null)?null:dInt(e.ws.lastMsgAgoMs) };
    if(e.db && typeof e.db==='object') ee.db={ loaded:!!e.db.loaded, cards:dInt(e.db.cards),
      source:(e.db.source==null)?null:errScrub(e.db.source,20) };
    if(e.storage && typeof e.storage==='object') ee.storage={ usedKB:(e.storage.usedKB==null)?null:dInt(e.storage.usedKB),
      quotaKB:(e.storage.quotaKB==null)?null:dInt(e.storage.quotaKB) };
    if(Array.isArray(e.decks)) ee.decks=e.decks.slice(0,30).map(x=>{ if(!x||typeof x!=='object') return {};
      const r={ name:errScrub(x.name,40), crypt:dInt(x.crypt), library:dInt(x.library), warn:x.warn?1:0 };
      if(x.era!==undefined) r.era=errScrub(x.era,8); return r; });
    if(e.settings && typeof e.settings==='object'){ const sg={}; let sn=0;   // srv-2.6.30 (Johan): the whole settings surface. A SHAPE whitelist rather than a name list -- the field set is client-defined and grows, so types and sizes are the contract: keys /^[A-Za-z0-9_]{1,24}$/, values boolean | finite number (3-dp rounded) | scrubbed string <=32, at most 48 entries; objects, arrays, functions and everything else dropped. Nothing unvetted reaches disk, same guarantee as the name lists above.
      for(const k in e.settings){ if(sn>=48) break; if(!/^[A-Za-z0-9_]{1,24}$/.test(k)) continue;
        const v=e.settings[k];
        if(typeof v==='boolean'){ sg[k]=v; sn++; }
        else if(typeof v==='number'&&Number.isFinite(v)){ sg[k]=Math.round(v*1000)/1000; sn++; }
        else if(typeof v==='string'){ sg[k]=errScrub(v,32); sn++; } }
      if(sn) ee.settings=sg; }
    out.env=ee;
  }
  return (out.game || out.env) ? out : null;
}
function errAppend(rec){                       // shared JSONL sink: rotation + total-guard + best-effort append. Used by the client intake AND the server's own process hooks (v2.6.25 amend)
  try{
    const f=path.resolve(DATA_DIR, 'elysium-errors.log'), f1=f+'.1';
    let sz=0, sz1=0; try{ sz=fs.statSync(f).size; }catch(e){} try{ sz1=fs.statSync(f1).size; }catch(e){}
    if(sz+sz1 > ERR_TOTAL_MAX) return;         // disk guard of last resort
    if(sz > ERR_LOG_MAX){ try{ fs.renameSync(f, f1); }catch(e){} }
    fs.appendFile(f, JSON.stringify(rec)+'\n', ()=>{});
  }catch(e){}
}
function serverErrRecord(src, err){            // the server reporting on itself: same log, same reading tools, kind:'server'
  return { ts:new Date().toISOString(), kind:'server', ver:VERSION, src:errScrub(src,20),
           msg:errScrub(err && err.message || err, 300), stack:errScrub(err && err.stack, 2000) };
}
function handleLogFetch(req, res){             // v2.6.25 amend (Johan): the pull half of the loop -- fetch the error log from ANY browser, admin-gated. 404-uniform on every failure (no key configured, wrong key, locked out): the route does not admit it exists
  const nope=()=>{ try{ res.writeHead(404, { 'Cache-Control':'no-store' }); res.end('Not found\n'); }catch(e){} };
  if(!CFG.adminPassHash) return nope();        // no --admin-pass => the route is OFF
  let ip='?'; try{ ip=deriveIp(req.socket.remoteAddress, req.headers); }catch(e){}
  if(strikes(ipLogAuth, ip, ERR_WIN) >= ERR_MAX) return nope();   // brute lockout: even a correct key waits out the window
  let key=''; try{ key=new URL(req.url, 'http://x').searchParams.get('key') || ''; }catch(e){}
  if(!passMatch(CFG.adminPassHash, key)){ strike(ipLogAuth, ip); return nope(); }
  let body='';                                  // rotated first keeps chronology; total is capped at 4 MB by construction
  try{ body += fs.readFileSync(path.resolve(DATA_DIR, 'elysium-errors.log.1'), 'utf8'); }catch(e){}
  try{ body += fs.readFileSync(path.resolve(DATA_DIR, 'elysium-errors.log'), 'utf8'); }catch(e){}
  const h={ 'Content-Type':'text/plain; charset=utf-8', 'Cache-Control':'no-store' };
  if(/[?&]dl=1\b/.test(req.url)) h['Content-Disposition']='attachment; filename="elysium-errors.txt"';
  try{ res.writeHead(200, h); res.end(body || '(log empty)\n'); }catch(e){}
}
function handleMaintenance(req, res){          // v2.6.26: GET /admin/maintenance?key=..[&on=1&hours=H&msg=..|&off=1] -- no action = status JSON. Same auth family as the log pull.
  const nope=()=>{ try{ res.writeHead(404, { 'Cache-Control':'no-store' }); res.end('Not found\n'); }catch(e){} };
  if(!CFG.adminPassHash) return nope();
  let ip='?'; try{ ip=deriveIp(req.socket.remoteAddress, req.headers); }catch(e){}
  if(strikes(ipLogAuth, ip, ERR_WIN) >= ERR_MAX) return nope();
  let q=null; try{ q=new URL(req.url, 'http://x').searchParams; }catch(e){ return nope(); }
  if(!passMatch(CFG.adminPassHash, q.get('key') || '')){ strike(ipLogAuth, ip); return nope(); }
  if(q.get('on') === '1'){                     // strict: ?on=0 must not arm (truthy-string trap)
    const hours = Math.max(0.1, Math.min(72, parseFloat(q.get('hours')) || 4));
    maint = { until: Date.now() + hours * 3600000, msg: errScrub(q.get('msg') || '', 200), setAt: new Date().toISOString() };
    saveMaint();
    const note = maintNotice();
    rooms.forEach(room => sys(room, note));    // re-arming re-broadcasts: that IS the manual reminder button
  } else if(q.get('off') === '1'){
    maint = null; try{ fs.unlinkSync(maintFile()); }catch(e){}
  }
  const body = JSON.stringify({ active: maintActive(), until: maint ? new Date(maint.until).toISOString() : null, msg: maint ? maint.msg : null }) + '\n';
  try{ res.writeHead(200, { 'Content-Type':'application/json; charset=utf-8', 'Cache-Control':'no-store' }); res.end(body); }catch(e){}
}
function handleClientError(req, res){
  const done=()=>{ try{ res.writeHead(204, { 'Cache-Control':'no-store' }); res.end(); }catch(e){} };
  let ip='?'; try{ ip=deriveIp(req.socket.remoteAddress, req.headers); }catch(e){}
  let n=0, dead=false; const chunks=[];
  req.on('data', d=>{ n+=d.length; if(n>ERR_BODY_MAX){ dead=true; try{ req.destroy(); }catch(e){} return; } chunks.push(d); });
  req.on('end', ()=>{
    if(dead) return;
    if(strikes(ipErrs, ip, ERR_WIN) >= ERR_MAX) return done();     // silently dropped -- 204 always, no probing feedback
    strike(ipErrs, ip);
    let m=null; try{ m=JSON.parse(Buffer.concat(chunks).toString('utf8')); }catch(e){ return done(); }
    const rec=errRecord(m);
    if(!rec) return done();
    errAppend(rec);                             // best-effort fire-and-forget, same contract as persistence
    done();
  });
  req.on('error', ()=>{ try{ res.end(); }catch(e){} });
}
const srv = http.createServer((req, res) => {
  if(req.method === 'POST' && req.url === '/client-error'){ handleClientError(req, res); return; }   // B-54: the ONLY body-reading route
  if(req.method === 'GET' && req.url.startsWith('/client-errors.txt')){ handleLogFetch(req, res); return; }   // v2.6.25 amend: admin-gated log pull (?key=<admin-pass>, ?dl=1 for attachment)
  if(req.method === 'GET' && req.url.startsWith('/admin/maintenance')){ handleMaintenance(req, res); return; }   // v2.6.26: graceful-drain control, same auth family
  if(req.method === 'GET' && req.url.startsWith('/images/')){
    let file; try{ file = decodeURIComponent(req.url.slice('/images/'.length)); }catch(e){ file = ''; }
    // v2.6.10 (Johan: the card DATABASE should be local-folder-able too, not just the images):
    // vtes.json is allowed as an EXPLICIT exact-name whitelist branch -- the regex below is NOT
    // widened to .json generally (exact-match is a tighter guarantee than another extension class).
    if(file === 'vtes.json'){
      fs.readFile(path.join(IMAGES_DIR, 'vtes.json'), (err, data) => {
        if(err){ res.writeHead(404, { 'Content-Type': 'text/plain' }); res.end('Not found'); return; }
        res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'public, max-age=86400', 'X-Content-Type-Options': 'nosniff' });
        res.end(data);
      });
      return;
    }
    ...
    fs.readFile(full, (err, data) => {
      ...
      res.writeHead(200, { 'Content-Type': ct, 'Cache-Control': 'public, max-age=86400', 'X-Content-Type-Options': 'nosniff' });
      res.end(data);
    });
    return;
  }
  if(req.method === 'GET' && (req.url === '/elysium-bot.js' || req.url === '/elysium-cardfx.json')){
    /* srv-2.6.28 (HB2c packaging): the hotseat-bot loader (client v2.6.60, hotLoadBotRes) fetches
       BOTH files from the serving host at start. EXACT req.url match -- no decoding, no user-shaped
       path ever touches the filesystem, so traversal is structurally impossible (the same
       tighter-than-regex guarantee the vtes.json whitelist chose). Served from __dirname: deploy
       artifacts versioned with the code (read-only is fine under the hardened unit -- the
       srv-2.6.20 EACCES class was WRITES). Cache 300 s, not vtes's 86400: the bot iterates fast,
       and the deploy ritual purges these two URLs at the edge (runbook).                        */
    const nm = req.url.slice(1);
    fs.readFile(path.join(__dirname, nm), (err, data) => {
      if(err){ res.writeHead(404, { 'Content-Type': 'text/plain' }); res.end('Not found'); return; }
      res.writeHead(200, { 'Content-Type': nm.endsWith('.js') ? 'application/javascript; charset=utf-8' : 'application/json; charset=utf-8',
  ...
  if(req.method === 'GET' && (req.url === '/' || req.url === '/index.html')){
    fs.readFile(CLIENT_FILE, (err, data) => {
      ...
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store',   // v2.6.21: standard hardening headers -- DENY framing (no legit embed case),
        'X-Content-Type-Options': 'nosniff', 'X-Frame-Options': 'DENY', 'Referrer-Policy': 'no-referrer' });   // nosniff, and no referrer leakage from the app
      res.end(data);
    });
    return;
  }
  if(req.method === 'GET' && req.url === '/healthz'){     // H1-f: uptime probe -- counts only, never names; no-store so the edge can't cache it
    res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store', 'X-Content-Type-Options': 'nosniff' });
    res.end(JSON.stringify({ ok: true, rooms: rooms.size, conns: conns.size, v: VERSION }));
    return;
  }
...
srv.on('error', err => {                                  // v2.6.19: a listen failure (typo'd --bind, port already in use) previously fell into
  console.log('[fatal] server error: ' + (err && err.message || err));   // the uncaughtException net and left a ZOMBIE -- running, logging, serving
  process.exit(1);                                        // nothing. Exit loudly instead, so systemd Restart=on-failure (or the operator) reacts.
});
srv.keepAliveTimeout = 65000;                             // v2.6.19 (H1-g revisited): a reusing proxy (cloudflared keeps origin connections
srv.headersTimeout   = 66000;                             // warm) racing Node's 5s default keep-alive close is the classic sporadic-502 class;
srv.requestTimeout   = 120000;                            // keep-alive sits ABOVE typical reuse windows, and the documented ordering holds:
                                                          // keepAlive < headers < request. WS connections detach from all three at upgrade.

function originOk(hdr){                                   // v2.6.21: CSWSH belt-and-braces. No cookies/session auth exists here, so the
  if(!CFG.origin) return true;                            // classic hijack is already defanged (tokens live in localStorage, unreachable
  if(!hdr) return true;                                   // cross-origin) -- what an allowlist closes is ABUSE-BY-PROXY: a malicious page
  return CFG.origin.has(String(hdr));                     // spending its visitors' per-IP budgets against us. Absent header = non-browser
}                                                         // client (bots/tools) = allowed; browsers always send Origin on WS.
srv.on('upgrade', (req, sock) => {
  ...
  if(!originOk(req.headers.origin)){                      // v2.6.21: reject before any resource is spent
    console.log('[abuse] WS origin rejected (' + String(req.headers.origin) + ') from ' + ip);
    try{ sock.end('HTTP/1.1 403 Forbidden\r\n\r\n'); }catch(e){}
    return;
  }
  if(conns.size >= MAX_CONN_TOTAL){                       // H1-b: absolute ceiling, deliberately NOT loopback-exempt -- behind a tunnel
    console.log('[abuse] global connection ceiling (' + MAX_CONN_TOTAL + ') hit; refusing ' + ip);   // everyone is loopback, and this is the last-line memory guard
    sock.destroy(); return;
  }
  if(!loopback){
...
setInterval(() => {
  ...
  ipErrs.forEach((a, ip) => { if(!a.some(t => now - t < ERR_WIN)) ipErrs.delete(ip); });   // B-54
  ipLogAuth.forEach((a, ip) => { if(!a.some(t => now - t < ERR_WIN)) ipLogAuth.delete(ip); });
  rooms.forEach((room, name) => {
    if(!room.everPop && now - room.created > NEVER_POP_GRACE_MS){   // v2.6.22: never-seated shell -> short grace, then vac it (squatter sweep)
      rooms.delete(name); scheduleRoomList();
      console.log('[room] "' + name + '" reaped (never seated anyone in ' + Math.round(NEVER_POP_GRACE_MS/60000) + ' min)');
      return;
    }
    const allOff = room.players.every(p => !p.sock);
    const oldest = Math.max(0, ...room.players.map(p => p.off || 0));
    /* srv-2.6.36: the hour is for a real table mid-match, where somebody is coming back.
       A room holding bots has a shorter honest life: the bots live in the host's tab, so
       once no human is online they are gone too and nothing in there can play. This also
       bounds the one case v2.6.104 could not reach -- a host who closes the TAB sends no
       helper-policy restore, so the locked policy dies with the room instead of outliving
       the session by an hour. */
    const botGrace = room.players.some(p => p.bot) && humansLeft(room) > 0;
    const ttl = botGrace ? BOT_ROOM_GRACE_MS : 60 * 60000;
    if(allOff && room.players.length && now - oldest > ttl){
      rooms.delete(name); scheduleRoomList();
      console.log('[room] "' + name + '" expired (everyone offline > ' + Math.round(ttl / 60000) + ' min' + (botGrace ? ', bot table' : '') + ')');
    }
  });
  sweepSaves(now);                                        // H1-e (D4): the named-save TTL rides the same housekeeping tick
}, 10 * 60000);

/* ---------- Persistence: the table survives a server restart ---------- */
const DATA_DIR = CFG.dataDir ? path.resolve(CFG.dataDir) : __dirname;   // v2.6.20: __dirname-relative was silently unwritable under ProtectSystem=strict (root-owned code dir) -- best-effort catch ate the EACCES and persistence just... didn't. Default unchanged.
const STORE = path.resolve(DATA_DIR, 'elysium-rooms.json');
function makeAsyncFileWriter(finalPath){                  // H1-d: non-blocking persistence with in-flight coalescing -- at most one
  let busy = false, next = null;                          // write chain runs; the newest queued JSON wins. .tmp-then-rename keeps
  return function write(json){                            // the on-disk file atomic exactly like the old sync path did.
    if(busy){ next = json; return; }
    busy = true;
    fs.writeFile(finalPath + '.tmp', json, err => {
      const done = () => { busy = false; if(next !== null){ const j = next; next = null; write(j); } };
      if(err){ done(); return; }                          // best-effort, same contract as before
      fs.rename(finalPath + '.tmp', finalPath, () => done());
    });
  };
}
const writeStore = makeAsyncFileWriter(STORE);
function snapshot(sync){
  try{
    ...
    if(sync){ fs.writeFileSync(STORE + '.tmp', json); fs.renameSync(STORE + '.tmp', STORE); }   // exit path only: the process is leaving, sync is CORRECT here (async would race process.exit)
    else writeStore(json);                                // H1-d: the 60s tick no longer blocks the event loop
...
restore();
loadMaint();   // v2.6.26: a crash-restart during the window must not silently reopen creation
setInterval(snapshot, 60000);
const _sleep = () => { snapshot(true); persistSaves(true); console.log('\n  Rooms saved. The table sleeps.'); process.exit(0); };   // H1-d: the EXIT flush stays synchronous by design
...
const SAVES = path.resolve(DATA_DIR, 'elysium-saves.json');
const saves = new Map();
const writeSaves = makeAsyncFileWriter(SAVES);
function persistSaves(sync){
  try{
    const json = JSON.stringify([...saves.values()]);
    if(sync){ fs.writeFileSync(SAVES + '.tmp', json); fs.renameSync(SAVES + '.tmp', SAVES); }   // exit path only (see snapshot)
    else writeSaves(json);                                // H1-d
  }catch(err){ /* best-effort */ }
}
function sweepSaves(now){                                 // H1-e: extracted so the dispatch suite can drive the TTL directly
  let n = 0;
  saves.forEach((s, name) => { if(now - (s.touchedAt || s.savedAt || 0) > SAVE_TTL_MS){ saves.delete(name); n++; } });
  if(n){ persistSaves(); console.log('[save] expired ' + n + ' saved match(es) untouched > 90 days'); }
  return n;
}
function loadSaves(){
  try{
    ...
    if(Array.isArray(data)) for(const s of data){ if(s && s.name){ if(!s.touchedAt) s.touchedAt = s.savedAt || Date.now(); saves.set(s.name, s); } }   // H1-e: legacy saves (pre-2.6.18) get their TTL clock backfilled from savedAt
...
function buildSave(room, name){
  return {
    name, savedAt: Date.now(), touchedAt: Date.now(), roomName: room.name, passHash: room.passHash,   // H1-e: touchedAt drives the 90-day TTL; save AND load refresh it
...
/* ---------- Boot ---------- */
srv.listen(PORT, CFG.bind || undefined, () => {         // H1-a: --bind 127.0.0.1 behind a tunnel = the raw port is unreachable even on-LAN
  console.log('');
  ...
  if(CFG.bind) console.log('  Bind:     --bind ' + CFG.bind + (CFG.bind === '127.0.0.1' || CFG.bind === '::1' ? '  (loopback only -- LAN addresses below are NOT served)' : ''));
  const ifs = os.networkInterfaces();
  if(!CFG.bind || CFG.bind === '0.0.0.0' || CFG.bind === '::') Object.keys(ifs).forEach(k => (ifs[k] || []).forEach(a => {
  ...
  console.log('  Data:     ' + DATA_DIR);   // v2.6.20: make the persistence location visible at boot -- the silent-EACCES class dies in daylight

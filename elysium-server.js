...
const VERSION = '2.6.37';   // srv-2.6.37 (14 Sep 2026, ONE MORE HELPER KEY -- restart): HELPER_KEYS gains `chatFx`, the client's new chat-animation tier (v2.6.107). No logic change -- but the key MUST live here or cleanHelperPolicy drops it from every host-locked policy, since it builds `settings` from this list alone. Two hand-written mirrored lists, exactly the sibling-drift shape the learnings name; a client test now diffs them. Previous note: srv-2.6.36 (14 Sep 2026, CLOSE TABLE -- restart): a room can now be ENDED as an intent instead of emptied by side effects. New REF_VERB `closeRoom` -> closeRoomNow(): broadcasts `closed{msg}` (so every client tears down knowing why, and does not try to rejoin a room that is gone), deletes the room, then closes every socket; idempotent against a race. Two bot-aware rules beside it, both resting on the new humansLeft(room) census -- p.bot has existed since 13 July but only ever fed the roster's control-my-elements exception, never a count: (1) a real leave that takes the last non-bot player closes the room immediately, since online bots live in the host's tab and a table of bots alone has nobody to play for; (2) the reaper gives a room that holds bots with no human ONLINE a 10-minute grace (BOT_ROOM_GRACE_MS) instead of the 60-minute all-offline rule -- the hour is for a real table mid-match. A disconnect still keeps its full reconnect window: the census counts roster seats, never sockets. Previous note: srv-2.6.35 (11 Sep 2026, PER-ROOM REFEREE -- restart): the host sets a referee password at creation (create{refPass} -> room.refHash); a spectator who joins with it (join{refPass}) is that room's REFEREE: watches, never plays, and reaches every host-gated verb (hostOrRef; REF_VERBS routes referee watchers to GAME_HANDLERS with a pseudo-p). joined{ref:true}, watcherList[].ref, roomList rooms[].ref (the room has a referee password; the srv-2.6.34 server-level flag is gone). Wrong or absent key: err + the room-password strike. The server-level key (--referee-pass, hello{admin}) stays an operator tool for --create-policy admin and still passes every gate.

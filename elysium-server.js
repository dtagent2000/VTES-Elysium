...
const VERSION     = '2.5.37';  // cosmetic-only bump to match the client's patch number (v2.5.37) -- NO server
                               // logic changed since 2.5.30 (see elysium-session-journal.md). Kept in lockstep
                               // with the client's VERSION: the client warns on any mismatch when joining.
...
const GAME_HANDLERS = {
  ...
  recall(conn, room, p, m){                                        // owner -> holder: ask the holder (seat m.seat) to hand a card back
    ...
    if(!q) return wsSend(conn.sock, { t:'err', msg:'No such seat.' });
    if(q === p) return wsSend(conn.sock, { t:'err', msg:'That is your own board.' });
    if(!q.sock || q.off) return wsSend(conn.sock, { t:'err', msg: q.name + ' is offline — the card stays there.' });

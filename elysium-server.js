...
const VERSION     = '2.5.69';  // kept in lockstep with the client's VERSION (major.minor policy; the client
...
const IMG_FILE_RE = /^[a-z0-9]+\.(webp|jpe?g)$/i;
const srv = http.createServer((req, res) => {
  if(req.method === 'GET' && req.url.startsWith('/images/')){
    ...
    fs.readFile(full, (err, data) => {
      ...
      res.writeHead(200, { 'Content-Type': /\.webp$/i.test(file) ? 'image/webp' : 'image/jpeg', 'Cache-Control': 'public, max-age=86400' });
      res.end(data);
    });
    return;
  }

...
Turn a teardown-and-rebuild renderer (`host.innerHTML=''` + rebuild) into a keyed reconcile so persisted elements ride their CSS transitions (position/rotation glide) and can play keyframe animations on state changes. Applied to `renderL4OppCards()` + `addRCard()`; liftable to `buildMat`/`renderFull` later (they still wipe -> unchanged today).

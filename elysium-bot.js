...
const BOT_VERSION = '0.6.157'; // v0.6.157 (22 Sep 2026, THE RUSH KIT + THE SILENT LONG -- Johan, before the deploy): (1) a SILENT opponent (a human table) at LONG range deals 0 -- their rulebook default is a hand strike; the old assume-1 stands only when their board shows a ranged option (`_oppHasRanged`: a strike-granting equipment, or a steal seen). (2) The rush reads its KIT (`_rushExpectation` kit / grapple / strikeCard / crows / reach): Johan -- rush only with real damage in hand (Crows + Grapple / Roundhouse), with the combat cards spent bleed with Deep Song. decide('action') multiplies the rush by 1.5 (Grapple behind a damage source; the reach doubles -- the bundled press buys round 2), 1.15 (one damage card) or 0.75 (no combat card); the target chooser's torpor reach reads the same kit. Both behind combatMind. Previous: v0.6.156 (22 Sep 2026, THE COMBAT MIND -- Johan's combat rulings after serie-cmb1, REPORT-serie-cmb1.md; knob combatMind, 0 = v0.6.155): (1) Immortal Grapple only with Roundhouse-class hand damage, active Carrion Crows or a strength bonus -- never with bare fists (cmb1: 8 of 17 Grapples wasted). (2) BUNDLED presses consumed: superior Grapple's and Hidden Strength [FOR]'s optional press live on `_bundledPress` for the round they were played in (Nosferatu's only press engine; Ventrue's press-to-END). (3) `_wantPress` replaces `blood > 1`: press when next round's damage torpors the opponent without torporing me, or when I hold a card round (Roundhouse / Theft / active Crows) and survive their shown return; wave-posture decks press only for a torpor shot against a non-combat seat. (4) Press-to-END: an incoming press is cancelled with a press of my own when I do not want the round (Ventrue's rule); a bot's 'presses to end' cancels my own continue. (5) `_bestPressFor` excludes prevent-bundled modes (Hidden Strength was played as a standalone press). (6) Taste of Vitae only after a round where the opponent lost 2+. (7) Range posture per deck (DECK_STRATEGY.combat: chase / snipe / wave) drives `_bestManeuverFor`: snipe goes long with a ranged strike/steal in hand, wave goes long with no strike card against a dangerous seat, chase never leaves close; at LONG range hand strikes deal 0 (rulebook) -- both bare fists and handStrike-tagged cards. (8) Stealth spend prefers cards WITHOUT a combat mode at equal stealth (Swallowed by the Night stays for the defensive maneuver). (9) One `combat-round` trace row per round (range, opponent, strike, dealt, taken, grapple, press) -- serie-cmb1 had to infer all of it. Previous: v0.6.155 (22 Sep 2026, THE RUSH BY CHAT -- Johan's interim rule until the cardfx rush sweep): a HUMAN rushes one of my vampires by playing the card with my vampire as Target (the client's play line carries it) and typing `rush` (or `rush <vampire>`); I open the block window / lock and fight exactly as on the frozen Deep Song line. `_lastAim` remembers the last opposing play line that named one of my ready vampires. Previous: v0.6.154 (22 Sep 2026, micro): the stale `rush` chat reply removed from _onChat (said 'no combat module yet (M2)' -- untrue since v0.4.0). Text only, no decision logic touched; previous: v0.6.153 (22 Sep 2026, THE BREATH -- self-review round): (1) the verdict clock (_askTimeout) waits in 400 ms steps while a block-duel card of mine is still waiting out the react beat (av._lookQ / av._spendQ, _deferredPending) -- with `--ask-secs 3` it could otherwise declare the block before the deferred stealth / cancel card landed; a FOREIGN counter move during my contest restarts the clock (table activity, v0.6.115's principle), so a bot blocker's deferred intercept card is never overtaken either. (2) One bubble: when the shout came without a minion line, the 'currently has N stealth' line carries the who-blocks hint itself and stands the 2.5 s 'Who blocks?' grace down. No decision logic changed since v0.6.152.

...
/* ============================================================================
   ...
   `_announcePhase` when the table's resolvers are live — the same
...
const BOT_VERSION = '0.6.107'; // v0.6.107 (11 Sep 2026, B1 -- the undirected block window, elysium-bot-block-doctrine.md section 3/5): bots now get a REACTION WINDOW for the other bots' undirected actions (hunt, equip/employ, recruit, blood-stock, tabling, fetch, strength) and for Deep Song rushes against their vampires -- the L12.undirected grammar family parses the bot's own frozen announce lines, the actor's prey answers first and its predator second (the predator bot opens on the prey's Pass; a human prey's silence leaves the actor's askSecs as the ceiling), and the actor's Block? resolves when every eligible blocker has passed instead of at the timeout; decide('block-as-target') gains the doctrine's threat classes T2 intercept / T3 combat / T4 horde / T4b minions / T5 starve-rush / T6 tempo with the deck-axis multiplier, the posture-based combatFit and the lockCost (the A&B price of the block) -- the directed T1 bleed path is byte-identical, its axes come with B2; every window emits block-eval {T, role, score, parts} for the dossier and the arena counts blocks per class. Previous notes: elysium-bot-CHANGELOG.md.
...
const ARCHETYPES = { hacking: DEFAULT_DECK, wall: WALL_DECK, sewer: SEWER_DECK, malkavian: MALKAVIAN_DECK, tremere: TREMERE_DECK, nosferatu: NOSFERATU_DECK };
const DECK_STRATEGY = {   // A2 (v0.6.73): the reviewer's yardstick -- curated intent per archetype, emitted in the manifest so play is judged AGAINST strategy, not in a vacuum
  ...
  nosferatu: { strategy: 'Menace attrition: Fame/Creeping pool pressure + [ANI] rush control keeps threats in torpor; win by bleed+drain, not combat volume.', axes: ['menace', 'rush-control', 'stealth-bleed'] }
...
const UND_KINDS = new Set(['stock', 'recruit', 'strength', 'creep', 'equip', 'fetch']);   // v0.6.107 (B1): the acting-side kinds whose Block? is answered by prey AND predator (hunt has no kind and no target)
...
const PERSONAS = {
  novice:  { aggression: 0.8, blockShy: 1.3, insight: 0.1 },
  grinder: { aggression: 1.0, blockShy: 1.0, insight: 0.5 },
  shark:   { aggression: 1.2, blockShy: 0.8, insight: 0.9 }
};
...
const L12 = {
  ...
  lockM: /^(.+?) plays Misdirection \u2014 locks (.+?)'s (.+?)\.$/,   // M1: frozen -- m[2]=owner seat name, m[3]=vampire; the named owner complies by locking
...
class Bot {
  constructor(opts){
    ...
    this.reads = {};                 // seat -> { seen: {bounce,stealth,intercept,votes} } — the honest hand-READ memory
    this.tally = null;               // last tally broadcast mirror (READ-ONLY, never written)
    this.caps = { tally: false, classic: false };   // capability detection by OBSERVATION
  ...
  _drain(){
    ...
    try{
      obj = this._sq.shift();
      if(obj && obj.__board){ this._pushQ = false; obj = { t: 'board', pub: this.buildPub() }; }
      this.ws.send(obj);
  ...
  chat(msg){ this.send({ t: 'chat', msg: String(msg).slice(0, 300) }); }
  ...
  async _pace(kind){                                   // v0.6.99 (THE BEAT MODEL, Johan's design locked 8 Aug): the flat gate becomes multidimensional -- no arg = 'step' (paceMs, every legacy site unchanged), 'chat' = chatPaceMs or paceMs*1.6 (a declaration bubble plays out before the next act), 'fx' = fxBeatMs or paceMs*0.4 (the event animation's moment). paceMs 0 zeroes EVERY kind, so the suite stays fast by construction.
    while(this._holdBy && this._holdBy.size) await sleep(150);   // v0.6.95 (HOLD): a human said Hold on... -- every visible act waits here
    ...
    while(this._holdBy && this._holdBy.size) await sleep(150);   // v0.6.101 amend (the sweep): the sleeps are ~5 s now -- a Hold on... arriving MID-sleep must still bite BEFORE the act fires, so the gate re-checks at the tail too (top + tail = airtight)
  ...
  /* v0.5 (Johan): behave like a human using the Structured phase-bar helpers
     ...
     other visible command. Gated on caps.tally (the same "resolvers/tally
     traffic observed" signal that already flips the bot into informed mode)
     since that's the same underlying condition (tournament off) that makes
     the phase bar exist for a human at all -- announcing it on a Classic or
     tournament table would just be narration noise nobody's helper reads. */
  async _announcePhase(name){
    if(!this.caps.tally) return;
    await this._pace();
    ...
    return {
      ...
      logTo: m => {                                    // R3: private line -- Revelations' discard command is the only consumed shape today
        ...
        if(ix >= 0){
          ...
          this.push();
          this.chat('Trainbot discards ' + nm + ' (Revelations) and draws.');
          this._emit('forced-discard', { name: nm });
        } else this.chat('Trainbot: I do not hold ' + nm + ' \u2014 table\u2019s call.');
      },
      joined: m => {
      ...
      dealt: m => {
        ...
        this.phaseBonus = { master: 0, influence: 0, discard: 0 };
        this.phaseUsed = { master: 0, influence: 0, discard: 0 };
        this.table = {}; this.reads = {}; this._clearPending('fresh deal'); this._clearCombatStashes();
        ...
        this.push();
        this.chat('Trainbot v' + BOT_VERSION + ' is seated. I answer to: "bleed N", "vote N", and "block/no" when I ask. Be gentle. 🦇');
        if(this.caps.classic)
          this.chat('Tip: I play best on a Structured table with the resolvers on — but I will follow along here.');
      ...
      drew: m => {
        ...
        if(m.zone === 'hand'){
          if(this._fetchDebt && (this._fetchDebt[m.name] | 0) > 0){   // E1: the phantom copy of an already-fetched card -- consume the debt, redraw, stay honest
            ...
            this.send({ t: 'draw' });
            if(!this._fetchNoteDone){ this._fetchNoteDone = true; this.chat('Trainbot: drew the previously fetched copy \u2014 cycling it (Magic of the Smith bookkeeping).'); }
      ...
      turn: m => {
        ...
        if(this.started && !this.out && m.seat === this.seat && !this._acting)
          this._playTurn().catch(e => this.o.log('turn error:', e.message));
      },
      ...
      forceSetPool: m => {                             // the host referee kit (hostSetPool → this) — v0.3.7: clamp + oust-on-zero (a host draining a bot to 0 means exactly that)
        ...
        this.chat('Trainbot: pool set to ' + this.pool + ' by the host.');
      ...
      bounty: m => {
        this.pool += 6;
        this.push();
        this.chat('Trainbot collects the bounty from ' + m.from + ': +6 pool (' + this.pool + ') and 1 VP. The Jyhad provides.');
      }
    };
  }
  ...
  _maybeEnterPlayDraw(card){         // VC: New Blood -- 'As New Blood enters play during your influence phase, choose any circle and draw 1 card from your crypt.' The crypt draw is the mechanic; the circle choice is table-noted (Blood Brother circles are unmodeled).
    ...
    this.chat(card.name + ' enters play \u2014 draws ' + dn + ' from the crypt (printed enter-play draw; circle choice unmodeled, note it at the table).');
    return dn; }
  _maybeLubomiraMark(v){             // VC: Lubomira Hradok -- 'If Lubomira successfully bleeds a Methuselah who controls a ready titled vampire, she does not unlock as normal during her next unlock phase.' Ruling [PIB 20110915]: SHE is the one who doesn't unlock. Called at the bleed-success commit; the target seat is the ACTUAL bleed target (_actingVamp.target follows bounces). Titles are PRINTED (fxLookup .title -- the rebelBlood read path); acquired titles stay politics-scope.
    ...
    this.chat(v.name + ' bled a Methuselah with a ready titled vampire \u2014 she will not unlock as normal next unlock phase (her printed text).');
    return true; }
  ...
  _mayPlay(canonical, entry, played){   // rules gate for playing a library card
  ...
  _modeUsableBy(mo, vamp){          // can THIS vampire meet the mode's discipline gate?
  ...
  _modeUsable(mo, vamps){ return vamps.some(v => this._modeUsableBy(mo, v)); }
  ...
  _deadHandIndex(vamps){             // v0.5 (§7.6 D, extracted for direct testability like _unlockPhase): the FIRST hand card no ready vampire can do anything with -- masters included now that module A can play the known income assets (no more unconditional master death); a bounce-only mode's 2-live-player deadness comes for free via _modeUsable → _modeUsableBy
    return this.hand.findIndex(n => {
      ...
      return (h.e.modes || []).length > 0 &&
             !(h.e.modes || []).some(mo => this._modeUsable(mo, vamps));
    });
  }
  _bestPeekFor(vamp){               // R3: Revelations (peekPreyDiscard/openHandPrey). Superior-if-able v1 -- persona knob later; prey-locked by card text (dir:'prey').
    ...
    for(let idx=0; idx<this.hand.length; idx++){
      ...
      if(!sup && !inf) continue;
      return { idx, name:h.name, kind:'peek', supMode:sup, st, n:1 };
    }
    return null;
  }
  ...
  _bestBleedCardFor(vamp){          // highest usable bleed-ACTION card in hand for this vampire
    ...
    this.hand.forEach((n, idx) => {
      ...
      (h.e.modes || []).forEach(mo => {
        ...
        if(!best || amt > best.n || (amt === best.n && st > (best.st | 0))) best = { idx, name: h.name, n: amt, st };
      });
    });
    ...
    while(!this.out){
      ...
      if(swapCard){
        ...
        if(dup && this.cryptN > 0 && this.inf >= swapCost){
          ...
          this.push();
          this.chat(swapCard.name + ': Trainbot uses ' + swapCost + ' transfer' + (swapCost === 1 ? '' : 's') + ' to draw a new vampire, removing a duplicate ' + dup.name + ' from the game.');
      ...
      const cheapest = this._pickRaise(raiseables);   // v0.6.75: scored (opening book), no longer literally cheapest -- the variable name stays for the guards below
      ...
      const floorEff = zeroReady ? 1 : this.o.poolFloor;
      if(!cheapest && this.cryptN > 0 && this.inf >= 4 && this.pool - 1 >= floorEff){
        ...
        this.push();
        this.chat('Crypt: Trainbot fetches a new vampire (4 transfers, 1 pool — pool ' + this.pool + ').');
      ...
      if(!cheapest && burnCard && this.inf >= burnCost){   // only when NOTHING better remains for these transfers (no duplicate to swap, no vampire to raise, no crypt card left to draw), matching the "don't waste a resource, but don't sacrifice a better option" spirit already established for Life in the City's own capacity check
        ...
        this.push();
        this.chat(burnCard.name + ': Trainbot uses ' + burnCost + ' transfers to burn it for +' + burnGain + ' pool (pool ' + this.pool + ') -- nothing better to spend the transfers on this turn.');
      ...
      if(this.inf < need || this.pool - need < floorEff) break;   // v0.6.69 (#1): floorEff -- the survival exception applies to the raise too
      ...
      this.inf -= need; this.pool -= need;
      cheapest.zone = 'ready'; cheapest.faceDown = false; cheapest.locked = false;
      this._maybeEnterPlayDraw(cheapest);   // VC: New Blood's printed enter-play crypt draw
      cheapest.blood = cheapest.cap;
      ...
      this._emit('raise', { name: cheapest.name, need, turn: this.ownTurns | 0, poolAfter: this.pool, infAfter: this.inf });   // v0.6.77 (A5): the turn field makes influence tempo EXACT   // v0.6.76: REALITY beside the reasoning -- phantoms are now exposable
      brought++;
      this.push();
      ...
      this.chat('Influence: ' + cheapest.name + ' rises (cap ' + cheapest.cap + (need < cheapest.cap ? ', ' + (cheapest.cap - need) + ' pre-stocked' : '') + ', pool ' + this.pool + ').');
    }
  }
  ...
  _syncOpenHand(){                   // R3: the granter owns the open-hand state (server design) -- re-send the live list on every change while active
    if(!this._openHandActive) return;
  ...
  _bestFetchFor(vamp){               // E1: Magic of the Smith -- fx.fetchEquip; target from the remaining library via the ladder
    ...
    for(let idx = 0; idx < this.hand.length; idx++){
      ...
      return { idx, name: h.name, kind: 'fetch', st, target, cost: target.cost, tier: target.tier, n: 1 };
    }
    return null;
  }
  ...
  _strayPressBurn(){                 // N4d consumer A: burn 1 life for a press to my combatant; the ally dies to 'burned' at 0
    ...
    st.life = (st.life | 0) - 1;
    if(st.life <= 0){ st.zone = 'burned'; this.chat('Trainbot: the Stray gives its last \u2014 press granted, the animal is spent.'); }
    else this.chat('Trainbot: the Stray burns 1 life \u2014 press granted (' + st.life + ' left).');
    this.push();
    return true;
  }
  _strayUnlockBurn(vamp){            // N4d consumer B: burn the WHOLE ally to TRULY unlock a ready minion (the locked-blocker chain's third rung); requires fx.allyBurnUnlock
    ...
    vamp.locked = false;
    this.push();
    this.chat('Trainbot: the Stray is burned \u2014 ' + vamp.name + ' unlocks to meet the threat.');
  ...
  _bestRecruitFor(vamp){             // N4d: an ally-type fx.recruitAlly card -- richest usable mode (ANI life 2 over ani life 1), pool guard
    ...
    for(let idx = 0; idx < this.hand.length; idx++){
      ...
      if(!best) continue;
      const costP = ((h.e.cost && h.e.cost.pool) | 0) || 1;
      if(this.pool < costP + 1) continue;   // leave >= 1 pool
      return { idx, name: h.name, kind: 'recruit', life: best.fx.allyLife | 0, at: best.at, costPool: costP, st: 0, n: 1 };
    }
    return null;
  }
  _emitManifest(){                   // A2 (v0.6.73): deck intent + effective knobs
    ...
    const kn = {}; for(const k of ['rushBackWeight','rushFwdWeight','crossRushWeight','poolFloor','paceMs','askSecs']) if(this.o[k] !== undefined) kn[k] = this.o[k];
  ...
  _complyRush(p){                     // v0.6.107 (B1): the rushed vampire locks and fights the rusher (the pre-B1 comply, now on MY Pass)
    ...
    this.chat('Trainbot: ' + vc.name + ' locks and defends (Deep Song rush).');
  ...
  async _onVoteOpen(m){              // P0 (the politics engine's FIRST brick): the polling window -- District's defensive votes land in the SHARED against-counter. v0.6.67: async for the wake pacing beat
    ...
    if(!vr){ this.chat('Trainbot abstains.'); return; }
    ...
    if(needsWakeV && !wkV){ this.chat('Trainbot abstains.'); return; }
    if(wkV){
      this.hand.splice(wkV.idx, 1); this._drawOwed = (this._drawOwed || 0) + 1;
      this._toAsh(wkV.name);
      this.fxClone({ name: wkV.name, kind: 'lib' }, 'may react as though unlocked');
    ...
    if(j < 0){ this.chat('Trainbot abstains.'); return; }
    this.hand.splice(j, 1); this._drawOwed = (this._drawOwed || 0) + 1;
    this._toAsh(vr.name);
    this._emitPlay(vr.name, 'react-votes', vr.vamp.name);
    this.fxClone({ name: vr.name, kind: 'lib' }, '+' + vr.n + ' votes against');
    this.send({ t: 'log', html: '<b>' + esc(vr.vamp.name) + '</b> plays <b>' + esc(vr.name) + '</b> \u2014 +' + vr.n + ' votes against the referendum.' });
    this.send({ t: 'tally', mode: 'vote', b: ((m && m.b) | 0) + vr.n });   // absolute against-total; `a` rides srv-2.6.17's absent-field preserve untouched
    this.chat('Trainbot: ' + vr.vamp.name + ' votes ' + vr.n + ' against (Protected District).');
    this._votedThisRef = true;
    this.push();
  }
  _onVoteClose(){ this._votedThisRef = false; }   // P0: the once-per-referendum guard re-arms on the mode-exit edge
  ...
  async _commitReduce(p, rd){        // N4c: the reducer IS the payer -- p.amount drops bot-locally, the log line is table-truth; then Pass closes my window. v0.6.67: async for the wake pacing beat
    ...
    if(wk){                                              // v0.6.67 (the combo): the wake play -- _commitBounce's sequence VERBATIM; the reactor STAYS locked ('as though unlocked' is the rule)
      ...
      this.fxClone({ name: wk.name, kind: 'lib' }, 'may react as though unlocked');
    ...
    p.reducedOnce = true;
    this.fxClone({ name: rd.name, kind: 'lib' }, 'reduces the bleed by ' + rd.n);
    this.send({ t: 'log', html: '<b>' + esc(rd.vamp.name) + '</b> plays <b>' + esc(rd.name) + '</b> \u2014 reduces the bleed by ' + rd.n + ' (now ' + p.amount + ').' });
    this.chat('Trainbot: ' + rd.vamp.name + ' reduces the bleed by ' + rd.n + '.');
    this.push();
    this.say('Pass');
  }
  ...
  _maybeRebelBlood(blocker, actingName){   // N4a: Rebel -- 'Once each turn, if they block a titled vampire ... gains 1 blood before block resolution.' Printed titles are fxLookup-readable; acquired titles stay politics-scope. Extracted from _commitBlock's success commit for direct testability.
    ...
    this.chat('Trainbot: Rebel \u2014 ' + blocker.name + ' gains 1 blood (blocking the titled ' + actingName + ').');
  }
  _maybeWarsawRefresh(v){            // N4a: after a SUCCESSFUL UNDIRECTED action by a clan-Nosferatu vampire -- lock the ready Station, unlock the actor (pure tempo)
    ...
    this.chat('Trainbot: Warsaw Station locks \u2014 ' + v.name + ' unlocks (refresh \u2014 acts again).');
  ...
  _maybeWarsawRescue(){              // N4a: burn the Station (locked or not) to move a torpored Nosferatu of mine to ready -- cost-free per the text; checked at my unlock
    ...
    this.push();
    this.chat('Trainbot: Warsaw Station burns \u2014 ' + vt.name + ' rises from torpor.');
  ...
  _fameTorporHook(vamp){             // N3: Fame -- 3 pool on EACH torpor entry of a famed vampire I control (the yo-yo's engine half). v0.6.54 (A1): ALWAYS emits 'torpor' first -- the arena's doctrine metric rides the same three flip sites
    ...
    if(famed){
      ...
      this.chat('Trainbot: Fame \u2014 ' + vamp.name + ' hits torpor, 3 pool burned.');
    }
    ...
    if(fx && fx.burnOnTorpor){
      vamp.zone = 'burned';
      this.chat('Trainbot: ' + vamp.name + ' goes into torpor and is BURNED (his own curse).');
    }
  ...
  _menaceUnlockEngine(){             // N3: the unlock-phase menace settle -- Creeping line, Jack tick, Jack pay, Fame drain
    ...
    for(const nm of Object.keys(burnGroups)){
      ...
      this.chat('Trainbot: ' + nm + ' ticks \u2014 ' + preyN + ' burns ' + burn + '.');
    }
    ...
    if(jack && this.pool > 0){       // MANDATORY wording ('move 1 counter from your pool') -- the only guard is having pool at all
      ...
      this.chat('Trainbot: Smiling Jack ticks \u2014 ' + jack.counters.length + ' counter(s).');
    }
    ...
    if(jackOwed > 0){
      ...
      if(this.pool >= jackFloor){ this.pool = Math.max(0, this.pool - jackOwed); this.chat('Trainbot: Smiling Jack \u2014 pays ' + jackOwed + ' pool.'); }
      else {
        let v = null; this.board.forEach(c => { if(c && c.kind === 'crypt' && c.zone === 'ready' && (!v || (c.blood | 0) > (v.blood | 0))) v = c; });
        if(v){ v.blood = Math.max(0, (v.blood | 0) - jackOwed); this.chat('Trainbot: Smiling Jack \u2014 ' + v.name + ' burns ' + jackOwed + ' blood.'); }
        else { this.pool = Math.max(0, this.pool - jackOwed); this.chat('Trainbot: Smiling Jack \u2014 pays ' + jackOwed + ' pool (no vampire).'); }
      }
      this.push();
    }
    if(this._famedMine && this._famedMine.size){   // the 1/unlock drain -- OWN-unlock settle only (documented UNDER-claim v1)
      ...
      if(drained > 0){ this.pool = Math.max(0, this.pool - drained); this.push(); this.chat('Trainbot: Fame drains ' + drained + ' (own-unlock settle).'); }
    }
  }
  ...
  _bestRushFor(vamp){                // N2: a usable rush card (fx.rush) + a live target
    ...
    for(let idx = 0; idx < this.hand.length; idx++){
      ...
      if(!target) return null;
      return { idx, name: h.name, kind: 'rush', target, lockTarget: lockT, st: 0, n: 1, exp: this._rushExpectation(vamp) };   // v0.6.74: the expectation rides the pick
    }
    return null;
  }
  _bestUnlockReactFor(vamp, when){   // N1: a usable TRUE-unlock reaction for this vampire in this WINDOW -- fx.unlock, mode.when === when (curated riders; bare fx.unlock cannot tell Guard Dogs from Cats' Guidance)
    ...
    for(let idx = 0; idx < this.hand.length; idx++){
      ...
      for(const mo of (h.e.modes || [])){
        ...
        if(!this._modeUsableBy(mo, vamp)) continue;
        const cB = (h.e.cost && h.e.cost.blood) || 0; if((vamp.blood | 0) < cB) continue;
        if(!found || mo.fx.maneuver) found = { idx, name: h.name, cost: h.e.cost || null, maneuverGrant: !!mo.fx.maneuver };   // v0.6.28's lesson, third appearance: a later RICHER usable mode overrides the earlier plain one (superior-if-able)
      }
  ...
  async _playUnlockReact(vamp, un){  // N1: splice/ash/pay + TRULY unlock (wake's stronger sibling) + the [ANI] maneuver-grant rider
    ...
    this.fxClone({ name: un.name, kind: 'lib' }, 'unlocks ' + vamp.name);
  ...
  async _maybePostBlockUnlock(blocker){   // N1: Cats' Guidance -- after block resolution, a LOCKED blocker with a postBlock unlock refreshes the wall
    ...
    this.chat('Trainbot: ' + blocker.name + ' unlocks (' + un.name + ') \u2014 the wall refreshes.');
  }
  ...
  _bestPreventFor(vamp){             // a usable damage-prevention card for MY incoming hit (v1: assumes 1, the rulebook default)
    ...
    this.hand.forEach((n, idx) => {
      ...
      (h.e.modes || []).forEach(mo => {
        ...
        if(!best || val > best._val) best = { idx, name: h.name, n: pv, _val: val, cost: h.e.cost || null };
      });
    });
    return best;
  }
  ...
  async _ashurResolve(iTriggered){     // v1.4.3 (14 July): the shared removal+benefit logic -- called EITHER when *I* just crossed my own threshold (iTriggered:true, from CARD_HANDLERS right after playing my 3rd copy) OR when _checkAshurTableWide observes someone ELSE crossed theirs (iTriggered:false). My own copies are removed EITHER way (the rulebook: "remove all copies in play... even controlled by other Methuselahs") -- only the actual trigger gets the +3 pool and the ash-heap dig; a collateral loss gets nothing, exactly per the card text.
    ...
    if(iTriggered){
      ...
      this.chat('Ashur Tablets: 3 in play \u2014 removing them for 3 pool + an ash-heap dig.');
      ...
      if(ashEntries.length){
        ...
        this.chat('Ashur Tablets: retrieved ' + pickEntry.c.name + ' to hand' +
      ...
      this.chat('Ashur Tablets: triggered elsewhere at the table \u2014 my own cop' + (copies.length === 1 ? 'y is' : 'ies are') +
  ...
  async _considerLockRescue(phaseKey){   // v0.6.39 (Johan's ask, Chantry -- the last card on phase-income-v2-plan.md): source-verified vekn.net first, both the card text AND the official ruling ("Rescue a Vampire from Torpor" is a standardized action type whose cost splits between the payer and the rescued vampire). Kept as its OWN function rather than a fourth branch on _considerLockIncome, even though the scan shape is identical -- "income" doesn't semantically fit a torpor-to-ready move, matching this session's own established naming precedent (_recomputeHandSizeBaseline is a separate sibling of _recomputePhaseActs, not a bolted-on branch). Measured the whole family first: only 3 cards mention "from torpor"+"ready region" at all, and the other two (Renewed Vigor, Torpid Blood) share nothing structurally with this shape -- rescueTorpor is Chantry-specific vocabulary, not a guessed-at generalization. v1 scope: only rescues the bot's OWN torpored vampire of the matching clan (the card technically can help ANY player's Tremere, per the official ruling's "their controller" -- multiplayer-diplomacy altruism is out of scope, no infrastructure for it). Cost preference: pool first (never weakens a combat-capable ready vampire), falling back to blood from a ready same-clan vampire only if pool is insufficient.
    for(const c of this.board){
      ...
      for(const mo of h.e.modes || []){
        ...
        this.chat(c.name + ': Trainbot locks it and burns ' + amount + ' ' + (payer === 'pool' ? 'pool' : 'blood from ' + payer.name) + ' to rescue ' + target.name + ' from torpor.');
      }
    }
  }
  async _considerHuntingGround(phaseKey){   // v0.6.41 (Johan's ask, "process the hunting ground family, gives many free to other decks"): source-verified vekn.net first -- 40 cards mention "hunting ground", measured before curating anything. 23 share this EXACT effect across two surface phrasings ("a ready vampire you control can gain 1 blood" / "you may move 1 blood from the blood bank to a ready vampire you control"), unlock-phase, NOT lock-gated -- a genuinely new THIRD shape (freestanding, phase-triggered, optional-but-obviously-always-taken), distinct from _applyPhaseIncome's host-attached automatic triggers and _considerLockIncome's lock-gated choices. bloodAddTo:'own-ready' already existed from Life in the City -- reused, not reinvented. Kept as its own function (not a branch on either sibling) for the same naming-clarity reason _considerLockRescue was: this scan has no lock check at all, a structurally different shape worth its own name. Fails closed on req.title exactly like the ALREADY-established precedent at _bestBloodStockFor's own title-gate (titles are untracked bot-wide, politics-engine scope) -- Papillon stays correctly unreachable, not silently mis-played.
    ...
    for(const c of this.board){
      ...
      for(const mo of h.e.modes || []){
        ...
        this.chat(c.name + ': ' + target.name + ' gains ' + mo.fx.bloodAdd + ' blood.');
  ...
  async _considerHandCycle(phaseKey){   // v0.6.42 (Johan's ask, The Barrens -- from the 19 July backlog note): source-verified vekn.net first -- "You can lock this card to discard a card (draw up afterward)", NOT phase-restricted by the card itself; phase:'master' is this bot's own sensible-timing pick, the SAME precedent already set for Dreams of the Sphinx's own un-phase-restricted choices. Kept as its own function (not a branch on _considerLockIncome) for the same naming-clarity reason _considerLockRescue/_considerHuntingGround were -- a hand-cycling trade isn't "income" in any sense those siblings share. Reuses _deadHandIndex verbatim (the SAME dead-card-detection already trusted for the discard-phase and _revertHandSizeBonus) -- only fires when a genuinely dead card exists to discard; never trades away a live, useful card for an unknown random one.
    for(const c of this.board){
      ...
      for(const mo of h.e.modes || []){
        ...
        this.fxClone({ name: c.name, kind: 'lib' }, 'is locked');
        this.chat(c.name + ': Trainbot locks it to discard ' + discarded + ' and draw a fresh card.');
        return;
      }
    }
  }
  _applyPhaseIncome(phaseKey){        // v0.5 (§7.6 C, Johan: phase-EXACT): recurring master-permanent income fires at the CARD's OWN phase -- the timing lives in cardfx's `phase` field on the mode (Blood Doll: 'master', Vessel: 'unlock'), never hardcoded per card name here, so any future income asset only needs tagging in the library, not a bot code change. v1: bloodToPool + poolGain only for HOST-ATTACHED permanents -- freestanding lock-gated income (Dreams of the Sphinx) goes through the separate _considerLockIncome above instead, since it's a CHOICE (spend the lock), not an automatic trigger.
    ...
    this.board.forEach(c => {
      ...
      (h.e.modes || []).forEach(mo => {
        ...
        if(typeof mo.fx.bloodToPool === 'number' && (host.blood | 0) > 0 && !(lastBlood && poolSafe)){
  ...
  _writeTally(extra){                // v0.6.7 (server v2.6.17 pairing): send ONLY the fields the caller owns -- the server now PRESERVES absent fields while the mode is unchanged, so the old merge-fill (which carried our last-OBSERVED copy of the OTHER side's counter) is exactly what raced: our stealth write could clobber the defender's just-written intercept under whole-state last-write-wins. `mode` is always sent (the server keys preservation on it). Our own state syncs via the broadcast echo -- the server echoes tally to the sender too (empirically verified 16 July; log lines are NOT echoed, tally/chat/say ARE).
    this.send(Object.assign({ t: 'tally', mode: (extra && extra.mode) || (this.tally && this.tally.mode) || null }, extra));
  }
  ...
  _announceTallyA(raw){
    ...
    if(this.caps.tally) this._writeTally({ mode: 'block', a: Math.min(99, Math.max(-9, raw | 0)), b: 0 });
  }
    ...
    switch(q){
      case 'block-as-target': {     // M2: bounce beats a contested block outright when available (cheap, decisive, no risk). steg 5 (16 July, rulebook-verified): `ctx.blockAllowed=false` means blocks were already FINALLY declined for this action -- the rulebook's "Detailed course of an action" (C: blocks declined by all) never reopens them short of a target change, but the reaction window (bounce) STAYS live through every modifier exchange: Deflection & co are, per their own card text, "only usable after blocks are declined", so post-decline is bounce's NORMAL home, not an exception.
        if(ctx.T && ctx.T !== 'T1'){    // v0.6.107 (B1, doctrine section 3.2): the UNDIRECTED classes -- odds x threat x combatFit - lockCost, times the deck axis, over blockShy; the directed T1 path below is byte-identical (its axes come with B2's arena evaluation)
          ...
          if(!this.caps.tally) return { block: false, bounce: false, score: 0, why: 'classic table (no resolver) \u2014 sandbox default: let it through' };
        ...
        if(!this.caps.tally) return { block: false, bounce: false, why: 'classic table (no resolver) \u2014 sandbox default: let it through' };
        ...
        if(ctx.icp <= 0) return { block: false, bounce: false, why: 'no usable intercept in hand' };
      ...
      case 'react-delay':                              // v0.4.5 (Johan, live test): a reaction is still a bot command -- it gets the SAME paceMs floor as a turn action (was capped at thinkMs=1600, under the 2s ask); a persona's thinkMs can only make the pause LONGER, never shorter than a real table's pace
        return { ms: this.o.paceMs ? Math.max(this.o.paceMs, P.thinkMs) : 40 };
      ...
      case 'action': {              // plain bleed 1 vs a bleed-action card vs a pool-damage action (fas B) from hand
        let pick = ctx.plain, why = 'plain bleed', sPick = 1 * P.aggression;
        const scores = { bleed: +(1 * P.aggression).toFixed(2) };   // A2: the losers' podium
        if(ctx.stock && ctx.readyCrypt < P.grinderReadyTarget){
        ...
        if(ctx.pd){
          ...
          const sPd = ctx.pd.n * P.aggression - (ctx.pd.costEdge ? 0.5 : 0); scores.pd = +(sPd).toFixed(2);
        ...
        if(ctx.peek){
          ...
          const sPk=(1.2 + (noIntel?0.6:0) + 0.4*this.readP(preyS,'bounce') + (ctx.peek.supMode?0.3:0)) * P.aggression; scores.peek = +(sPk).toFixed(2);
        ...
        if(ctx.recruit){
          const sRec = (0.85 + 0.1 * ctx.recruit.life) * P.aggression; scores.recruit = +(sRec).toFixed(2);   // N4d: a defensive tool -- below live pressure lines, above idling
          if(sRec > sPick){ pick = ctx.recruit; sPick = sRec; why = ctx.recruit.name + ' [' + (ctx.recruit.at || '') + '] life ' + ctx.recruit.life + ': ' + sRec.toFixed(2); }
        }
        if(ctx.strength){
          const sSt = (0.9 + 0.15 * ctx.strength.bonus) * P.aggression; scores.strength = +(sSt).toFixed(2);   // N4b: combat prep -- loses to a live bleed line, wins over idling
        ...
        if(ctx.creep){
          ...
          const sC = (1.15 + 0.1 * myCreeps) * P.aggression; scores.creep = +(sC).toFixed(2);   // N3: the drip stacks -- each tabled copy makes the next worth more
        ...
        if(ctx.rush){
          ...
          const tb = ctx.rush.target.tBlood | 0;
          const mult = ex.expDmg >= tb ? 1.35 : (ex.expDmg >= tb - 1 ? 1.0 : 0.55);   // v0.6.74: Johan's rule -- no combat cards wants 0-1 blood targets
          const sR = (0.8 + 0.12 * (ctx.rush.target.w || 0)) * mult * P.aggression + Math.min(0.15, 0.05 * ex.nCombat); scores.rush = +(sR).toFixed(2);
        ...
        if(ctx.equip){
          const sEq = ctx.equip.tier === 1 ? 1.5 : 1.15; scores.equip = +(sEq).toFixed(2);   // E1: intercept-equipment (the wall investment) outranks weapons/armor; deliberately NOT aggression-scaled
          if(sEq > sPick){ pick = ctx.equip; sPick = sEq; why = 'equip ' + ctx.equip.name + ' (tier ' + ctx.equip.tier + '): ' + sEq.toFixed(2); }
        }
        if(ctx.fetch){
          const sF = 1.35 + (ctx.fetch.tier === 1 ? 0.35 : 0); scores.fetch = +(sF).toFixed(2);   // E1: the tutor beats the raw equip slightly (card selection), tier-1 targets more still
          if(sF > sPick){ pick = ctx.fetch; sPick = sF; why = 'Magic of the Smith -> ' + ctx.fetch.target.name + ' (tier ' + ctx.fetch.tier + '): ' + sF.toFixed(2); }
        }
        if(ctx.card){
          const sCard  = ctx.card.n * P.aggression - 0.35 + (ctx.card.st | 0) * 0.2 - (this._lastPickName === ctx.card.name ? 0.15 * (this._pickStreak | 0) : 0);   // v0.6.79: repeat-pick decay -- the freeze antidote   // a card in hand has value — small spend cost; fas A (16 July): built-in stealth is WORTH something (each point lowers block odds -- Night Moves at bleed 1 was scoring 0.65 vs plain 1.0 and never got played, its +3/+6 stealth weighed at zero); 0.2/point is the deliberately dumb v1 weight
          if(sCard > sPick){ pick = ctx.card; sPick = sCard; why = ctx.card.name + ': ' + sCard.toFixed(2) + ' vs best-so-far'; }
        }
      ...
      case 'spend-stealth': {        // v0.5+ (Johan, 14 July): "every action treated fundamentally the same" -- the bot CAN try to answer Block! with stealth regardless of which action is under attack (bleed, hunt, anything future); aggression/insight decide overall willingness same as before, and a NEW stakes factor lets a persona weight WHICH actions are worth burning a stealth card on -- a bleed's stakes scale with its amount (2 = an unmodified typical bleed, today's baseline, unchanged behaviour), hunt's fixed 1-blood prize is deliberately worth less than that baseline, so the SAME persona is now less eager to spend precious stealth defending a hunt than an equivalent-strength bleed. Capped at 1 (a huge bleed doesn't get MORE eager than the base aggression/insight read already allows) and floored so it never zeroes out entirely.
        const stakesW = Math.min(1, Math.max(0.25, (ctx.stakes || 1) / 2));
        return { spend: P.aggression * (1 - P.insight * ctx.pInt) * stakesW >= 0.5,
                 why: 'aggr ' + P.aggression + ' × (1 − insight ' + P.insight + ' × P(int) ' + ctx.pInt.toFixed(2) + ') × stakes-weight ' + stakesW.toFixed(2) + ' (stakes ' + (ctx.stakes || 1) + ')' };
      }
  ...
  _onBoard(m){
    ...
    if(this._pentexWatch && m.seat === this._pentexWatch.seat){   // M2: freedom returns when the actor's Pentex leaves ready play
      ...
      if(pxNow) this._pentexWatch.seen = true;
        ...
        this.chat('Trainbot: Pentex left play \u2014 ' + (this._pentexVamp || 'the vampire') + ' can block again.');
    ...
    if(this._openHandActive && m.seat === this._openHandActive.seat){   // R3: the grant dies with the actor's tabled Revelations
      ...
      if(revNow) this._openHandActive.seen = true;
        ...
        this.chat('Trainbot: Revelations left play \u2014 open hand ends.');
      }
    }
  ...
  _reactWindow(){
    ...
    const bounceLegal = !!actorSeat && this.preySeat() !== actorSeat && p.kind !== 'pool' && p.kind !== 'undirected' && p.kind !== 'rush';   // v0.6.107 (B1): only a BLEED can be redirected   // fas B: pool damage is NOT a bleed -- Deflection & co are, per their own text, "only usable by the Methuselah being bled"; no bounce window exists against Inside Dirt
    ...
    if(bounce){                        // v0.6.79 (match improvement 1): save the LAST bounce for lethal hits -- amount/pool danger gate
      ...
      const PB = this.persona || PERSONAS.grinder;   // v0.6.104 (#6): same fix as the Jack floor -- the v0.6.16 class, site two
    ...
    const d = this.decide('block-as-target', { icp: this.interceptPotential(p.reacted, p), amount: p.amount, stealthA, hasBounce: !!bounce, blockAllowed: !p.declined,
    ...
    this._emit('block-eval', { kind: p.kind, sub: p.sub || null, T: p.T || 'T1', role: p.role || 'target', block: !!d.block, bounce: !!d.bounce, score: (typeof d.score === 'number') ? +d.score.toFixed(2) : null, parts: d.parts || null, why: d.why });   // v0.6.107 (B1): every window is evaluable
  ...
  async _commitBounce(p, bounce){          // Deflection & co (rulebook, source-verified): redirects to ANY Methuselah other than the acting minion's controller -- v1 defaults to MY OWN prey, the conventional choice (pass the bleed downstream, same as a real player usually does). v1.4.3 (14 July): the log line is now a SPEC-CORRECT L12.bleed re-announce with a Target: suffix, not a freeform prose line -- fixed because (a) the diagnostic run proved the old freeform line didn't match any L12 pattern at all, so the NEW target's pending never armed, and (b) the real human client uses exactly this mechanism (pool-targeting + targetSuffix() on the next log line) for the same operation, confirmed from klientkoden. Johan, 14 July: "man sätter target på den man vill Bounca till och spelar kortet, som då genom klonamination och logg visar vem som är target." steg 6 (16 July): now async -- a LOCKED reactor spends a wake FIRST (rulebook glossary: a woken vampire "can ... play reaction cards as though unlocked for the duration of the action"; it STAYS locked -- wake ≠ unlock), mirroring _commitBlock's own wake path, with a pace beat between the two plays.
    ...
    this.fxClone({ name: bounce.name, kind: 'lib' }, 'bounces the bleed');
    ...
    this.chat('Trainbot: ' + bounce.vamp.name + ' plays ' + bounce.name + ' \u2014 the bleed changes target to ' + newPreyName + '.');
  ...
  _playBleedModifier(vamp, mod){     // steg 2 (15 July): pays the cost, bumps _actingVamp.n, and sends a SPEC-CORRECT L12.add re-announce -- the bot's first-ever sender of this line (PROTOCOL §12 already documented it for a human's own +X bleed submenu; a receiving bot's _onLog already reopens ITS _reactWindow on any matching add, built for that human case, works identically here with zero new receiving-side code). Re-anchors the hand index by NAME (the same v0.4.2 bounce lesson: an await between scoring and playing may let the hand shift under us).
    ...
    av.n += mod.n;
    this.fxClone({ name: mod.name, kind: 'lib' }, '+' + mod.n + ' bleed');
    ...
    this.chat('Trainbot: ' + vamp.name + ' plays ' + mod.name + ' (+' + mod.n + ' bleed, now ' + av.n + ').');
    this.push();
    return true;
  }
  ...
  async _commitBlock(p, d){          // wake (if needed) -> an intercept card if one's usable -> the shared counter learns MY commitment
    ...
    if(!blocker){
      ...
      if(!plan && wk){
        const sleeper = this.board.find(c => eligible(c) && (c.blood | 0) >= wkCost);   // v0.4.2: only SPEND the wake when there is actually a sleeper to wake (and it can pay)
        if(sleeper) plan = { kind: 'wake', vamp: sleeper, wk, wkCost };
      }
    ...
    if(!blocker){
      if(plan.kind === 'unlockReact'){ await this._playUnlockReact(plan.vamp, plan.un); blocker = plan.vamp; }
        ...
        this.fxClone({ name: wk.name, kind: 'lib' }, 'may block as though unlocked');
    ...
    const ic = blocker && this._bestInterceptFor(blocker, p.reacted);
    if(ic){
      ...
      this.fxClone({ name: ic.name, kind: 'lib' }, '+' + ic.s + ' intercept');
      this.send({ t: 'log', html: '<b>' + esc(blocker.name) + '</b> plays <b>' + esc(ic.name) + '</b>: +' + ic.s + ' intercept.' });
      this.chat('Trainbot: ' + blocker.name + ' plays ' + ic.name + ' (+' + ic.s + ' intercept).');
      ...
      this.chat('Trainbot: ' + blocker.name + ' attempts the block.');
    }
  ...
  _clearCombatStashes(){             // v0.6.103 (#1): the five one-shot stashes + the persistent opponent Carrion Crows + the armed flag -- called at every own announce (actor), at every block commit (defender), at a rush comply, at dealt and at oust. NEVER at _resolveCombat entry: the defender's verdict hand-off runs ahead of the attacker's Blocked branch, so a legitimate opponent line may already sit here (the dodge-branch comment's case)
    this._oppStrike = null; this._oppTaken = null; this._oppPress = undefined; this._oppManeuver = undefined; this._oppGrapple = undefined; this._oppCarrionCrows = null;
  ...
  async _resolveCombat(myVamp, oppName){      // v0.6.22 (Johan's combat GO, the multi-round loop): the OUTER wrapper _resolveCombatRound was always meant to grow into (§7.5-C's original design: "plays at most the rounds a press grants"). Deliberately a THIN wrapper, not an internal rewrite -- _resolveCombatRound stays exactly as tested (every existing 37a-37g test calls it directly and is untouched), this just repeats it while there's an uncancelled press. v1 scope (source-verified vekn.net 19 July, combat plan §5.5): "if there is an UNCANCELLED press to continue, a new round begins" -- only ONE side needs to press. My bot NEVER attempts to cancel an incoming press (that decision -- "is it worth spending my own press just to deny theirs" -- is a future addition, not built); it presses when IT wants to (a simple, deliberately-dumb v1 threshold, matching grinderReadyTarget's own precedent: not fragile), and separately just detects whether the opponent pressed at all, treating any detected opponent press as uncancelled. Silent opponent (timeout, human table) is NOT treated as "they definitely didn't press" vs "they definitely did" -- it's simply absent evidence, so it doesn't extend combat on its own; only a DETECTED press (mine or theirs) does.
    ...
    for(let round = 1; ; round++){
      ...
      if(press && (myVamp.blood | 0) > 1){                    // v1-dumb "do I want to" threshold: not fragile -- same spirit as case 'combat-strike', no persona weighting yet
        ...
        this.fxClone({ name: press.name, kind: 'lib' }, 'presses to continue');
        this.chat('Trainbot combat \u2014 ' + myVamp.name + ' presses to continue.');
        this.push();
        continuing = true;
        ...
        this.chat('Trainbot combat \u2014 ' + myVamp.name + ' presses to continue (the Stray).');
        continuing = true;
      }
  ...
  async _resolveCombatRound(myVamp, continued){ // v1 CUT LINE (spec §7.5-C): announce-only, one round, applies MY side; the opponent's strike isn't modelled -- assumes the rulebook default (hands, 1) unless I prevent it. Never steps the shared ph/rd counters. v0.6.23 (standing-question fix, the multi-round loop): `continued` is a NEW, backward-compatible optional 2nd arg -- every existing call (all of 37a-37g, all 6 real call sites for round 1) passes nothing, so this defaults falsy and behaves EXACTLY as before. `_resolveCombat` passes true for round 2+ ONLY.
    ...
    if(!this._carrionCrows){
      if(this._borneEnvDmg > 0 && !this._borneAnnounced){
        ...
        this.chat('Trainbot combat \u2014 ' + myVamp.name + "'s Murder of Crows: opposing minion takes " + this._borneEnvDmg + ' environmental damage each round for the rest of this combat (unpreventable by strikes, cannot be dodged).');
        await this._pace();
      }
      ...
      if(cc){
        ...
        this.fxClone({ name: cc.name, kind: 'lib' }, 'plays Carrion Crows');
        this._emitPlay('Carrion Crows', 'combat-env', myVamp.name);
        this.chat('Trainbot combat \u2014 ' + myVamp.name + ' plays Carrion Crows: opposing minion takes ' + cc.n + ' environmental damage each round for the rest of this combat (unpreventable by strikes, cannot be dodged).');
        this.push();
      }
    }
    this._range = 'close';
    ...
    if(this._forceCloseNextRound){
      ...
      if(maneuver){
        ...
        if(!maneuver.fromEquip){ this.hand.splice(maneuver.idx, 1); this._drawOwed = (this._drawOwed || 0) + 1; } else { if(maneuver.fromGrant) this._reactManeuverGrant = false; else this._equipManeuverUsed = true; }   // E1/N1: board- and grant-maneuvers never splice; each consumes its OWN once-flag
          ...
          this.fxClone({ name: maneuver.name, kind: 'lib' }, 'maneuvers to long');
          this.chat('Trainbot combat \u2014 ' + myVamp.name + ' maneuvers to long.');
          this.push();
          myManeuvers = 1;
        }
      }
    ...
    if(grapple){
      ...
      if(!wouldCostMe){
        ...
        this.fxClone({ name: grapple.name, kind: 'lib' }, 'plays Immortal Grapple');
        this.chat('Trainbot combat \u2014 ' + myVamp.name + ' plays Immortal Grapple: strikes that are not hand strikes cannot be used this round (by either combatant).');
        this.push();
      }
    }
    ...
    if(dodge && dd.dodge){           // steg 6 (16 July): Strike: dodge -- no damage in EITHER direction this round FROM THE STRIKE ITSELF (so no prevent needed there); the vampire's own strike slot is consumed by the dodge
      ...
      this.fxClone({ name: dodge.name, kind: 'lib' }, 'dodges');
    ...
    if(steal && (myVamp.blood | 0) < (myVamp.cap | 0) && (!strike || steal.n >= (strike.n | 0))){
      ...
      this.fxClone({ name: steal.name, kind: 'lib' }, 'steals ' + steal.n + ' blood');
      ...
      this.chat('Trainbot combat \u2014 ' + myVamp.name + ' strikes with ' + steal.name + ': steals ' + steal.n + ' blood (not damage; blood now ' + myVamp.blood + ').');
      ...
      this.fxClone({ name: strike.name, kind: 'lib' }, 'strikes for ' + strike.n);
    ...
    if(taken > 0){ const _ep = this._equipPrevent(myVamp); if(_ep > 0){ taken = Math.max(0, taken - _ep); this.chat('Trainbot: equipment prevents ' + _ep + ' (once this combat).'); } }   // E1: Kevlar-class fires BEFORE spending a hand card
    ...
    if(prevent){
      ...
      this.fxClone({ name: prevent.name, kind: 'lib' }, 'prevents damage');
    ...
    if(unmended === 0){
      ...
      if(taste){
        ...
        if(oppT && oppT.n > 0){
          ...
          this.fxClone({ name: taste.name, kind: 'lib' }, 'gains ' + oppT.n + ' blood');
          this.chat('Trainbot combat \u2014 ' + myVamp.name + ' plays ' + taste.name + ': gains ' + oppT.n + ' blood lost by ' + oppT.vamp + ' (blood now ' + myVamp.blood + ').');
          this.push();
        }
      }
    }
  ...
  _applyPending(via){
    ...
    if(p.kind === 'peek'){
      ...
      if(p.peekSup){
        ...
        this._syncOpenHand();
        this.chat('Trainbot plays with an open hand (Revelations).');
        ...
        this.chat('Trainbot reveals the hand to ' + (p.who || 'the actor') + ' (Revelations).');
    ...
    if(p.kind === 'pool'){
      ...
      this._emit('took-pooldmg', { n: p.amount, pool: this.pool });
      if(this.pool > 0){
        this.push();
        this.chat('Trainbot takes the ' + p.amount + ' pool damage. Pool: ' + this.pool + '.');
      } else this._oust(false);
      return;
    }
    this._emit('took-bleed', { n: p.amount, pool: this.pool });
    if(this.edge) this.edge = false;                     // the successful bleeder takes the Edge from me
    if(this.pool > 0){
      this.push();
      this.chat('Trainbot takes the bleed for ' + p.amount + '. Pool: ' + this.pool + '.');
    } else this._oust(false);
  }
  ...
  _onLog(who, html){
    ...
    if((m = t.match(L12.pentexM))){
      ...
      if(m[2] === this.seatName(this.seat) && !this.out){
        ...
        this.chat('Trainbot: ' + m[3] + ' cannot block (Pentex).');
    ...
    if((m = t.match(L12.lockM))){
      ...
      if(m[2] === this.seatName(this.seat) && !this.out){
        ...
        if(vc && !vc.locked){ vc.locked = true; this.push(); this.chat('Trainbot: ' + vc.name + ' locks (Misdirection).'); this._emit('locked-by-misdirection', { vamp: vc.name }); }
      }
      return;
    }
    ...
    if((m = t.match(L12.fameM))){
      ...
      if(m[2] === this.seatName(this.seat) && !this.out){
        ...
        this.chat('Trainbot: ' + m[3] + ' is famed \u2014 torpor now costs me.');
      }
      return;
    }
    if((m = t.match(L12.rescueE))){   // v2 (#7): the OWNER side -- move my named vampire torpor->ready and pay my forced share (the acting controller chose the split; rulebook 6.5.3)
      if(m[2] === (this.o && this.o.name) || m[2] === this.name || m[2] === this.seatName(this.seat)){   // v0.6.72-fix: this.name lands at welcome -- o.name is constructor-truth (the creep comply uses seatName for the same reason)
        ...
        if(vc){
          ...
          this.push();
          this.chat('Trainbot: ' + m[3] + ' is rescued (pays ' + m[5] + ') \u2014 ready again.');
        }
      }
      return;
    }
    if((m = t.match(L12.creep))){
      ...
      if(m[2] === this.seatName(this.seat) && !this.out){
        ...
        if(nC > 0){ this.pool = Math.max(0, this.pool - nC); this._predPressure = (this._predPressure | 0) + nC; this.push(); this.chat('Trainbot: Creeping Sabotage \u2014 I burn ' + nC + ' pool.'); }   // v0.6.58: creep loss IS predator pressure -- 'your prey burns' makes the actor MY predator by definition (Jack/Fame payments stay excluded: cross/self, not predator-attributable)
      }
      return;
    }
    if(this._parseUndirected(who, t)) return;   // v0.6.107 (B1): the undirected family -- arms a window for the actor's prey/predator
    if((m = t.match(L12.attempt)) || (m = t.match(L12.interceptPlay))){   // v0.6.106 (#14): another seat's block attempt -- the blocker's vampire name (latest attempt wins) + the table climate counter
  ...
  say(phrase){ const i = sayIdx(phrase); if(i >= 0) this.send({ t: 'say', i }); }
  _seatOfName(name){ const p=(this.players||[]).find(q=>q && q.name===name); return p ? (p.seat|0) : 0; }   // R3: roster lookup, 0 when unknown
  _emit(type, data){ if(this.onEvent) try{ this.onEvent('bot:' + type, data || {}); }catch(e){} }
  _flushDraws(){                     // v0.5: extracted so master-play and hunt (which `continue`s past the old inline drain) can never silently DEFER a draw-owed -- every code path that increments _drawOwed now has a nearby place to flush it instead of relying on some LATER vampire's bleed branch happening to reach it
    while((this._drawOwed || 0) > 0){ this._drawOwed--; this.send({ t: 'draw' }); }
  }
  ...
  fxClone(card, verb, opts){              // a presentational clone for the humans (v0.3.7, Johan: bleeds should ANIMATE). v0.4.5: optional opts.kind ('rise' for a vampire brought into play -- the client's OWN uncontrolled/torpor->ready move() already fires the identical cardFx(c,'rise',{reveal}), and the protocol/server already relay 'rise'; the bot just never sent it) + opts.reveal (the rise flip). Omitted opts = byte-identical to the old 2-arg call (kind:'play', reveal:undefined).
    ...
    this.send({ t: 'fx', kind: opts.kind || 'play', card: { name: card.name, kind: card.kind || 'lib', faceDown: false },
                target: null, actor: null, onto: null, verb: String(verb || '').slice(0, 40), reveal: opts.reveal, toName: '' });
  }
  ...
  _noteBlockAttempt(vamp, own){      // v0.6.106 (#14): one site for both readers -- others' attempts arrive as frozen log lines, my own are noted at _commitBlock. The name is what the actor's handshake filter keys on (_expectedOppVamp); the counter is v0.6.93's table block climate (READ = BASE; consumers are documented, not built)
    if(!own && vamp) this._lastBlockerName = vamp;
  ...
  _holdHook(who, i, txt){
    ...
    if(hold){
      ...
      const fresh = this._holdBy.size === 0;
      this._holdBy.add(who);
      if(fresh){ this.chat('\u23f8 Holding \u2014 say "It resolves" (or any flow phrase) when ready.'); this._holdRemind(); }
    } else if(rel && this._holdBy && this._holdBy.size){
      this._holdBy.delete(who);
      if(this._holdBy.size === 0){
  ...
  _holdRemind(){
    ...
    this._holdRemT = setTimeout(() => {
      ...
      if(this.turnSeat === this.seat && !this.out)
  ...
  _onSay(m){
    ...
    if(this._ask){
      ...
      if(phrase === 'Hold on\u2026'){                               // someone is thinking — pause my timeout
        a.pausedBy.add(m.who); if(a.timer){ clearTimeout(a.timer); a.timer = null; }
        return;
      }
      if(phrase === 'Block!') return this._onBlockAttempt(m.who);
      if(phrase === 'No block' || phrase === 'No reaction' || phrase === 'Pass'){
  ...
  async _maybeAttachMarkedPath(v, targetSeat, played){
    ...
    this.fxClone({ name: best.name, kind: 'lib' }, 'is put on ' + v.name);
    this.send({ t: 'log', html: '<b>' + esc(v.name) + '</b> puts <b>' + esc(best.name) + '</b> in play (on itself).' });
    this.chat('Trainbot: ' + best.name + ' goes on ' + v.name + ' \u2014 burnable later for +' + best.s + ' stealth against the same target.');
    this.push();
  }
  _maybePerfectionist(v){
    ...
    this.chat('Trainbot: ' + v.name + ' +1 blood (Perfectionist \u2014 clean successful action). Blood: ' + v.blood + '.');
    this.push();
  }
  ...
  _onBlockAttempt(who){
    ...
    if(cb){
      ...
      if(d.spend){
        ...
        this.fxClone({ name: cb.name, kind: 'lib' }, 'cancels the block');
        ...
        this.chat('Trainbot: ' + av.v.name + ' plays ' + cb.name + ' \u2014 the block attempt is cancelled (that minion cannot try again this action).');
        this.push();
    ...
    if(st && !a.contested){
      ...
      const d = this.decide('spend-stealth', { pInt, stakes: (typeof av.n === 'number' ? av.n : 1) });
      this.o.log('block attempt by ' + who + ': ' + d.why + ' → ' + (d.spend ? 'stealth' : (this.caps.tally ? 'await verdict' : 'combat')));
      if(d.spend){
        a.contested = true;
        ...
        this.fxClone({ name: st.name, kind: 'lib' }, '+' + st.s + ' stealth');
        ...
        this.chat('Trainbot: ' + av.v.name + ' ' + (st.fromBoard ? 'burns' : 'plays') + ' ' + st.name + ' (+' + st.s + ' stealth).');
        this.push();
        this._restartAskTimer();             // the table resolves the contest (§12 Block line / prey pass)
        return;
      }
    }
    if(this.caps.tally){                               // v0.3.7 (Johan): with resolvers live, 'Block!' opens the REFERENDUM — wait for the §12 verdict instead of assuming the block lands. v0.3.8: EVERY repeated 'Block!' refreshes the wait (the old !contested guard fell through to instant combat on the second shout)
      a.contested = true;
      this._restartAskTimer();
      return;
    }
    this._answer('block', who);
  }
  ...
  _onChat(who, msg){
    ...
    let tm = txt.match(/(?:dealt; |^(?:Trainbot combat \u2014 )?)([^;]+?) (?:takes|dodges the strike .*?takes) (\d+) \(loses (\d+) blood/);   // v0.6.103 (#1): the vampire that TOOK the damage (after 'dealt; ' in the normal line, after the prefix in the dodge+crows line) -- the old lazy (.+?) captured the whole strike clause
    ...
    const pm0 = txt.match(/^(?:Trainbot combat \u2014 )?(.+?) presses to (continue|end)/i);   // v0.6.103 (#1): bare vampire name (the old regex captured 'Trainbot combat -- Lisa')
    ...
    const mm0 = txt.match(/^(?:Trainbot combat \u2014 )?(.+?) maneuvers to (close|long)/i);
    ...
    const ccm0 = txt.match(/^(?:Trainbot combat \u2014 )?(.+?)(?: plays carrion crows|'s murder of crows): opposing minion takes (\d+) environmental damage each round/i);
    ...
    const igm0 = txt.match(/^(?:Trainbot combat \u2014 )?(.+?) plays immortal grapple: strikes that are not hand strikes cannot be used this round/i);
    ...
    /* damage grammar: "bleed 2" / "vote 3" (optionally prefixed "trainbot:" / "@trainbot") */
    const dm = txt.match(/^\s*(?:@?trainbot[:,]?\s*)?(bleed|vote)\s+(\d{1,2})\s*$/i);
    if(dm){
      if(this.out) return this.chat('Trainbot is already ousted — the dead bleed no more.');
      const kind = dm[1].toLowerCase(), n = Math.min(30, parseInt(dm[2], 10) || 0);
      if(!n) return;
      this.pool -= n;
      if(this.pool > 0){
        this.push();
        this.chat('Trainbot takes the ' + kind + ' for ' + n + '. Pool: ' + this.pool + '.');
      } else this._oust(false);
      return;
    }
    if(/^\s*(?:@?trainbot[:,]?\s*)?rush\b/i.test(txt))
      return this.chat('Trainbot has no combat module yet (M2). Treat my vampires kindly — or the host can referee.');
    if(/^\s*(?:@?trainbot[:,]?\s*)?help\b/i.test(txt))
      return this.chat('Trainbot v' + BOT_VERSION + ' — I understand: "bleed N", "vote N", "block"/"no" (when I ask), "help".');
  }
  ...
  _answer(what, who){
    ...
    if(a){ if(a.timer) clearTimeout(a.timer); a.resolve({ what, who }); }
  }
  _armReactTimer(){                    // v0.3.10 (Johan): after 5 s of silence, ring the table's EXISTING Reaction timer — the server broadcasts who + the room's reactSecs, every client shows the countdown. The bot's own patient window (askSecs) is untouched: the stopwatch prompts, the ask decides.
    ...
    this._rcT = setTimeout(() => {
      this._rcT = null;
      if(this._ask) this.send({ t: 'decide' });
    }, 5000);
  }
  _restartAskTimer(){
    const a = this._ask; if(!a) return;
    ...
    a.timer = setTimeout(() => {
      ...
      if(a.contested && this.caps.tally && this.tally && this.tally.mode === 'block'){
    ...
    }, this.o.askSecs * 1000 + this._chatSettleMs());       // v0.6.100 (Johan): the clock starts AFTER the declaration bubbles settle -- animation, then declaration, then the stopwatch, in succession. v0.3.8: non-contested silence always means 'no' -- post-stealth quiet is a conceded block (classic); v0.6.3: a CONTESTED timeout now resolves the referendum from the tally instead (see above)
  }
  ...
  _askBlock(question){
    return new Promise(resolve => {
      ...
      if(undirected){
        ...
        if(!elig.size) return resolve({ what: 'no', who: null });   // nobody left to block
        this.chat(question + ' \u2014 say "block" or "no" (prey first, then predator; ' + this.o.askSecs + 's \u2192 no)');
        this._ask = { resolve, timer: null, pausedBy: new Set(), contested: false, target: targetSeat, undirected: true, eligible: elig, passed: new Set() };
        this._restartAskTimer();
        this._armReactTimer();
        return;
      }
      ...
      this.chat(question + ' — say "block" or "no" (' + this.o.askSecs + 's → no)');
      this._ask = { resolve, timer: null, pausedBy: new Set(), contested: false, target: targetSeat };   // v0.6.106 (#3): the roster hook reads THIS
      this._restartAskTimer();
  ...
  _oust(byHost){
    ...
    this.push();
    this.chat('Trainbot is ousted' + (byHost ? ' by the host' : '') + '. The shadows reclaim their own. 🦇');
    if(!byHost) this.send({ t: 'bounty' });        // server routes +6/+1 VP to my predator
  }
  ...
  async _unlockPhase(){
    ...
    this.board.forEach(c => {
      ...
      if((c.zone === 'ready' || c.zone === 'torpor') && c.locked){
        if(skip){ this.chat(c.name + ' does not unlock as normal (her printed text).'); return; }
        c.locked = false; n++; } });
  ...
  async _playTurn(){
    ...
    try{
      this.ownTurns++;
      await pace();
      this.chat('— Trainbot, turn ' + this.ownTurns + ' —');
      ...
      await this._considerHandCycle('master');   // v0.6.42: The Barrens -- not phase-restricted by the card itself, master is this bot's own sensible-timing pick
      {
        ...
        while(this._phaseAvail('master') > 0 && !this.out){
          ...
          if(_lockM && _lockT){
            ...
            this.chat('Trainbot: Misdirection \u2014 ' + _lockT.name + ' locks. Clearing the lane.');
            this.push();
          ...
          if(_fmM && _fmT){
            ...
            this.chat('Trainbot: Fame on ' + _fmT.name + ' \u2014 torpor will cost them.');
            this._famedSeen = this._famedSeen || {};
          ...
          if(_sjM){
            ...
            this.chat('Trainbot: Smiling Jack is in play \u2014 the meter starts at my unlock.');
            this.push();
            played.add(FX_NORM(d.pick.name));
            this.phaseUsed.master++;
            if(d.pick.trifle) this._grantTrifleBonus();
          ...
          if(_nbM && _nbT){
            ...
            this.chat('Trainbot: ' + d.pick.name + ' \u2014 ' + _nbT.name + ' cannot block.');
            this.push();
            played.add(FX_NORM(d.pick.name));
            this.phaseUsed.master++;
            if(d.pick.trifle) this._grantTrifleBonus();
          ...
          this.chat('Trainbot: ' + d.pick.name + (host ? ' on ' + host.name : '') + (d.pick.curated ? '' : ' (uncurated \u2014 adjust if needed)') + '.');
          this.push();
      ...
      const actors = this.board.filter(c => c.zone === 'ready' && c.kind === 'crypt' && !c.locked)
                                .sort((a, b) => ((a.blood | 0) === 0 ? 0 : 1) - ((b.blood | 0) === 0 ? 0 : 1));   // v0.5: 0-blood vampires FIRST — the mandatory-hunt precondition
      ...
      for(let _ai = 0; _ai < actors.length; _ai++){ const v = actors[_ai];   // v0.6.64: index form over the live queue (was a for..of SNAPSHOT -- the refreshed vampire's extra action was an illusion)
        if(this.out) break;
        await pace();
        ...
        if(hd.hunt){
          ...
          await sleep(150);
          if(ans.what === 'block'){
            this.chat('Blocked' + (ans.who ? ' by ' + ans.who : '') + '. Combat begins.');
            ...
            this.chat(v.name + ' hunts successfully \u2014 +1 blood (' + v.blood + ').');
            this.push();
          }
        ...
        const choice = this.decide('action', { plain: { n: 1 + this._cryptBleedBonus(v) }, card: this._bestBleedCardFor(v), pd: this._bestPoolDmgActionFor(v), stock: this._bestBloodStockFor(v), peek: this._bestPeekFor(v), equip: this._bestEquipActionFor(v), fetch: this._bestFetchFor(v), rush: this._bestRushFor(v), creep: this._bestCreepFor(), strength: this._bestStrengthFor(v), recruit: this._bestRecruitFor(v), enemyRescue: this._bestEnemyRescueFor(v), readyCrypt: this.board.filter(c => c.kind === 'crypt' && c.zone === 'ready').length, edge: this.edge });   // fas B: edge in the ctx -- the arena traces need it to distinguish 'Dirt unfound' from 'Edge not held' (in 2-player the opponent's intervening bleed usually reclaims it, so Dirt windows are rare and PRECIOUS)   // VA: plain bleed carries the Lenny-class crypt bonus from birth
        const act = choice.pick; this.o.log('action (' + v.name + '): ' + choice.why);
        if(act.name){ this.hand.splice(act.idx, 1); this._drawOwed = (this._drawOwed || 0) + 1; }
        ...
        if(act.kind === 'stock'){
          ...
          if(act.cost && act.cost.blood) v.blood = Math.max(0, (v.blood | 0) - act.cost.blood);   // v0.6.14: the card's blood cost, paid by the acting vampire at announce (Govern: 1) -- the same Math.max floor the bounce payment uses
          this.push();
          this.fxClone({ name: act.name, kind: 'lib' }, 'stocks ' + act.n + ' blood');
          ...
          await sleep(150);
          if(ansS.what === 'block'){
            this.chat('Blocked' + (ansS.who ? ' by ' + ansS.who : '') + '. Combat begins.');
            ...
            this.chat('Trainbot: ' + act.n + ' blood banked onto an uncontrolled vampire \u2014 the next rise comes cheaper.');
            this.push();
        ...
        if(act.kind === 'enemyRescue'){
          ...
          await sleep(150);
          if(ansR.what === 'block'){
            this.chat('Blocked' + (ansR.who ? ' by ' + ansR.who : '') + '. Combat begins.');
            ...
            this.push();
            this.chat('Trainbot: ' + act.name + ' rises \u2014 famed and ready. The yo-yo turns.');
          }
          this._actingVamp = null;
          this._flushDraws();
          continue;
        ...
        if(act.kind === 'recruit'){
          ...
          this.push();
          this.fxClone({ name: act.name, kind: 'lib' }, '[' + (act.at || '') + '] recruits (life ' + act.life + ')');
          ...
          await sleep(150);
          if(ansRc.what === 'block'){
            this.chat('Blocked' + (ansRc.who ? ' by ' + ansRc.who : '') + '. Combat begins.');
            this._toAsh(act.name);
            ...
            this.push();
            this.chat('Trainbot: ' + act.name + ' enters play (life ' + act.life + ').');
        ...
        if(act.kind === 'strength'){
          ...
          this.push();
          this.fxClone({ name: act.name, kind: 'lib' }, '[' + (act.at || '') + '] on self');
          this._announceTallyA((act.st | 0));
          ...
          if(ansS.what === 'block'){
            ...
            this.chat('Trainbot: ' + v.name + ' now has +' + act.bonus + ' strength (base fist ' + (1 + this._strengthBonus(v)) + ').');
        ...
        if(act.kind === 'creep'){
          ...
          this.push();
          this.fxClone({ name: act.name, kind: 'lib' }, 'puts in play');
          this._announceTallyA((act.st | 0));
          ...
          await sleep(150);
          if(ansC.what === 'block'){
            this.chat('Blocked' + (ansC.who ? ' by ' + ansC.who : '') + '. Combat begins.');
            this._toAsh(act.name);
            this._unblockedStreak = 0; this._emit('blocked', { vamp: v.name, n: act.n, by: this._lastBlockerName || null });
            await this._resolveCombat(v);
          } else {
            ...
            this.board.push({ id: this.gid(), name: act.name, kind: 'lib', zone: 'ready',
                              faceDown: false, x: 64, y: 64, locked: false });
            this.push();
            this.chat('Trainbot: ' + act.name + ' is in play \u2014 the drip starts at my unlock.');
        ...
        if(act.kind === 'rush'){
          ...
          this.push();
          this.fxClone({ name: act.name, kind: 'lib' }, '[ANI] rushes ' + act.target.name);
          this._announceTallyA(0);
          ...
          if(ansR.what === 'block'){
            ...
            this.say('It resolves');
            this._toAsh(act.name);
            this.chat('Trainbot: ' + act.target.name + ' locks and is the acting minion this combat (Deep Song). Combat begins.');
        ...
        if(act.kind === 'equip'){
          ...
          this.fxClone({ name: act.name, kind: 'lib' }, eqVerb);
          ...
          await sleep(150);
          if(ansE.what === 'block'){
            this.chat('Blocked' + (ansE.who ? ' by ' + ansE.who : '') + '. Combat begins.');
            this._toAsh(act.name);
            this._unblockedStreak = 0; this._emit('blocked', { vamp: v.name, n: act.n, by: this._lastBlockerName || null });
            await this._resolveCombat(v);
          } else {
            ...
            this.chat('Trainbot: ' + v.name + ' now bears ' + act.name + '.');
            this._maybePerfectionist(v);
          }
          this._actingVamp = null;
          this._flushDraws();
        ...
        if(act.kind === 'fetch'){
          ...
          this.push();
          const supF = act.st >= 3;
          this.fxClone({ name: act.name, kind: 'lib' }, supF ? '[THA]' : '[tha]');
          this._announceTallyA((act.st | 0));
          ...
          await sleep(150);
          if(ansF.what === 'block'){
            this.chat('Blocked' + (ansF.who ? ' by ' + ansF.who : '') + '. Combat begins.');
            this._toAsh(act.name);
            this._unblockedStreak = 0; this._emit('blocked', { vamp: v.name, n: act.n, by: this._lastBlockerName || null });
            await this._resolveCombat(v);
          } else {
            ...
            this.chat('Trainbot: fetches ' + act.target.name + ' from the library and equips ' + v.name + ' (library shuffled \u2014 the fetched copy is off the top).');
            this._maybePerfectionist(v);
          }
          this._actingVamp = null;
          this._flushDraws();
        ...
        if(act.kind === 'peek'){
          ...
          this.push();
          this.fxClone({ name: act.name, kind: 'lib' }, act.supMode ? '[AUS]' : '[aus]');
          this._announceTallyA((act.st|0));
          ...
          await sleep(150);
          if(ansK.what === 'block'){
            this.chat('Blocked' + (ansK.who ? ' by ' + ansK.who : '') + '. Combat begins.');
            this._toAsh(act.name);
            ...
            if(act.supMode){
              ...
              } else this.chat("Trainbot: no reveal received \u2014 table's call; skipping the discard.");
        ...
        if(act.kind === 'pool'){
          ...
          this.push();
          this.fxClone({ name: act.name, kind: 'lib' }, act.n + ' pool damage');
          ...
          await sleep(150);
          if(ansP.what === 'block'){
            this.chat('Blocked' + (ansP.who ? ' by ' + ansP.who : '') + '. Combat begins.');
            ...
            this.say('It resolves');
            this.chat('Trainbot: the pool damage lands \u2014 ' + this.seatName(prey) + ', \u2212' + act.n + ' pool (adjust yours).');
        ...
        this.push();
        this.fxClone(act.name ? { name: act.name, kind: 'lib' } : v, 'bleeds for ' + act.n);   // the table SEES the play (clone animation)
        ...
        if(ans.what === 'block'){
          ...
          if(curAns.what === 'block'){
            this.chat('Blocked' + (curAns.who ? ' by ' + curAns.who : '') + '. Combat begins.');
            ...
            this.say('It resolves');
            this.chat('The bleed goes through: ' + this.seatName(this._actingVamp.target) + ', -' + this._actingVamp.n + ' pool (adjust yours).');
            this.push();
      ...
      /* INFLUENCE — real opening-transfer stagger: your first influence
         phase grants min(available, your seat) transfers; every later turn
         the FULL available amount (v0.5+: phaseActs.influence + any bonus,
         not a hardcoded 4 — a future in-play `actGrant` card, e.g. a
         curated Information Highway, raises the PERMANENT baseline via
         _recomputePhaseActs and this reads it generically). */
      await this._announcePhase('Influence');
      const availInf = this._phaseAvail('influence');
      this.inf += (this.ownTurns === 1) ? Math.min(availInf, this.seat) : availInf;
      this.phaseUsed.influence += (this.ownTurns === 1) ? Math.min(availInf, this.seat) : availInf;
      ...
      if(!this.out){
        this.push();
        this.send({ t: 'log', html: '<b>Turn passed.</b>' });   // v0.5: parity with a human's own Pass button (pass() → log(), unconditional — Pass itself isn't gated behind helpers being on, unlike the five phase buttons)
        this.chat('Trainbot passes.');
        this.send({ t: 'pass' });
      }
    } finally {
      this._acting = false;
...
const BOT_EXPORTS = { Bot, wsConnect, wsFrame, parseArgs, DEFAULT_DECK, WALL_DECK, SEWER_DECK, ARCHETYPES, BOT_VERSION, WIRE_V,

...
const BOT_VERSION = '0.6.108'; // v0.6.108 (12 Sep 2026): INFLUENCE -- transfers are now a FRESH per-turn grant (this.inf = , not +=) with the rulebook's table-wide stagger (tableTurn = (ownTurns-1)*nPlayers + seat, capped at 4), blood is invested INCREMENTALLY one transfer at a time and the vampire rises the moment it reaches capacity, and _planInfluence simulates fill-orderings over the known transfer schedule scoring tempo / wasted-transfers (crypt draws counted as pipeline, not waste) / hand synergy. MULTI-BOT UX -- only the lowest-seated bot greets or answers a bare help/rush, all chat carries the persona name with a (bot) suffix (the combat handshake prefix stays bare so the \\S+ parsers match), and the damage grammar is scoped: a bare "bleed N" reaches only the sender's prey, "@Name bleed N" aims. THE UNIFIED HOLD RULE (client handoff items 1/3/4) -- _holdHook is tracking only, the announcement and reminder moved to _pace's wait gate so the bot that is actually BLOCKED announces (exact anti-flood, and a hold placed while no bot is acting is no longer silent forever), a release from any non-bot clears the whole name-keyed set, and the ask-window pause finally applies the bot filter that _holdHook always had -- the measured 2-bot round-3 deadlock. Also: a turn arriving while _acting is queued and replayed with a staleness guard (handoff item 2), say() creates a pacing footprint so the 3 s bubble plays out, and a 30 s turn watchdog heartbeats only when the wait is the bot's own. Previous notes: elysium-bot-CHANGELOG.md.
...
class Bot {
  ...
  chat(msg){ this.send({ t: 'chat', msg: String(msg).replace(/Trainbot combat/g, this.o.name + ' combat').replace(/Trainbot/g, this.o.name + ' (bot)').slice(0, 300) }); if(this._wdT) this._watchdogArm(); }   // v0.6.108: persona name + (bot) suffix; re-arm watchdog on visible output
    ...
    return {
      ...
      dealt: m => {
        ...
        if(this._isSpokesBot()){
          ...
          this.chat('Trainbot v' + BOT_VERSION + (we ? ' (\u00d7' + nBots + ')' : '') + ' is seated. ' + (we ? 'We answer' : 'I answer') + ' to: "bleed N", "vote N", and "block/no" when ' + (we ? 'we ask' : 'I ask') + '. Be gentle. 🦇');
          if(we)   // v0.6.108: with several bots seated, a bare "bleed N" only reaches the sender's prey — teach the scoped form up front
            this.chat('With several bots seated, a bare "bleed N" goes to your prey \u2014 say "@Name bleed N" to aim at a specific one.');
          if(this.caps.classic)
            this.chat('Tip: ' + (we ? 'Bots play' : 'I play') + ' best on a Structured table with the resolvers on \u2014 but ' + (we ? 'we' : 'I') + ' will follow along here.');
        }
  ...
  async _influenceLoop(){   // v0.6.37 (Johan's ask, Wider View -- the last Inside Dirt card): extracted verbatim from _playTurn's own influence-phase while-loop, specifically so it's directly testable on its own (matching the same reasoning that pulled _revertHandSizeBonus out of the discard-phase block) -- no behavioral change to the pre-existing raise/draw logic, just a name and a call site. Wider View's own two abilities (a 1-transfer duplicate-swap, a 4-transfer burn-for-2-pool) are new branches inside this same loop, since transfers (this.inf) were already tracked here and both abilities are plain transfer-spending options, NOT lock-gated like Dreams of the Sphinx.
    ...
    while(!this.out){
      ...
      cheapest.faceDown = false;                        // blood is being placed — the vampire is now visible in uncontrolled
      if(cheapest.blood >= cheapest.cap){
        /* RISE — blood reached capacity, move to ready */
        cheapest.zone = 'ready'; cheapest.locked = false;
        ...
        this.chat('Influence: ' + cheapest.name + ' rises (cap ' + cheapest.cap + (preStocked > 0 ? ', ' + preStocked + ' pre-stocked' : '') + ', pool ' + this.pool + ').');
      } else {
        /* PARTIAL — blood placed but vampire stays in uncontrolled */
        this.push();
        this.chat('Influence: Trainbot places ' + canSpend + ' blood on ' + cheapest.name + ' (' + cheapest.blood + '/' + cheapest.cap + ', pool ' + this.pool + ').');
      }
    }
  }
  ...
  _planInfluence(cands){
    ...
    const poolFloor = zeroReady ? 1 : (this.o.poolFloor || 0);
    ...
    for(const ord of orderings){
      ...
      const score = this._scorePlan(plan, schedule, synScore);
      if(!best || score > best.score){
        best = { ordering: ord, plan, score, synScore,
                 parts: { firstRise: plan.rises[0] ? plan.rises[0].turn : 99,
                          ...
                          waste: plan.waste, draws: plan.draws, synScore: +synScore.toFixed(2) } };
      }
    }
    if(this._infPickTurn !== this.ownTurns){
      this._infPickTurn = this.ownTurns;
  ...
  _onChat(who, msg){
    ...
    if(dm){
      ...
      if(this.out) return this.chat(this.o.name + ' is already ousted \u2014 the dead bleed no more.');
      const kind = dm[1].toLowerCase(), n = Math.min(30, parseInt(dm[2], 10) || 0);
      if(!n) return;
      this.pool -= n;
      if(this.pool > 0){
        this.push();
        this.chat(this.o.name + ' takes the ' + kind + ' for ' + n + '. Pool: ' + this.pool + '.');
      } else this._oust(false);
      return;
    }
    ...
    if(/^rush\b/i.test(bodyMatch) && (namedMe || this._isSpokesBot()))
      return this.chat(this.o.name + ' has no combat module yet (M2). Treat my vampires kindly \u2014 or the host can referee.');

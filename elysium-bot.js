...
const BOT_VERSION = '0.6.112'; // v0.6.112 (15 Sep 2026, JOHAN'S FEEDBACK ROUND -- R4, DEBUG MODE): the item the 14 Sep handoff omitted entirely. o.debug opens my hand to the whole table (openHand to:'all'), turns my uncontrolled CRYPT face up (crypt only -- a face-down library permanent is not mine to expose), and writes the decision weighting to the log; o.debugWeights mutes that last half on its own. OFF BY DEFAULT and never for a tournament: it hands every opponent perfect information about me. Armed on the DEAL, not at construction -- openHand is started-gated, so an earlier grant is dropped silently -- and ANNOUNCED in chat, because a table that can see my hand must know it can. THE LIVE VIEW: the server stores nothing and only routes the granter's card list, so a grant is a snapshot unless the granter re-pushes; the hand has 34 mutation sites and hooking each is how you miss the 35th, so _syncOpenHand runs from the ONE board-push flush every one of them already ends in -- the same architecture the client uses (refreshOpenHands off schedulePush) -- behind a signature guard so an unchanged hand never reaches the wire. The weighting hangs off decide(), already the single funnel every judgement passes, and is STATUS tier, never speech. Suite +20 (section 87). Previous notes: elysium-bot-CHANGELOG.md.
...
class Bot {
    ...
    return {
      ...
      dealt: m => {
        ...
        if(this.o.debug && !this._debugHand){
          ...
          this.chat('Trainbot is in DEBUG mode: open hand, crypt face up' + (this.o.debugWeights !== false ? ', decision weighting in the log' : '') + '.');
        }
        ...
        if(this._isSpokesBot()){
          ...
          this.chat('Trainbot' + (we ? ' (\u00d7' + nBots + ')' : '') + ' is seated. ' + (we ? 'We answer' : 'I answer') + ' to: "bleed N", "vote N", and "block/no" when ' + (we ? 'we ask' : 'I ask') + '. Be gentle. 🦇');
  ...
  async _influenceLoop(){   // v0.6.37 (Johan's ask, Wider View -- the last Inside Dirt card): extracted verbatim from _playTurn's own influence-phase while-loop, specifically so it's directly testable on its own (matching the same reasoning that pulled _revertHandSizeBonus out of the discard-phase block) -- no behavioral change to the pre-existing raise/draw logic, just a name and a call site. Wider View's own two abilities (a 1-transfer duplicate-swap, a 4-transfer burn-for-2-pool) are new branches inside this same loop, since transfers (this.inf) were already tracked here and both abilities are plain transfer-spending options, NOT lock-gated like Dreams of the Sphinx.
    ...
    while(!this.out){
      ...
      if(cheapest.blood >= cheapest.cap){
        ...
        this.push();
        this.chat('Influence: Trainbot puts ' + canSpend + ' blood on an uncontrolled vampire (pool ' + this.pool + ').');   // v0.6.109 (#3a, Johan): a PARTIAL fill leaks nothing any more -- the card is still face down in my uncontrolled region, so naming it (and its cap, and its progress) handed the table a free read on what is coming. The RISE line below keeps the name deliberately: the moment it rises the card is public on the felt anyway (fxClone reveal:true), so hiding it there would be theatre, not secrecy.
      }
    }
  }
  ...
  _onVoteClose(){ this._votedThisRef = false; this._castThisRef = false; this._refVotes = null; }   // P0: the once-per-referendum guard re-arms on the mode-exit edge; v0.6.111 (R3): so do the title-vote guard and the running count
  /* v0.6.111 (R3, #10) ------------------------------------------------------
  ...
  _castTitleVotes(m){
    ...
    this.send({ t: 'tally', mode: 'vote', b: ((m && m.b) | 0) + tv.n });   // absolute against-total, the P0 convention; `a` rides srv-2.6.17's absent-field preserve
    ...
    this.chat('Trainbot: ' + tv.n + ' vote' + (tv.n === 1 ? '' : 's') + ' against \u2014 this costs me ' + stake + ' pool if it passes.');
  ...
  _onLog(who, html){
    ...
    if(who !== this.o.name && (m = t.match(L12.actObs))){
      ...
      if(oseat && oseat !== this.seat) this._obs = { who, seat: oseat, vamp: m[1], verb: m[2], card: m[4] || null, line: t, at: Date.now(), poolLoss: null };
    }
  ...
  _openAskedWindow(){
    ...
    if(!(o.poolLoss > 0)){ this.say('No block'); return; }
    ...
    this.pending = { who: o.who, amount: o.poolLoss, kind: 'asked', sub: 'asked', T: 'T1', role: 'target', actorSeat: o.seat, actorUnlocked,
                     actingVampName: o.vamp, contested: false, reacted: new Set(), blockerVamp: null, declined: false, attempted: new Set(), waitingFor: null };
    this.o.log('asked window: ' + o.verb + ' by ' + o.who + ' (' + o.poolLoss + ' pool if it resolves)');
    this._reactWindow();
  }

...
const BOT_VERSION = '0.6.115'; // v0.6.115 (16 Sep 2026, SELF-REVIEW OF THE STOPWATCH): two fixes in _armReactTimer/_restartAskTimer. (1) A Hold on... pause nulls the ask's timer AND deadline; the v0.6.114 ring re-stamp read `!a.deadline` as "no deadline yet" and armed a clock on a window a human was holding -- the window closed ~7 s later despite the hold. The ring never re-stamps a paused window. (2) The stopwatch was armed only when a window OPENED (3 sites) while _restartAskTimer runs at every table activity (15 sites) and resets to the full askSecs -- so one reaction played after the ring brought back the 20 s of dead air v0.6.114 removed. Silence restarts on activity, so every restart re-arms the stopwatch: a window now ends <= reactClockMs + reactSecs + grace after the LAST activity. Section 89 +3 assertions. Previous notes: elysium-bot-CHANGELOG.md.
...
class Bot {
  ...
  async _influenceLoop(){   // v0.6.37 (Johan's ask, Wider View -- the last Inside Dirt card): extracted verbatim from _playTurn's own influence-phase while-loop, specifically so it's directly testable on its own (matching the same reasoning that pulled _revertHandSizeBonus out of the discard-phase block) -- no behavioral change to the pre-existing raise/draw logic, just a name and a call site. Wider View's own two abilities (a 1-transfer duplicate-swap, a 4-transfer burn-for-2-pool) are new branches inside this same loop, since transfers (this.inf) were already tracked here and both abilities are plain transfer-spending options, NOT lock-gated like Dreams of the Sphinx.
    ...
    while(!this.out){
      ...
      if(!cheapest && this.cryptN > 0 && this.inf >= 4 && this.pool - 1 >= floorEff){
        ...
        this.push();
        this.note('Crypt: Trainbot fetches a new vampire (4 transfers, 1 pool — pool ' + this.pool + ').');
      ...
      if(cheapest.blood >= cheapest.cap){
        ...
        this.note('Influence: ' + cheapest.name + ' rises (cap ' + cheapest.cap + (preStocked > 0 ? ', ' + preStocked + ' pre-stocked' : '') + ', pool ' + this.pool + ').');
      } else {
        /* PARTIAL — blood placed but vampire stays in uncontrolled */
        this.push();
        this.note('Influence: Trainbot puts ' + canSpend + ' blood on an uncontrolled vampire (pool ' + this.pool + ').');   // v0.6.109 (#3a, Johan): a PARTIAL fill leaks nothing any more -- the card is still face down in my uncontrolled region, so naming it (and its cap, and its progress) handed the table a free read on what is coming. The RISE line below keeps the name deliberately: the moment it rises the card is public on the felt anyway (fxClone reveal:true), so hiding it there would be theatre, not secrecy.
      }
    }
  }
  ...
  async _onVoteOpen(m){              // P0 (the politics engine's FIRST brick): the polling window -- District's defensive votes land in the SHARED against-counter. v0.6.67: async for the wake pacing beat
    ...
    if(!vr){ this.note('Trainbot abstains.'); return; }
    ...
    if(needsWakeV && !wkV){ this.note('Trainbot abstains.'); return; }
    if(wkV){
    ...
    if(j < 0){ this.note('Trainbot abstains.'); return; }
    ...
    this.note('Trainbot: ' + vr.vamp.name + ' votes ' + vr.n + ' against (Protected District).');
  ...
  _castTitleVotes(m){
    ...
    this.note('Trainbot: ' + tv.n + ' vote' + (tv.n === 1 ? '' : 's') + ' against \u2014 this costs me ' + stake + ' pool if it passes.');
  ...
  _applyPending(via){
    ...
    if(p.kind === 'pool'){
      ...
      if(this.pool > 0){
        this.push();
        this.note('Trainbot takes the ' + p.amount + ' pool damage. Pool: ' + this.pool + '.');
    ...
    if(this.pool > 0){
      this.push();
      this.note('Trainbot takes the bleed for ' + p.amount + '. Pool: ' + this.pool + '.');
    } else this._oust(false);
  }
  ...
  _onLog(who, html){
    ...
    if((m = t.match(L12.lockM))){
      ...
      if(m[2] === this.seatName(this.seat) && !this.out){
        ...
        if(vc && !vc.locked){ vc.locked = true; this.push(); this.note('Trainbot: ' + vc.name + ' locks (Misdirection).'); this._emit('locked-by-misdirection', { vamp: vc.name }); }
      }
      return;
    }
  ...
  _onChat(who, msg){
    ...
    if(dm){
      ...
      if(this.pool > 0){
        this.push();
        this.note(this.o.name + ' takes the ' + kind + ' for ' + n + '. Pool: ' + this.pool + '.');
      } else this._oust(false);
      return;
    }
  ...
  _oust(byHost){
    ...
    this.push();
    this.note('Trainbot is ousted' + (byHost ? ' by the host' : '') + '. The shadows reclaim their own. 🦇');
  ...
  async _playTurn(){
    ...
    try{
      this.ownTurns++;
      await pace();
      this.note('— Trainbot, turn ' + this.ownTurns + ' —');
      ...
      await this._considerHandCycle('master');   // v0.6.42: The Barrens -- not phase-restricted by the card itself, master is this bot's own sensible-timing pick
      {
        ...
        while(this._phaseAvail('master') > 0 && !this.out){
          ...
          this.note('Trainbot: ' + d.pick.name + (host ? ' on ' + host.name : '') + (d.pick.curated ? '' : ' (uncurated \u2014 adjust if needed)') + '.');
          this.push();

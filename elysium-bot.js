...
const BOT_VERSION = '0.6.117'; // v0.6.117 (16 Sep 2026, THE SPOKEN PHASES -- Johan's finlir round after the v0.6.114 noise cut): (1) the bot's own DUPLICATES of what the table already shows are gone -- `Trainbot passes.` after `Turn passed.`, the `-- Trainbot, turn N --` banner under the client's TURN N header, the master-play note after the frozen play line + fx (it survives only as the uncurated warning), the per-batch `Influence:` notes, the District/title-vote notes after their frozen lines; online every one of them also sat beside the server's ledger rows (ledgerLive, srv v2.5.66 -- NOT a helper, unconditional). (2) what a human says ALOUD is one chat bubble in table words: the influence phase ends with ONE sentence (`I influence 3 blood to Nassir, who enters play, and 1 blood to an uncontrolled vampire.` / `... and use 4 transfers to draw a new vampire.`) via _influenceLoop's collector + _influenceSentence; votes say `I vote N against.`; a referendum with nothing to say gets `No votes from me.` once (_sayAbstain, reset at _onVoteClose); a landed bleed/pool hit is acknowledged `I take N. Pool: X.` (the three sites). Phase lines and hog-2 notes (the WHY the ledger cannot show: Jack ticks, does-not-unlock, Misdirection...) are unchanged. (3) cardfx compile v1.6.14 (THE INHERITED COST): the curated overlay dropped the data's price on 27 entries -- 23 hunting grounds (pool 2), Papillon (3), Meditative Grove (2), Revelations + Magic of the Smith (blood 1); they inherit it now, and the two newly-priced actions gate + pay it here (_bestPeekFor / _bestFetchFor carry it, the branches pay at announce -- the v0.6.116 pattern). Section 91 (27 assertions); the retargeted fixtures: the turn-banner wait -> Phase: Unlock, two Influence: waits -> the chat sentence, three takes-the-bleed acks -> chat, the crypt-fetch note -> the sentence + a board push.
...
class Bot {
  ...
  _sayAbstain(){   // v0.6.117: a referendum the bot has nothing to say against -- one bubble per referendum, the way a human says "no votes from me" (was a `Trainbot abstains.` note, invisible at the table)
    if(this._abstainSaid) return;
    this._abstainSaid = true;
    this.chat('No votes from me.');
  }
  _onVoteClose(){ this._votedThisRef = false; this._castThisRef = false; this._refVotes = null; this._abstainSaid = false; }   // P0: the once-per-referendum guard re-arms on the mode-exit edge; v0.6.111 (R3): so do the title-vote guard and the running count
  /* v0.6.111 (R3, #10) ------------------------------------------------------
    ...
    switch(q){
      ...
      case 'action': {              // plain bleed 1 vs a bleed-action card vs a pool-damage action (fas B) from hand
        ...
        if(ctx.card){
          ...
          const sCard  = ctx.card.n * P.aggression - 0.35 + (ctx.card.st | 0) * 0.2 - cardB * 0.1 - lowB - (this._lastPickName === ctx.card.name ? 0.15 * (this._pickStreak | 0) : 0);   // v0.6.79: repeat-pick decay -- the freeze antidote   // a card in hand has value — small spend cost; fas A (16 July): built-in stealth is WORTH something (each point lowers block odds -- Night Moves at bleed 1 was scoring 0.65 vs plain 1.0 and never got played, its +3/+6 stealth weighed at zero); 0.2/point is the deliberately dumb v1 weight
          if(sCard > sPick){ pick = ctx.card; sPick = sCard; why = ctx.card.name + ': ' + sCard.toFixed(2) + (cardB ? ' (blood cost ' + cardB + (lowB ? ', leaves ' + ctx.card.bloodAfter : '') + ')' : '') + ' vs best-so-far'; }
        }
    ...
    try{
      ...
      for(let _ai = 0; _ai < actors.length; _ai++){ const v = actors[_ai];   // v0.6.64: index form over the live queue (was a for..of SNAPSHOT -- the refreshed vampire's extra action was an illusion)
        ...
        if(act.kind === 'rush'){
          ...
          if(act.cost && act.cost.blood) v.blood = Math.max(0, (v.blood | 0) - act.cost.blood);   // v0.6.116 round 2: the finder (_bestRushFor) already gated on cost.blood and carried it, but the branch never paid it -- latent (no compiled rush card costs blood today), closed for symmetry with the bleed/stock/pool branches
          this.push();
        ...
        if(act.kind === 'fetch'){
          ...
          if(act.costBlood) v.blood = Math.max(0, (v.blood | 0) - act.costBlood);   // v0.6.117: the card's own blood cost, paid at announce (the bleed branch's v0.6.116 precedent)
          this.push();
          const supF = act.st >= 3;
        ...
        if(act.kind === 'peek'){
          ...
          if(act.cost && act.cost.blood) v.blood = Math.max(0, (v.blood | 0) - act.cost.blood);   // v0.6.117: Revelations' 1 blood, paid at announce
          this.push();
        ...
        if(act.kind === 'pool'){
          ...
          if(act.cost && act.cost.blood) v.blood = Math.max(0, (v.blood | 0) - act.cost.blood);   // v0.6.116: blood cost at announce (latent for Inside Dirt, which costs the Edge, not blood)
          this.push();

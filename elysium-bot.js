...
const BOT_VERSION = '0.6.162'; // v0.6.162 (23 Sep 2026, RULES OUT OF THE KNOB -- Johan's pre-spec code review): `combatMind` gated RULES corrections along with Johan's decisions, so the documented rollback (`combatMind: 0`) would have re-introduced rule bugs. Now unconditional: hand strikes deal 0 / are not offered at long range, a silent opponent at long deals 0 unless a ranged option is shown, a continued round waits two strike windows (the race), the victim of a steal loses the blood, Hidden Strength's prevent-bundled press is never a standalone press (it is played in damage resolution, not the press step). `combatMind` now gates DECISIONS only (Grapple gating, bundled-press use, press worth / press-to-end, range posture, Taste threshold, stealth order, rush kit, seize the moment, chase press). v0.6.161 (22 Sep 2026, THE CHASE PRESS -- Johan, from Black Chantry's Nosferatu guide): a chase-posture deck whose opponent maneuvered to long presses to continue (`_wantPress` tag 'chase-wave', any available press -- for the V5 Nosferatu the Underbridge Stray's life), provided it survives the opponent's shown return; the opponent must spend another maneuver or fight at close. combatMind. v0.6.160 (22 Sep 2026, THE VICTIM OF A STEAL -- Johan asked how steals are counted): the victim side of Steal Blood was never built -- since v0.6.19 a Theft of Vitae against a BOT gained the stealer blood while the victim read the frozen 'for 0' and lost nothing. `_parseCombatNarration` reads the stealer's note ('X strikes with CARD: steals N blood') from my combat opponent and takes min(N, blood) from `_combatMyVamp` (not damage -- no torpor from it alone); the combat-round row carries `stole` (mine) and `stolenFromMe`. combatMind. v0.6.159 (22 Sep 2026, THE CONTINUED-ROUND RACE -- found reading serie-cmb4's unmatched rounds): in a round after a press the non-presser starts at once while the presser still waits out its own press window, so a strike wait of one window expired exactly as the presser's strike arrived -- a phantom opp-silent round on one side and a lone extra round on the other (cmb4: Ventrue's Hidden Strength bundled press vs Malkavian, twice). `_strikeWaitMs(continued)`: round 2+ waits two windows (combatMind). v0.6.158 (22 Sep 2026, SEIZE THE MOMENT -- Johan: a torpor-certain rush trumps permanents-first when it is likely to go through): in decide('action') a rush whose damage exceeds the target's blood in one round, or over two with Immortal Grapple behind a damage source, and whose target's controller has no OTHER unlocked ready vampire or ally (`_rushLikelyThrough`), is picked at 97 -- above the permFirst gate (now `sPick < 97`), below the 99+ structural picks. Never with a rusher at 0 blood. Behind combatMind. serie-cmb3: permFirst decided 7 of 13 rush situations, incl. a Grapple + Roundhouse rush on a 1-blood target that lost to laying out Raven Spy. Previous: v0.6.157 THE RUSH KIT + THE SILENT LONG, v0.6.156 THE COMBAT MIND, v0.6.155 THE RUSH BY CHAT -- full notes in elysium-bot-CHANGELOG.md (82c: no line over 4 kB).
// (v0.6.152 note moved to elysium-bot-CHANGELOG.md)
// (v0.6.151 note moved to elysium-bot-CHANGELOG.md)
...
const DECK_STRATEGY = {   // A2 (v0.6.73): the reviewer's yardstick -- curated intent per archetype, emitted in the manifest so play is judged AGAINST strategy, not in a vacuum
  hacking:   { strategy: 'Ashur-cycling swarm: cap-1/2 crypt floods the table fast, Leverage-class bleed pressure every turn, Ashur Tablets recycle the engine; win by tempo before the table organizes.', axes: ['swarm-bleed', 'cycling', 'tempo'], combat: 'wave' },
  wall:      { strategy: 'Intercept fortress: block-first posture, equipment intercept (Bowl/Bike class), punish attempted actions and grind; win late by outlasting.', axes: ['wall', 'intercept', 'attrition'], combat: 'snipe' },
  sewer:     { strategy: 'Weenie stealth-bleed: obfuscate swarm slips small bleeds through every turn; minimal combat, race the clock.', axes: ['stealth-bleed', 'swarm', 'race'], combat: 'wave' },
  malkavian: { strategy: 'Stealth-bleed with intel: dem/obf bleed engine, Kindred Spirits class chip damage, Revelations-style hand reads; win by steady pressure and information.', axes: ['stealth-bleed', 'intel', 'pressure'], combat: 'wave' },
  tremere:   { strategy: 'Bounce-wall with THA reach: aus intercept + Deflection-class bounce redirects pressure, Theft tempo swings blood; win by deflected attrition.', axes: ['wall', 'bounce', 'blood-tempo'], combat: 'snipe' },
  ventrue:   { strategy: 'Vote-and-bleed toolbox: princes call Kine Resources Contested / Parity Shift on their own title votes, dom/pre bleeds (Enchant Kindred, Conditioning) between referendums, Deflection + Second Tradition at home; Freak Drive acts twice; win by pool damage the prey cannot block or outvote.', axes: ['vote', 'bleed', 'bounce'], combat: 'wave' },
  nosferatu: { strategy: 'Menace attrition: Fame/Creeping pool pressure + [ANI] rush control keeps threats in torpor; win by bleed+drain, not combat volume.', axes: ['menace', 'rush-control', 'stealth-bleed'], permFirst: true, combat: 'chase' }   // v0.6.156: combat = range posture (chase: never leave close, Nosferatu's fists · snipe: long with a ranged strike/steal in hand, Tremere's Theft · wave: long defensively with no strike card, the bleeders)   // v0.6.132: the helpsheet's early game -- permanents before pressure
...
function nameRe0(bot){ return new RegExp('^\\s*@?' + bot.o.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '[,:!]?\\s*', 'i'); }   // v0.6.152: the '@Name ' prefix, shared by the intercept-by-chat grammar and the damage grammar's own nameRe
function namedMe0(bot, txt){ return nameRe0(bot).test(txt); }
const BEAT_VERBS = new Set(['say', 'log', 'chat', 'tally', 'fx']);   // v0.6.152: the wire verbs that count as a visible table event for the beat (boards excluded: a human dragging cards must not hold the bot indefinitely)
...
const L12 = {
  ...
  aimAtVamp: /^(.+?)(?: plays (.+?)|'s card action is used)\. Target: (.+?)'s (.+?)\.$/   // v0.6.155: the CLIENT's play / card-action line with its Target suffix on a VAMPIRE (a pool target reads "X's pool" and never matches m[5] to a ready vampire). Not a frozen bot line; the canary does not pin it
};
...
class Bot {
  constructor(opts){
    this.o = Object.assign({
      ...
      paceMs: 1500,            // thinking pause between announces (0 in tests) — human tables need the beats to breathe (Johan, live test). v0.6.152 (Johan, 22 Sep): 2000 -> 1500, and the beat is now measured from the LAST TABLE EVENT (mine or anybody's), see _pace
      ...
      reactPaceMs: null,       // v0.6.109 (#13, the OTHER half): how long I sit on a reaction of my own once the table has declared its intention. null = paceMs (v0.6.152, Johan: one beat everywhere; was paceMs * 0.6 -- 0.9 s of a 1.5 s beat read as 'before' the human's act rather than 'after'). Deliberately its own knob and not paceMs: Johan wants the bot's ANSWERS quicker without speeding up its turn, and the old value (max(paceMs, thinkMs)) made a persona's thinkMs able to lengthen it but never shorten it.
      poolFloor: 6,            // never influence below this pool
    combatMind: 1,           // v0.6.156 (Johan's combat rulings, 22 Sep 2026): 1 = the combat mind (Grapple gating, bundled presses, press worth, press-to-end, range posture, long-range fists deal 0); 0 = v0.6.155's thresholds
  ...
  async _pace(kind){                                   // v0.6.99 (THE BEAT MODEL, Johan's design locked 8 Aug): the flat gate becomes multidimensional -- no arg = 'step' (paceMs, every legacy site unchanged), 'chat' = chatPaceMs or paceMs*1.6 (a declaration bubble plays out before the next act), 'fx' = fxBeatMs or paceMs*0.4 (the event animation's moment). paceMs 0 zeroes EVERY kind, so the suite stays fast by construction.
    ...
    const ms = kind === 'chat' ? (this.o.chatPaceMs !== undefined ? this.o.chatPaceMs : Math.round(base * 1.6))
             ...
             : kind === 'react' ? (this.o.reactPaceMs != null ? this.o.reactPaceMs : base)   // v0.6.152: the block-duel cards (stealth, intercept, cancel-block) take the reaction beat before they land
             : base;
    ...
    const anchor = Math.max(this._lastActAt || 0, this._lastTableAt || 0);   // v0.6.152 (Johan, 22 Sep): the beat is measured from the LAST VISIBLE EVENT AT THE TABLE, not only my own -- a 'No block' arriving 6 s after my Block? used to find the beat long spent, so the modifier fired in the same instant as the answer. One anchor, so a long ask wait followed by an answer still gives exactly one breath, never two.
    const wait = Math.max(anchor + ms - Date.now(), chatHold);
  ...
  _beat(kind){ return this._pace(kind); }
  _afterBeat(kind, guard, fn){                          // v0.6.152: a SYNCHRONOUS handler (a shout, a tally) wants a visible act to follow -- after the beat, if the world still looks the same. paceMs 0 runs it at once (the suite and the arena are byte-for-byte unchanged). Errors are logged, never thrown into the dispatcher.
    if(!this.o.paceMs){ try{ if(guard()) fn(); }catch(e){ this.o.log('after-beat error:', e.message); } return; }
    this._pace(kind).then(() => { if(guard()) fn(); }).catch(e => this.o.log('after-beat error:', e.message));
  }              // v0.6.99: the locked design's vocabulary -- an alias today, the seam for closed-loop beats (HB2e) tomorrow
  ...
  _dispatch(m){
    ...
    if(BEAT_VERBS.has(m.t) && m.who && m.who !== this.o.name) this._lastTableAt = Date.now();   // v0.6.152 (Johan: 'the bot answers quick phrases and blocks the same instant'): a visible act by SOMEBODY ELSE re-anchors my beat -- _pace waits paceMs from the latest of my own act and the table's, so a human's 'No block' / Block! / tally is always followed by a breath before I move. Server ledger lines carry no who and never re-anchor (they echo MY acts).
  ...
  _mkHandlers(){
    return {
      ...
      tally: m => {
        ...
        if(m.mode === 'block' && this._ask && this._ask.contested && this._actingVamp && !this.out){ if(m.who && m.who !== this.o.name) this._restartAskTimer(); try { this._stealthSecondLook(); } catch(e){ this.o.log('second-look error:', e.message); } }   // v0.6.143c; v0.6.153: a foreign counter move is table activity -- the verdict clock restarts (v0.6.115's silence principle), so a bot blocker's intercept card landing after ITS beat is never overtaken
  ...
  _bestStealthFor(vamp, played, targetSeat){   // highest usable +stealth for this acting vampire: hand cards, PLUS (fas C, 16 July) board-attached Marked Path-class burns whose recorded target matches THIS action's target. v0.6.12 (18 July, Johan's standing question): TYPE-FILTERED to action modifiers -- 147 non-mod cards (ally/equip/master/retainer grants, combat stealth) carry mode fx.stealth and were offerable from hand as mid-action modifiers; the actStealth sweep closed the Action-card half of this class, the type filter closes the rest structurally
    ...
    this.hand.forEach((n, idx) => {
      ...
      (h.e.modes || []).forEach(mo => {
        ...
        const hasCombat = (h.e.t || []).includes('combat');   // v0.6.156 (Johan): Swallowed by the Night is stealth AND the deck's only defensive maneuver -- at equal stealth spend the pure stealth card first and keep the dual card for combat
        if(typeof st === 'number' && this._modeUsableBy(mo, vamp) && (!best || st > best.s || (st === best.s && this._combatMind() && best.combat && !hasCombat)))
          best = { idx, name: h.name, s: st, cost: cB ? { blood: cB } : null, combat: hasCombat };
      });
    });
    if(targetSeat){
  ...
  _emitManifest(){                   // A2 (v0.6.73): deck intent + effective knobs
    ...
    const kn = {}; for(const k of ['rushBackWeight','rushFwdWeight','crossRushWeight','poolFloor','paceMs','askSecs','polOverBuild','permFirst','fitWeight','critRisk','homeWeight','stealthStack','lateBounce','bounceSpare','combatMind']) if(this.o[k] !== undefined) kn[k] = this.o[k];
  ...
  _complyRush(p){                     // v0.6.107 (B1): the rushed vampire locks and fights the rusher (the pre-B1 comply, now on MY Pass)
    ...
    this.note('Trainbot: ' + vc.name + ' locks and defends (rushed by ' + p.actingVampName + ').');   // v0.6.155: the rusher's name -- Deep Song's line and a human's rush by chat share this path
  ...
  _rushExpectation(vamp){            // v0.6.74 (Johan's design): expected combat outcome -- the rush scorer's ground truth
    ...
    /* v0.6.157 (Johan, 22 Sep: "rush only with real damage in hand -- Crows + Grapple / Roundhouse; with the combat cards spent, bleed with Deep Song"):
       the KIT behind a rush. grapple = Immortal Grapple usable (denies the target's combat ends / ranged answer), strikeCard = a damage card of 2+,
       crows = Carrion Crows in hand or borne. reach = the damage the fight can land: with Grapple behind a damage source the superior's bundled press
       (or the Stray) buys the second round, so the reach doubles. */
    const grapple = !!this._bestImmortalGrappleFor2(vamp);
    const strikeCard = this.hand.some(nm => { const h = this.fxLookup(nm); return h && h.kind === 'lib' && (h.e.modes || []).some(mo => mo.fx && (mo.fx.strike | 0) >= 2 && this._modeUsableBy(mo, vamp)); });
    const crows = envBorne + envHand > 0;
    const dmgSource = strikeCard || crows || this._strengthBonus(vamp) > 0;
    const kit = (grapple ? 1 : 0) + (strikeCard ? 1 : 0) + (crows ? 1 : 0);
    const reach = (grapple && dmgSource) ? expDmg * 2 : expDmg;
    return { expDmg, nCombat, env: envBorne + envHand, kit, grapple, strikeCard, crows, dmgSource, reach };
  }
  ...
  _rushScore(vamp, c){               // v1.1: value x feasibility - chump + Fame bonus (the frozen baseline, rush-theory section 3)
    ...
    let dmg = ((this._bestStrikeFor(vamp) || {}).n || 1) + this._borneEnvFor(vamp) + this._strengthBonus(vamp);   // N4b: torpor-reach honesty
    if(this.o.combatMind !== 0){ const ex = this._rushScoreExp || (this._rushScoreExp = this._rushExpectation(vamp)); dmg = Math.max(dmg, ex.reach | 0); }   // v0.6.157: Crows in hand and the Grapple second round count toward who is in torpor reach (cached per targeting pass)
  ...
  _bestRushTargetFor(vamp){          // v1.1 (rush-theory section 3): the DIRECTION CHOOSER with tunable weights + the counterfactual log
    ...
    this._rushScoreExp = null;   // v0.6.157: fresh kit per targeting pass
  ...
  _bestPreventFor(vamp){             // a usable damage-prevention card for MY incoming hit (v1: assumes 1, the rulebook default)
    ...
    this.hand.forEach((n, idx) => {
      ...
      (h.e.modes || []).forEach(mo => {
        ...
        if(!best || val > best._val || (val === best._val && mo.fx.preventX && !best.x) || (val === best._val && mo.fx.press && !best.press && this._combatMind())) best = { idx, name: h.name, n: pv, _val: val, cost: h.e.cost || null, x: !!mo.fx.preventX, press: !!mo.fx.press };   // v0.6.142   // v0.6.156: press = the mode bundles an optional press (Hidden Strength [FOR]) -- consumed via _bundledPress: x = 'pay X blood for X more' (Hidden Strength)
      });
    });
    return best;
  }
  ...
  _bestPressFor(vamp){               // v0.6.22 (Johan's combat GO, the multi-round loop -- Press): the SIMPLEST press shape only (Apportation: standalone, no bundled strike/steal, no third-party target) -- source-verified 19 July that press is OPTIONAL by default convention (Talbot's Chainsaw is the documented mandatory exception, not the rule), so this finder's presence just means "I COULD press", the decide-shaped "do I WANT to" choice lives in the caller (_resolveCombat), same split as _bestBloodStockFor/grinderReadyTarget. Excludes modes that ALSO carry maneuver/strike/stealBlood (a bundled ability like Grasp of the Python or Slam's superior -- documented in the combat plan §5.6, not built) and the same four unverifiable-precondition req flags every other v2 finder already excludes.
    ...
    this.hand.forEach((n, idx) => {
      ...
      if(h.e.req && h.e.req.rangeLong && this._range !== 'long') return;
      (h.e.modes || []).forEach(mo => {
        if(!mo.fx || !mo.fx.press || mo.fx.maneuver || typeof mo.fx.strike === 'number' || mo.fx.stealBlood || mo.fx.restrictHandStrike || mo.fx.prevent !== undefined) return;   // v0.6.156: Hidden Strength [FOR] bundles its press WITH a prevent -- serie-cmb1 saw a Ventrue bot play it as a standalone press (a prevent card burned to continue a fight the deck wants no part of); the bundled press is consumed via _bundledPress when the card is played as a PREVENT   // v0.6.28 fix (found while verifying the half-finished Immortal Grapple work): Immortal Grapple's [POT] mode bundles press WITH restrictHandStrike -- without this exclusion, a press-decision step at end-of-round could mistake it for a plain press card and play it for that reason alone, silently also imposing the hand-strike restriction the caller never intended
  ...
  /* ==================== v0.6.156: THE COMBAT MIND (Johan's combat rulings, 22 Sep 2026) ====================
     REPORT-serie-cmb1.md measured v0.6.155's combat: Immortal Grapple with bare fists 8 of 17 times, superior
     Grapple's press never used, Tremere never maneuvering, presses into rounds fought with fists, Hidden Strength
     played as a press. These helpers hold Johan's rules; every consumer is gated on o.combatMind (0 = the old
     thresholds) so serie-cmb2 can be read against cmb1 on the same seeds.                                   */
  _combatMind(){ return this.o.combatMind !== 0; }
  _combatPosture(){                  // chase (Nosferatu: never leave close) · snipe (Tremere: long with a ranged strike/steal in hand) · wave (bleeders: long defensively, no strike card in hand)
    const ds = (typeof DECK_STRATEGY !== 'undefined' && DECK_STRATEGY[this.o.deckName]) || {};
    return ds.combat || 'wave';
  }
  _combatOpp(){                      // the table-model record of the vampire I am fighting: { seat, o (seat record), rec (vampire: blood, cap), name } -- null when the opponent is unknown (a human table before any board scan)
    const name = this._combatOppName || this._duelOppName(); if(!name) return null;
    const seat = this._seatOfVamp(name); if(!seat || seat === this.seat) return null;
    const o = this._opp(seat); return { seat, o, rec: (o.vamps || {})[name] || null, name };
  }
  _oppShown(cat){ const c = this._combatOpp(); return c ? ((c.o.seen || {})[cat] | 0) : 0; }   // combat cards this seat has shown (table model categories: dodge = dodge OR combat ends, strike, stealBlood, press, maneuver, aggr)
  _oppExpectedDmg(){                 // what this seat's minions have dealt me per combat, else the disciplines/weapons prior, else the rulebook fist (1)
    const c = this._combatOpp(); if(!c) return 1;
    if(c.o.combat && c.o.combat.n) return Math.max(1, c.o.combat.dmgSum / c.o.combat.n);
    const pr = this._oppPrior(c.seat); return pr && pr.dmgPerCombat ? pr.dmgPerCombat : 1;
  }
  _myRoundDmg(vamp, opts){           // the damage (or blood swing) my NEXT strike lands: best strike card > bare fist + strength; steal counts as its blood swing; active Carrion Crows add each round. opts.close = evaluate as if at close range (a bundled Grapple press forces close next round)
    const saveR = this._range; if(opts && opts.close) this._range = 'close';
    const strike = this._bestStrikeFor(vamp), steal = this._bestStealStrikeFor(vamp);
    this._range = saveR;
    const fist = (this._range === 'long' && !(opts && opts.close)) ? 0 : 1 + this._strengthBonus(vamp);
    let dmg = fist, card = null;
    if(strike && strike.n > dmg){ dmg = strike.n; card = strike.name; }
    if(steal && steal.n >= dmg && (vamp.blood | 0) < (vamp.cap | 0)){ dmg = steal.n; card = steal.name; }
    const crows = this._carrionCrows ? (this._carrionCrows.n | 0) : 0;
    return { dmg: dmg + crows, card, crows, fist: 1 + this._strengthBonus(vamp) };
  }
  _strikeWaitMs(continued){          // v0.6.159 (serie-cmb4, the phantom round 3): in a CONTINUED round the presser is still sitting in its own press window (askSecs, waiting to see whether I cancel) while I -- who only received the press -- start the next round at once; with equal windows my strike wait expired exactly as its strike arrived (Malkavian 'opp-silent' at -6402, Ventrue's strike at -6402). Round 2+ waits two windows.
    const base = (this.o.askSecs || 2) * 1000; return continued ? base * 2 : base;
  }
  _oppHasRanged(){                   // v0.6.157: does the combat opponent's seat show a RANGED option -- a strike-granting equipment on the table model, or a steal / ranged strike already seen? Unknown opponent -> false (the rulebook default is a hand strike)
    const c = this._combatOpp(); if(!c) return false;
    const eq = (c.rec && c.rec.equip) || [];
    if(eq.some(n => { const h = this.fxLookup(n); return h && h.kind === 'lib' && (h.e.modes || []).some(mo => mo.fx && typeof mo.fx.strike === 'number' && !mo.fx.handStrike); })) return true;
    return ((c.o.seen || {}).stealBlood | 0) > 0;
  }
  _wantGrapple(vamp){                // Johan: Grapple only with Roundhouse-class hand damage, active Carrion Crows or a permanent strength bonus -- never for a fist of 1. Too valuable to de-clog with.
    const r = this._myRoundDmg(vamp, { close: true });
    const bonus = this._strengthBonus(vamp) > 0;
    if(r.card && r.dmg >= 2) return { play: true, why: 'Grapple: ' + r.card + ' for ' + r.dmg + ' behind it' };
    if(this._carrionCrows) return { play: true, why: 'Grapple: Carrion Crows peck ' + this._carrionCrows.n + ' each round -- deny the combat ends' };
    if(bonus) return { play: true, why: 'Grapple: strength +' + this._strengthBonus(vamp) + ' fists' };
    return { play: false, why: 'Grapple held: only a fist of ' + r.fist + ' behind it' };
  }
  _wantPress(vamp, have){            // "do I want another round" -- have = a press is available (card or bundled). Returns { press, why, tag }.
    if(!have) return { press: false, tag: 'none', why: 'no press available' };
    const lt = this._oppLastTaken && Date.now() - this._oppLastTaken.at < 15000 && (!this._combatOppName || this._oppLastTaken.vamp === this._combatOppName) ? this._oppLastTaken : null;   // v0.6.156: their own taken line for THIS round, if it has arrived
    if(lt && lt.down) return { press: false, tag: 'opp-down', why: 'no press: ' + lt.vamp + ' is down (their own line)' };   // (cmb2 pre-read: torpor-shot presses went into an empty round)
    const c = this._combatOpp();
    let oppBlood = c && c.rec ? (c.rec.blood | 0) : null;
    if(lt && lt.now != null) oppBlood = lt.now;                                          // certain: their own line
    else if(oppBlood != null && this._rnd && (this._rnd.dealt | 0) > 0) oppBlood = oppBlood - (this._rnd.dealt | 0);   // the board may not have caught up with MY strike yet (cmb2 pre-read): assume it has not
    if(oppBlood != null && oppBlood < 0) return { press: false, tag: 'opp-down', why: 'no press: my ' + this._rnd.dealt + ' already exceeded their blood' };
    const mine = this._myRoundDmg(vamp, { close: this._forceCloseNextRound });
    const ret = this._oppExpectedDmg(); const blood = vamp.blood | 0;
    const survive = blood > ret;                                   // damage EXCEEDING blood torpors; paying exactly all of it does not
    const oppEnds = this._oppShown('dodge') > 0;                   // dodge or combat ends seen from this seat
    if(this._combatPosture() === 'chase' && this._rnd && this._rnd.range === 'long' && survive && this._combatMind()){   // v0.6.161 (Johan + Black Chantry's Nosferatu guide: "if an opposing minion maneuvers to long range, burn 1 life from an Underbridge Stray during the press step to get a press and continue combat"): a chase deck never goes long itself, so long = THEY waved; the press drags the fight back to a range where the opponent must spend another maneuver or eat the fists
      const fistClose = this._myRoundDmg(vamp, { close: true });
      return { press: true, tag: 'chase-wave', why: 'press: they waved to long -- back to close (' + (fistClose.card ? fistClose.card + ' ' : 'fists ') + fistClose.dmg + '), they must spend another maneuver' };
    }
    if(oppBlood != null && mine.dmg > oppBlood && survive) return { press: true, tag: 'torpor-shot', why: 'press: ' + mine.dmg + ' damage next round beats ' + oppBlood + ' blood (they torpor), I keep ' + (blood - Math.ceil(ret)) };
    const post = this._combatPosture();
    if(post === 'wave'){                                          // Ventrue/Malkavian: continue only for the torpor shot above, and only against a seat that is not a combat seat
      return { press: false, tag: 'wave-declines', why: 'press declined: ' + post + ' deck, no torpor shot' };
    }
    if(mine.card && (mine.dmg >= 2 || post === 'snipe') && survive && !(oppEnds && !mine.crows)) return { press: true, tag: 'card-round', why: 'press: ' + mine.card + ' for ' + mine.dmg + ' next round, I survive their ' + ret.toFixed(1) };   // snipe (Johan): Theft + press + Theft beats maneuver + Theft -- a steal of 1 is still a card round
    if(mine.crows && survive) return { press: true, tag: 'crows', why: 'press: Carrion Crows peck ' + mine.crows + ' more' };
    return { press: false, tag: 'fists', why: 'press declined: ' + (mine.card ? mine.card + ' but blood ' + blood + ' vs their ' + ret.toFixed(1) : 'only fists of ' + mine.fist + ' next round') };
  }
  _wantPressToEnd(vamp){             // the opponent pressed to continue and I hold a press: cancel it? Wave decks always; chase/snipe when the next round would cost me more than it earns
    const post = this._combatPosture(); if(post === 'wave') return { end: true, why: 'press to end: ' + post + ' deck wants no second round' };
    const w = this._wantPress(vamp, true); return { end: !w.press, why: w.press ? 'let their press stand (' + w.why + ')' : 'press to end: ' + w.why };
  }
  _rangedOptionFor(vamp){            // a strike or steal card in hand usable at LONG range for this vampire (not handStrike-tagged), ignoring the current range
    const saveR = this._range; this._range = 'long';
    const st = this._bestStrikeFor(vamp), sb = this._bestStealStrikeFor(vamp);
    this._range = saveR; return st || sb;
  }
  _wantLong(vamp){                   // the range step's question under the combat mind: go to long?
    const post = this._combatPosture();
    if(post === 'chase') return { long: false, why: 'chase: stay close' };
    if(post === 'snipe'){ const r = this._rangedOptionFor(vamp); return r ? { long: true, why: 'snipe: ' + r.name + ' reaches, their fists do not' } : { long: false, why: 'snipe: no ranged card in hand' }; }
    const saveR = this._range; this._range = 'close'; const close = this._bestStrikeFor(vamp) || this._bestStealStrikeFor(vamp); this._range = saveR;
    if(close) return { long: false, why: 'wave: ' + close.name + ' wants close' };
    const danger = this._combatOpp() ? this.oppCombat(this._combatOpp().seat) : 0.3;
    return { long: danger >= 0.2 || this._oppShown('strike') > 0, why: 'wave: no strike card, their danger ' + danger };
  }
  _rushLikelyThrough(target){        // v0.6.158 (Johan: a torpor-certain rush trumps permanents-first "if it is likely to go through -- not blocked by another vampire"): the target's controller has no OTHER unlocked ready vampire or ally (the rushed vampire itself blocking still gives me the fight)
    if(!target || target.seat == null) return false;
    const tb = this.table && this.table[target.seat]; const cards = (tb && tb.pub && tb.pub.cards) || [];
    const others = cards.filter(c => c && c.zone === 'ready' && !c.locked && c.name !== target.name &&
      (c.kind === 'crypt' || (() => { const h = this.fxLookup(c.name); return h && h.kind === 'lib' && ((h.e.t || []).includes('ally')); })())).length;   // other unlocked vampires AND allies -- intercept only matters on a body that can block, so no body = no block
    return others === 0;
  }
  _bestImmortalGrappleFor2(vamp){    // v0.6.157: the Grapple finder for PLANNING (a rush not yet fought) -- range-independent, a combat starts at close
    const saveR = this._range; this._range = 'close';
    try { return this._bestImmortalGrappleFor(vamp); } finally { this._range = saveR; }
  }
  ...
  _decide(q, ctx){
    ...
    switch(q){
      ...
      case 'react-delay':                              // v0.4.5 (Johan, live test): a reaction is still a bot command -- it gets the SAME paceMs floor as a turn action (was capped at thinkMs=1600, under the 2s ask); a persona's thinkMs can only make the pause LONGER, never shorter than a real table's pace
        ...
        return { ms: this.o.paceMs ? (this.o.reactPaceMs != null ? this.o.reactPaceMs : this.o.paceMs) : 40 };   // v0.6.152: default 1.0 x pace (was 0.6) -- Johan wants the same breathing room after every table event
      ...
      case 'action': {              // plain bleed 1 vs a bleed-action card vs a pool-damage action (fas B) from hand
        ...
        const rushSeize = (() => {   // v0.6.158 (Johan, 22 Sep: "combat is about seizing the moment when it presents itself"): a torpor-certain rush that is likely to go through beats permanents-first
          if(this.o.combatMind === 0 || !ctx.rush || !ctx.rush.exp || ctx.rush.exp.kit === undefined) return null;
          const ex = ctx.rush.exp, tbR = (ctx.rush.target.blood | 0);
          if(!(ex.expDmg > tbR || (ex.grapple && ex.dmgSource && ex.reach > tbR))) return null;   // torpor-certain: damage EXCEEDING their blood in one round, or over two with Grapple behind a damage source (its bundled press buys round 2)
          if((ctx.rush.myBlood | 0) <= 0) return null;                                              // a rusher at 0 goes down on the first fist
          if(!this._rushLikelyThrough(ctx.rush.target)) return null;
          return { why: 'reach ' + ex.reach + ' > ' + tbR + ' blood, kit ' + ex.kit + ', no other unlocked blocker at ' + ctx.rush.target.owner };
        })();
        if(rushSeize){ scores.rushSeize = rushSeize.why; if(sPick < 97){ pick = ctx.rush; sPick = 97; why = 'rush ' + ctx.rush.target.name + ' \u2014 seize the moment: ' + rushSeize.why; } }   // 97: above permFirst's 98-gate (which now requires sPick < 97) and every score, below the 99+ structural picks (own rescue while building, referendum)
        if(this.o.permFirst && sPick < 97 && this._deckPermFirst() && !(this.plan && (this.plan.posture === 'lunge' || this.plan.reads.preyTto <= 1.5))){
        ...
        if(ctx.rush){
          ...
          const mind = this.o.combatMind !== 0 && ex.kit !== undefined;   // v0.6.157: the kit (Johan's rush rule)
          const reachD = mind ? ex.reach : ex.expDmg;
          const mult = reachD >= tb ? 1.35 : (reachD >= tb - 1 ? 1.0 : 0.55);   // v0.6.74: Johan's rule -- no combat cards wants 0-1 blood targets   // v0.6.157: reach = two rounds when Grapple stands behind a damage source
          const kitW = !mind ? 1 : (ex.grapple && ex.dmgSource) ? 1.5 : (ex.kit >= 1 && ex.dmgSource) ? 1.15 : 0.75;   // v0.6.157: Grapple + Roundhouse / Crows = rush who you want; one damage card = a modest lift; no combat card = bleed with it instead
          ...
          const sR = (0.8 + 0.12 * (ctx.rush.target.w || 0)) * mult * riskW * P.aggression * dirW * kitW + Math.min(0.15, 0.05 * ex.nCombat); scores.rush = +(sR).toFixed(2); scores.rushParts = { tb, expDmg: ex.expDmg, reach: reachD, kit: ex.kit, kitW, mult, riskW };
          if(sR > sPick){ pick = ctx.rush; sPick = sR; why = 'rush ' + ctx.rush.target.name + ' (blood ' + ctx.rush.target.blood + ', expDmg ' + ex.expDmg + (mind && reachD !== ex.expDmg ? ' (reach ' + reachD + ' with Grapple)' : '') + ' vs ' + tb + ', combat cards ' + ex.nCombat + (mind ? ', kit x' + kitW : '') + '): ' + sR.toFixed(2); }   // v0.6.74: the calculation visible in every dossier
        }
        if(ctx.equip){
  ...
  async _resolveCombat(myVamp, oppName){      // v0.6.22 (Johan's combat GO, the multi-round loop): the OUTER wrapper _resolveCombatRound was always meant to grow into (§7.5-C's original design: "plays at most the rounds a press grants"). Deliberately a THIN wrapper, not an internal rewrite -- _resolveCombatRound stays exactly as tested (every existing 37a-37g test calls it directly and is untouched), this just repeats it while there's an uncancelled press. v1 scope (source-verified vekn.net 19 July, combat plan §5.5): "if there is an UNCANCELLED press to continue, a new round begins" -- only ONE side needs to press. My bot NEVER attempts to cancel an incoming press (that decision -- "is it worth spending my own press just to deny theirs" -- is a future addition, not built); it presses when IT wants to (a simple, deliberately-dumb v1 threshold, matching grinderReadyTarget's own precedent: not fragile), and separately just detects whether the opponent pressed at all, treating any detected opponent press as uncancelled. Silent opponent (timeout, human table) is NOT treated as "they definitely didn't press" vs "they definitely did" -- it's simply absent evidence, so it doesn't extend combat on its own; only a DETECTED press (mine or theirs) does.
    ...
    this._combatMyVamp = myVamp;   // v0.6.160: the victim side of a steal needs to know whose blood goes
    try{
    for(let round = 1; ; round++){
      const res = await this._resolveCombatRound(myVamp, round > 1);
      const rowEnd = why => this._emit('combat-round', Object.assign({ round, vamp: myVamp.name, opp: this._combatOppName || null, press: null, continuing: false, end: why, blood: myVamp.zone === 'ready' ? (myVamp.blood | 0) : 0, posture: this._combatPosture() }, this._rnd || {}));   // v0.6.156
      if(myVamp.zone === 'torpor' || myVamp.zone === 'burned'){ rowEnd(myVamp.zone); return; }   // combat already over from my own side
      if(res && res.ended){ rowEnd('i-ended'); return; }                            // v0.6.123: I ended the combat (Majesty)
      if(this._oppEndedCombat){ this._oppEndedCombat = false; rowEnd('opp-ended'); return; }   // v0.6.123: THEY did -- no press into a fight that is over
      if(res && res.opponentSilent){ rowEnd('opp-silent'); return; }                   // v0.6.23: a round-2+ opponent who never even answered the STRIKE handshake is the strongest signal this bot can get that there's no one left to press against -- stop offering more presses into the void rather than looping on phantom rounds
      const press = this._bestPressFor(myVamp);
      let continuing = false, pressTag = null;
      const mind = this._combatMind();
      const bundled = mind && !!this._bundledPress;                                     // v0.6.156: superior Grapple / Hidden Strength [FOR] played THIS round
      const strayCan = !press && !bundled && !!this._findStray();   // N4d: the Underbridge Stray can grant the press for a life
      if(mind && (press || bundled || strayCan) && !(this._oppLastTaken && Date.now() - this._oppLastTaken.at < 3000)) await this._awaitOppTaken(Math.min(1500, (this.o.askSecs || 2) * 1000));   // v0.6.156: let the opponent's taken line ('now M', torpor) arrive before judging a press -- a bot answers in ms, a human table just costs the short wait
      const want = mind ? this._wantPress(myVamp, !!press || bundled || !!strayCan) : { press: !!press && (myVamp.blood | 0) > 1, tag: 'v155', why: 'blood > 1' };   // v0.6.156: expected value replaces the blood > 1 threshold (cmb1: 6 of 14 Tremere presses were followed by a fist)
      this.o.log('press (' + myVamp.name + '): ' + want.why);
      if(want.press && bundled){                              // the free press first -- no card spent
        pressTag = 'bundled:' + want.tag; this._bundledPress = false;
        this.note('Trainbot combat \u2014 ' + myVamp.name + ' presses to continue (optional press).');
        continuing = true;
      } else if(want.press && press){
        pressTag = press.name + ':' + want.tag;
        ...
        this.note('Trainbot combat \u2014 ' + myVamp.name + ' presses to continue (' + press.name + ').');   // v0.6.156: frozen phrase (silent since v0.6.114 -- the opponent fought no round 2)
        this.push();
        continuing = true;
      } else if(want.press && (mind ? true : (myVamp.blood | 0) > 1) && this._strayPressBurn()){   // N4d consumer A: no press card -- the Stray burns a life for the press (v0.6.156: the same _wantPress judgment)
        pressTag = 'stray:' + want.tag;
        this.note('Trainbot combat \u2014 ' + myVamp.name + ' presses to continue (the Stray).');
        continuing = true;
      }
      const oppP = await this._awaitOppPress((this.o.askSecs || 2) * 1000);
      if(oppP && oppP.continue){
        if(!continuing && mind && (press || this._bundledPress) && this._wantPressToEnd(myVamp).end){   // v0.6.156 (Johan): press to END -- Ventrue's use of the press; a chase/snipe deck cancels a round it would lose
          const w = this._wantPressToEnd(myVamp); this.o.log('press (' + myVamp.name + '): ' + w.why);
          if(this._bundledPress){ this._bundledPress = false; pressTag = 'bundled:end'; }
          else { pressTag = press.name + ':end'; this.hand.splice(press.idx, 1); this._drawOwed = (this._drawOwed || 0) + 1; this._toAsh(press.name); this._emitPlay(press.name, 'combat-press-end', myVamp && myVamp.name); if(press.cost && press.cost.blood) myVamp.blood = Math.max(0, (myVamp.blood | 0) - press.cost.blood); this.fxClone({ name: press.name, kind: 'lib' }, 'presses to end', { actor: myVamp }); }
          this.note('Trainbot combat \u2014 ' + myVamp.name + ' presses to end: the press to continue is cancelled.');
          this.push();
          continuing = false;
        } else continuing = true;
      } else if(oppP && oppP.continue === false && continuing && mind){   // v0.6.156: THEIR press to end cancels MY press to continue (rulebook: presses cancel one another)
        this.o.log('press (' + myVamp.name + '): my press was cancelled by ' + oppP.vamp);
        continuing = false;
      }
      this._emit('combat-round', Object.assign({ round, vamp: myVamp.name, opp: this._combatOppName || null, press: pressTag, continuing, blood: myVamp.blood | 0, posture: this._combatPosture() }, this._rnd || {}));   // v0.6.156: the row serie-cmb1 had to infer (range, opponent, strike, dealt, taken)
  ...
  async _resolveCombatRound(myVamp, continued){ // v1 CUT LINE (spec §7.5-C): announce-only, one round, applies MY side; the opponent's strike isn't modelled -- assumes the rulebook default (hands, 1) unless I prevent it. Never steps the shared ph/rd counters. v0.6.23 (standing-question fix, the multi-round loop): `continued` is a NEW, backward-compatible optional 2nd arg -- every existing call (all of 37a-37g, all 6 real call sites for round 1) passes nothing, so this defaults falsy and behaves EXACTLY as before. `_resolveCombat` passes true for round 2+ ONLY.
    this._bundledPress = false; this._rnd = { grapple: false, strike: 'hands', dealt: null, taken: null };   // v0.6.156: per-round -- a bundled optional press (superior Grapple, Hidden Strength [FOR]) is usable only in the round its card was played (EC2010 ruling); _rnd feeds the combat-round trace row
    ...
    if(this._forceCloseNextRound){
      ...
      let myManeuvers = 0;
      if(maneuver){
        const wl = this._combatMind() ? this._wantLong(myVamp) : null;   // v0.6.156: deck range posture (chase / snipe / wave) -- serie-cmb1: Tremere never maneuvered in 133 rounds because only a req.rangeLong card counted
        if(wl) this.o.log('range (' + myVamp.name + '): ' + wl.why);
        const wantLong = (wl && wl.long) || this.hand.some(n => {
          const h = this.fxLookup(n); if(!h || h.kind !== 'lib') return false;
        ...
        if(!maneuver.fromEquip){ this.hand.splice(maneuver.idx, 1); this._drawOwed = (this._drawOwed || 0) + 1; } else { if(maneuver.fromGrant) this._reactManeuverGrant = false; else this._equipManeuverUsed = true; }   // E1/N1: board- and grant-maneuvers never splice; each consumes its OWN once-flag
          ...
          this.note('Trainbot combat \u2014 ' + myVamp.name + ' maneuvers to long (' + maneuver.name + ').');   // v0.6.156: the frozen phrase the opposing bot's _parseCombatNarration reads (silent since v0.6.114)
          this.push();
          myManeuvers = 1;
        }
      }
    ...
    if(grapple){
      ...
      const wg = this._combatMind() ? this._wantGrapple(myVamp) : { play: true, why: 'v0.6.155: play unless it costs me a non-hand option' };   // v0.6.156 (Johan): Grapple only with Roundhouse-class damage, active Crows or a strength bonus -- cmb1 wasted 8 of 17 on fists
      this.o.log('grapple (' + myVamp.name + '): ' + wg.why + (wouldCostMe ? ' -- but it would cost me a non-hand option' : ''));
      if(!wouldCostMe && wg.play){
        this._rnd.grapple = grapple.superior ? 'superior' : 'inferior';
        if(grapple.superior && this._combatMind()) this._bundledPress = true;   // v0.6.156: "[POT] ... with 1 optional press" -- Nosferatu's only press engine, never consumed before
        ...
        this.note('Trainbot combat \u2014 ' + myVamp.name + ' plays Immortal Grapple: strikes that are not hand strikes cannot be used this round (by either combatant).');   // v0.6.156: frozen phrase, read by the opponent (silent since v0.6.114)
        this.push();
      }
    }
    ...
    if(ce && this.decide('combat-end', { ce, vamp: myVamp }).end){   // v0.6.123: Strike: combat ends (Majesty). On the wire it IS the dodge shape -- no damage either way this round -- plus a `Combat ends` line so a bot opponent does not press into a fight that is over. It resolves before every other strike, so environmental damage (Carrion Crows) does not land either.
      ...
      this._rnd.strike = ce.name; this._rnd.dealt = 0; this._rnd.taken = 0;
      await this._awaitOppStrike(this._strikeWaitMs(continued));   // consume their strike line (the dodge branch's reason): keeps the one-slot stash clean for the NEXT combat
    ...
    if(dodge && dd.dodge){           // steg 6 (16 July): Strike: dodge -- no damage in EITHER direction this round FROM THE STRIKE ITSELF (so no prevent needed there); the vampire's own strike slot is consumed by the dodge
      ...
      this._rnd.strike = dodge.name; this._rnd.dealt = 0; this._rnd.taken = this._oppCarrionCrows ? this._oppCarrionCrows.n : 0;
      await this._awaitOppStrike(this._strikeWaitMs(continued));   // v0.6.4: a dodge needs nothing from the opponent (0/0 regardless) -- but consuming their strike line here keeps the one-slot stash clean for the NEXT combat, and synchronises the round's end
    ...
    const longFist = this._range === 'long';   // v0.6.156: hand strikes cannot be used at long range (rulebook 6.4.3) -- a fist at long deals 0; the frozen strikeC channel carries 'for 0' so the opponent's parser reads the truth
    ...
    const fistDmg = longFist ? 0 : Math.max(0, 1 + sbN4);   // v0.6.156: what the fist would land this round
    const stealOk = steal && (this._combatMind()
      ? (steal.n >= Math.max(fistDmg, strike ? (strike.n | 0) : 0) && ((myVamp.blood | 0) < (myVamp.cap | 0) || fistDmg === 0))   // v0.6.156: a steal DRAINS the opponent whatever my overflow -- at long range (fist 0) it is always better than nothing; at close the v1 tie-break (below capacity) stands
      : ((myVamp.blood | 0) < (myVamp.cap | 0) && (!strike || steal.n >= (strike.n | 0))));
    if(stealOk){
      ...
      if(this._rnd) this._rnd.stole = steal.n;   // v0.6.160: the steal is its own column (blood swing, not damage)
    } else if(strike){
      ...
      if(longFist){ dealt = 0; strikeLine = myVamp.name + ' strikes with hands for 0 (long range)'; }
    ...
    this._rnd.strike = (steal && strikeLine.includes(steal.name)) ? steal.name : (strike && strikeLine.includes(strike.name)) ? strike.name : 'hands'; this._rnd.dealt = dealt;   // v0.6.156: combat-round trace row
    ...
    const opp = await this._awaitOppStrike(this._strikeWaitMs(continued));
    ...
    const silentLong = !opp && !continued && this._range === 'long' && !this._oppHasRanged();   // v0.6.157 (Johan, before the deploy): a SILENT opponent (a human table) at LONG range -- their rulebook default is a hand strike, which deals 0 at long; assume 1 only when their board shows a ranged option (a gun, a steal / ranged card seen)
    let taken = opp ? (opp.dodge ? 0 : (opp.dealt | 0)) : (continued || silentLong ? 0 : 1);
    this.o.log('combat incoming (' + myVamp.name + '): ' + (opp ? (opp.dodge ? opp.vamp + ' dodged \u2014 0' : opp.vamp + ' dealt ' + opp.dealt) : silentLong ? 'no strike line within the window \u2014 long range and no ranged option seen: a hand strike deals 0' : 'no strike line within the window \u2014 assuming the rulebook default 1'));
    ...
    if(prevent){
      ...
      if(prevent.press && this._combatMind()) this._bundledPress = true;   // v0.6.156: Hidden Strength [FOR] -- the prevent grants an optional press this round (Ventrue uses it to press to END)
    }
    ...
    if(unmended === 0){
      ...
      if(taste){
        ...
        if(oppT && oppT.n >= (this._combatMind() ? 2 : 1)){   // v0.6.156 (Johan): Taste of Vitae after a Roundhouse / Crows round -- cmb1: 8 of 12 were spent on a fist of 1
  ...
  _onLog(who, html){
    ...
    if(who !== this.o.name && (m = t.match(L12.aimAtVamp))){   // v0.6.155: the client's Target suffix -- 'Lisa plays Bum's Rush. Target: U's Colette.' / "Lisa's card action is used. Target: U's Colette." A human's rush has no frozen line of its own (only Deep Song's bot-emitted one), so the aim is remembered and `rush` in chat cashes it in
      if(m[3] === this.seatName(this.seat) && this.board.some(c => c && c.kind === 'crypt' && c.zone === 'ready' && c.name === m[4]))
        this._lastAim = { who, actor: m[1], card: m[2] || null, vamp: m[4], at: Date.now() };
    }
    ...
    if(who !== this.o.name) this._parseCombatNarration(t);   // v0.6.156: combat event lines ride the log tier since v0.6.114 -- parse them here too
  }
  ...
  _interceptSecondLook(){            // v0.6.143d (Johan, 20 Sep: 'a bot that plays ONE intercept card even when it holds more and more are needed is a clear limitation / bug'): the blocker's side of the exchange. When the tally moves and my block now FAILS, I look again: can the intercept I still hold reach their stealth? Then the next card goes down; if it cannot, the cards are kept.
    ...
    if(!go) return;
    if(p._icQ) return; p._icQ = true;
    this._afterBeat('react', () => { p._icQ = false; if(this.pending !== p || !p.contested || !p.blockerVamp || this.out) return false;   // v0.6.152: the card lands after the reaction beat, if the block still fails as the counter then stands
                                     const t2 = this.tally; return !!(t2 && t2.mode === 'block' && (t2.b | 0) < (t2.a | 0) && this.hand[ic.idx] === ic.name); }, () => {
      const t2 = this.tally, b2 = t2.b | 0;
      p.reacted.add(FX_NORM(ic.name));
      ...
      this.fxClone({ name: ic.name, kind: 'lib' }, '+' + ic.s + ' intercept', { actor: blocker });
      this.send({ t: 'log', html: '<b>' + esc(blocker.name) + '</b> plays <b>' + esc(ic.name) + '</b>: +' + ic.s + ' intercept.' });
      this._setTallyB(b2 + ic.s); this.push();
    });
  }
  _stealthSecondLook(){               // v0.6.143c THE SECOND LOOK (smoke1: `need` was 1 in 20 of 20 decisions -- the first look fires on the blocker's 'Block!' SHOUT, before it has played its intercept and written the tally; and nothing ever looked again, so no second stealth card was ever played). When the tally moves and the block currently SUCCEEDS, the actor looks at the real numbers: can my remaining stealth reach it?
    ...
    { const key = sa + ':' + b; if(av._lookKey === key) return; av._lookKey = key; }   // v0.6.152: the same counter state is looked at once (the chat grammar's write and its echo both arrive here)
    ...
    if(d.spend && st) this._queueStealthSpend(st, av, a);   // v0.6.152: after the beat
  }
  ...
  _sayAhead(who, av){                   // v0.6.152 (Johan): 'I currently have N stealth' -- said to a HUMAN blocker when the counter already favours me, once per (blocker, a, b); the bot then waits for played intercept (the tally moving runs _stealthSecondLook)
    const t = this.tally; if(!t || t.mode !== 'block' || !av || !av.v) return false;
    const ta = t.a | 0, tb = t.b | 0; if(ta <= tb) return false;
    const human = !((this.players || []).find(p => p.name === who) || {}).bot;
    const key = who + ':' + ta + ':' + tb;
    if(av._aheadKey !== key){ av._aheadKey = key;
      this._emit('stealth-ahead', { vamp: av.v.name, from: who, tally: { a: ta, b: tb } });
      this.o.log('block attempt by ' + who + ': stealth ' + ta + ' vs intercept ' + tb + ' \u2192 ahead, waiting for intercept');
      if(human){ const a = this._ask; const noMinion = !!(a && !a._lineBlockFrom && a._sayBlockFrom === who);   // v0.6.153: the shout came without a minion line -- fold the who-blocks hint into THIS bubble and stand the grace down (two bubbles a second apart read as nagging)
        if(a && a._whoT){ clearTimeout(a._whoT); a._whoT = null; }
        this.chat(av.v.name + ' currently has ' + ta + ' stealth \u2014 ' + (noMinion ? 'right-click your blocker and choose Block, then ' : '') + 'play your intercept, or resolve the block.'); } }
    return true;
  }
  _stealthFirstLook(who, av, a, st){   // v0.6.147: the v0.6.141-143 first look, lifted out of _onBlockAttempt unchanged so the tally net can run it too; returns true when a stealth card went down
    if(this._sayAhead(who, av)) return false;   // v0.6.152: my stealth already beats their intercept -- no card, say so, wait
  ...
  _queueStealthSpend(st, av, a){        // v0.6.152: a spend decided on a tally that just moved lands after the reaction beat -- if the counter still says the block succeeds and nothing else has landed meanwhile (one queued spend at a time)
    if(av._spendQ) return; av._spendQ = true;
    this._afterBeat('react', () => { av._spendQ = false; if(this._ask !== a || this._actingVamp !== av || !a.contested || this.out) return false;
                                     const t = this.tally; if(!t || t.mode !== 'block' || (t.b | 0) < (t.a | 0)) return false;   // the block fails as it stands now (they withdrew, or a verdict landed)
                                     return this.hand[st.idx] === st.name || !!st.fromBoard; },
                              () => this._spendStealthCard(st, av, a));
  }
  _onBlockAttempt(who){
    { const a0 = this._ask;
    ...
    if(cb){
      ...
      if(d.spend){
        a.contested = true; this._restartAskTimer();   // v0.6.152: the contest exists until the card lands after the beat
        av._lookQ = (av._lookQ | 0) + 1;   // v0.6.153
        this._afterBeat('react', () => { av._lookQ = Math.max(0, (av._lookQ | 0) - 1); return this._ask === a && this._actingVamp === av && a.contested && !this.out && this.hand[cb.idx] === cb.name; }, () => {
    ...
    if(av && !a.contested && !(botBlocker && this.o.stealthStack !== 0)){
      /* v0.6.152 (Johan's 1v1 report, 22 Sep): the first look used to run in the same tick as the shout -- before the client's 120 ms tally
         (a Sport Bike's intercept), before the human had played anything, and it floored `need` to 1, so a Govern at superior (tally 1-0)
         answered every Block! with a stealth card it did not need. Now: the contest opens at once, the LOOK waits the reaction beat and
         reads the counter as it then stands; while my stealth already beats their intercept it says so and waits for played intercept. */
      av._lookKey = null;   // a new attempt: its counter states are judged afresh
      a.contested = true; this._restartAskTimer();
      av._lookQ = (av._lookQ | 0) + 1;   // v0.6.153: a look is pending -- the verdict clock waits for it
      this._afterBeat('react', () => { av._lookQ = Math.max(0, (av._lookQ | 0) - 1); return this._ask === a && this._actingVamp === av && a.contested && !this.out; }, () => {
        if(av._lookKey || av._spendQ){ this._sayAhead(who, av); return; }   // a tally arrived during the beat and the second look already judged the real counter (or queued a card) -- one judgment per state
        const st2 = this._bestStealthFor(av.v, av.played, av.target);
        if(st2) this._stealthFirstLook(who, av, a, st2); else this._sayAhead(who, av);
      });
      return;
    }
      ...
      av._deferred = { who, at: Date.now() }; av._lookKey = null;
  ...
  _parseCombatNarration(txt){        // v0.6.156 (found reading serie-cmb1): v0.6.114 moved the bot's combat EVENT lines (Crows, and the deleted maneuver / press / Grapple 'duplicates') from chat to the log tier -- but these parsers only ever ran on CHAT, so since 16 Sep a bot's press never reached the opposing bot (a pressing Tremere fought round 2 alone and stole blood from nobody), a maneuver never moved the shared range, Grapple never restricted the opponent and Carrion Crows never pecked. Now fed from _onChat AND _onLog (the server never echoes a sender's own log lines; chat self-filters at _onChat's top), and the three silent announce sites emit their frozen phrase as a note() again (log tier, v0.6.114's own rule: an event is a note).
    ...
    let tm = txt.match(/(?:dealt; |^(?:\S+ combat \u2014 )?)([^;]+?) (?:takes|dodges the strike .*?takes) (\d+) \(loses (\d+) blood(?:, now (\d+))?/);   // v0.6.156: + 'now M' (their blood after the round)   // v0.6.103 (#1): the vampire that TOOK the damage (after 'dealt; ' in the normal line, after the prefix in the dodge+crows line) -- the old lazy (.+?) captured the whole strike clause
    if(tm && !this._stashAccept(tm[1])) tm = null;   // v0.6.103 (#1): not my opponent -> not my stash
    if(tm){
      this._oppTaken = { vamp: tm[1], n: parseInt(tm[3], 10), down: /goes to torpor|goes into torpor|is BURNED/.test(txt), now: tm[4] != null ? parseInt(tm[4], 10) : null };   // v0.6.156: down = their own narration says torpor/burn; now = their blood after the round
      this._oppLastTaken = Object.assign({ at: Date.now() }, this._oppTaken);   // v0.6.156: a NON-consumed copy for _wantPress (the stash above is consumed by Taste of Vitae's waiter)
    ...
    /* v0.6.160 (Johan's question about how steals are counted, 22 Sep): the VICTIM side of Steal Blood was never built. Since v0.6.19
       the stealing bot gains the blood and its frozen strike line says "for 0" (correct: a steal is not damage) -- but the opponent
       bot read that 0 and lost NOTHING. Every Theft of Vitae against a bot for two months created blood from nowhere. The stealer's
       note carries the amount ("X strikes with CARD: steals N blood (not damage; ...)"); the victim now loses min(N, blood) -- not
       damage, so no torpor from it on its own (rulebook: a steal is not damage) -- and records it on the round row. */
    const sm = this._inCombat && this._combatMyVamp ? txt.match(/^(?:\S+ combat \u2014 )?(.+?) strikes with (.+?): steals (\d+) blood/) : null;
    if(sm && this._stashAccept(sm[1])){
      const v = this._combatMyVamp, n = parseInt(sm[3], 10), lost = Math.min(n, v.blood | 0);
      v.blood = (v.blood | 0) - lost; if(this._rnd) this._rnd.stolenFromMe = (this._rnd.stolenFromMe | 0) + lost;
      this.o.log('combat (' + v.name + '): ' + sm[1] + ' steals ' + lost + ' blood with ' + sm[2] + ' (not damage)');
      this.push();
    }
  ...
  _onChat(who, msg){
    ...
    this._parseCombatNarration(txt);   // v0.6.156: the four opp-parsers (taken / press / maneuver / Crows / Grapple) now live in ONE method fed from BOTH tiers -- see _parseCombatNarration
    ...
    /* v0.6.152 (Johan's design, 22 Sep): INTERCEPT BY CHAT during a block against MY action. A bare number N = the blocker's total
       intercept right now (Sport Bike + Bowl = "2"), +N = intercept added since ("+1" -> 3). I book it on the shared counter for the
       human (b is theirs, but they said the number) and judge the real counter. Only while my action is contested, only from the
       blocker (or the @Name form); outside such a duel a bare number means nothing to me. */
    if(this._ask && this._ask.contested && this._actingVamp){
      const a0 = this._ask, im = txt.replace(/^\s*@?trainbot[:,]?\s*/i, '').match(/^\s*(\+)?(\d{1,2})\s*$/i) || (namedMe0(this, txt) ? txt.replace(nameRe0(this), '').match(/^\s*(\+)?(\d{1,2})\s*$/i) : null);
      if(im){
        const blocker = a0._lineBlockFrom || a0._sayBlockFrom || null, named = nameRe0(this).test(txt) || /^\s*@?trainbot[:,]/i.test(txt);
        if(blocker && who !== blocker && !named) return;   // somebody else's number is table talk
        const cur = (this.tally && this.tally.mode === 'block') ? this.tally : { a: 0, b: 0 }, n = parseInt(im[2], 10) | 0;
        const nb = Math.min(99, im[1] ? (cur.b | 0) + n : n);
        this._emit('intercept-chat', { from: who, text: txt, b: nb, prev: cur.b | 0, a: cur.a | 0 });
        this.o.log('intercept by chat from ' + who + ': "' + txt + '" \u2192 intercept ' + nb + ' (was ' + (cur.b | 0) + ')');
        this._setTallyB(nb);                                   // write + local mirror (offline has no echo)
        this.note(who + ' has ' + nb + ' intercept (booked on the counter).');
        this._stealthSecondLook();                             // judge the real counter now; the online echo is deduped by state
        this._restartAskTimer();
        return;
      }
    }
    ...
    /* v0.6.155 THE RUSH BY CHAT (Johan, 22 Sep -- interim until the cardfx rush sweep tags Bum's Rush & co): a human who has played a rush card with
       one of MY vampires as Target types `rush` (or `rush <vampire>` when the aim did not reach me). I resolve it exactly like the frozen Deep Song
       line: the block window first when I have an unlocked body or a wake (comply on my Pass), else lock and fight. Scope: the aim's owner decides
       which bot; with no aim a bare `rush` goes to the sender's PREY bot (the bleed grammar's rule) or the named one. (v0.6.154 removed the July
       reply 'has no combat module yet (M2)' -- untrue since v0.4.0.) */
    { const rm = bodyMatch.match(/^rush(?:\s+(.+?))?\s*$/i);
      if(rm && this.started && !this.out){
        const sp = (this.players || []).find(q => q && q.name === who); if(sp && sp.bot) return;   // bots rush by their own frozen line, never by chat
        const aim = (this._lastAim && this._lastAim.who === who && Date.now() - this._lastAim.at < 120000) ? this._lastAim : null;
        const mine = this.board.filter(c => c && c.kind === 'crypt' && c.zone === 'ready');
        let vc = null;
        if(rm[1]){ const q = rm[1].trim().toLowerCase(); vc = mine.find(c => c.name.toLowerCase() === q) || mine.find(c => c.name.toLowerCase().startsWith(q)) || null; }
        else if(aim) vc = mine.find(c => c.name === aim.vamp) || null;
        else if(mine.length === 1) vc = mine[0];
        if(!vc){
          if(!(namedMe || aim || this.seatName(this.predSeat()) === who || this.seatName(this.preySeat()) === who)) return;   // not aimed at me and not my neighbour: table talk
          if(!namedMe && !aim && mine.length > 1 && this.seatName(this.predSeat()) !== who) return;   // a bare rush with no aim goes to the sender's prey
          return this.chat(this.o.name + ': which of my vampires? Set it as target and play your rush card, or type "rush <name>"' + (mine.length ? ' (' + mine.map(c => c.name).join(', ') + ')' : '') + '.');
        }
        if(!namedMe && !aim && this.seatName(this.predSeat()) !== who) return;   // rush at a bot that is not the sender's prey needs the aim or the @Name form
        const rusher = (aim && aim.actor && !/face-down card/i.test(aim.actor)) ? aim.actor : (who + "'s vampire");
        this._emit('rush-chat', { from: who, vamp: vc.name, rusher, card: aim ? aim.card : null, aimed: !!aim });
        this.o.log('rush by chat from ' + who + ': ' + rusher + ' rushes ' + vc.name + (aim && aim.card ? ' (' + aim.card + ')' : ''));
        this._lastAim = null;
        if(this._rushWindowOpens(who, rusher, vc)) return;              // the owner may block with another unlocked minion (v0.6.107); comply follows my Pass
        vc.locked = true; this.push();
        this.note('Trainbot: ' + vc.name + ' locks and defends (rushed by ' + rusher + ').');
        this._emit('rushed', { vamp: vc.name, by: rusher });
        this._clearCombatStashes(); this._combatArmed = true;
        this._resolveCombat(vc, rusher).catch(e => this.o.log('rush defend error:', e.message));
        return;
      } }
    if(/^help\b/i.test(bodyMatch) && (namedMe || this._isSpokesBot())){
      const several = (this.players || []).filter(p => p.bot && !p.vacant).length > 1;
      return this.chat(this.o.name + ' v' + BOT_VERSION + ' \u2014 I understand: "bleed N", "vote N", "block"/"no" (when I ask), "N"/"+N" (your intercept while you block me), "rush" or "rush <vampire>" (after playing your rush card with my vampire as Target), "help".' + (several ? ' Aim at one bot with "@Name bleed N".' : ''));
    }
  }
  ...
  _deferredPending(){                 // v0.6.153: a block-duel card of mine is waiting out the react beat (+ a chat hold) -- the verdict must not be declared over its head
    const av = this._actingVamp; return !!(av && ((av._lookQ | 0) > 0 || av._spendQ));
  }
  _askTimeout(){
    const a = this._ask; if(!a) return;
    if(a.contested && this._deferredPending()){   // v0.6.153 (self-review after v0.6.152): with `--ask-secs 3` the clock could beat a deferred stealth / cancel card (react beat 1.5 s + a 2.4 s chat hold after "currently has N stealth") and declare a block that the card was about to break. Wait in short steps until it has landed; _spendStealthCard's own restart then runs the clock afresh.
      if((a._deferWaits | 0) < 20){ a._deferWaits = (a._deferWaits | 0) + 1; a.deadline = Date.now() + 400; a.timer = setTimeout(() => this._askTimeout(), 400); return; } }
